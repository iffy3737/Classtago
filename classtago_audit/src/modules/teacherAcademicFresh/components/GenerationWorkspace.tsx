...
export function GenerationWorkspace({ title, description, taskType, assignments, materials, onSaved, children, fields = [], allowPublish = false, allowAttachment = false, allowDivisionCopy = false }: Props) {
  ...
  const [pendingMariaAcademic, setPendingMariaAcademic] = useState<MariaAcademicGenerateCommand | null>(null);
  const [mariaAcademicStage, setMariaAcademicStage] = useState('');
  const [mariaAcademicStatus, setMariaAcademicStatus] = useState('');
  const normalizeMariaText = (value: unknown) => String(value || '').normalize('NFKC').toLowerCase().replace(/[^a-z0-9\u0900-\u097f\u0600-\u06ff]+/g, ' ').trim();
  ...
  useEffect(() => {
    const accept = (command: any) => {
      if (command?.type !== 'maria_academic_generate' || command?.taskType !== taskType) return;
      setPendingMariaAcademic(command as MariaAcademicGenerateCommand);
      setMariaAcademicStage('scope');
      setMariaAcademicStatus('Maria is preparing this request inside the existing academic builder. Nothing will be saved automatically.');
    };
  ...
  useEffect(() => {
    if (!pendingMariaAcademic || mariaAcademicStage !== 'scope' || !assignments.length) return;
    const requestedId = String(pendingMariaAcademic.assignmentId || '');
    const subject = normalizeMariaText(pendingMariaAcademic.subjectName);
    const className = normalizeMariaText(pendingMariaAcademic.className);
    const division = normalizeMariaText(pendingMariaAcademic.division);
    const matches = assignments.filter(item => requestedId ? item.id === requestedId :
      (!subject || normalizeMariaText(item.subjectName) === subject) &&
      (!className || normalizeMariaText(item.className) === className) &&
      (!division || normalizeMariaText(item.division) === division));
    if (matches.length !== 1) {
      setError(matches.length ? 'Maria found more than one matching assigned subject. Please specify Class/Division.' : 'Maria could not find that Subject in your current assigned academic scope.');
      setMariaAcademicStatus('Maria stopped before changing the existing academic form.');
      setPendingMariaAcademic(null); setMariaAcademicStage('');
      return;
    }
    const match = matches[0];
    setAssignmentId(match.id);
    setPrompt(String(pendingMariaAcademic.instruction || '').trim());
    if (pendingMariaAcademic.structuredInputs && typeof pendingMariaAcademic.structuredInputs === 'object') {
      const allowed = new Set(fields.map(field => field.key));
      setFieldValues(current => ({ ...current, ...Object.fromEntries(Object.entries(pendingMariaAcademic!.structuredInputs || {}).filter(([key]) => allowed.has(key))) }));
    }
    setResult(null); setEditableContent(''); setError(''); setSaveMessage('');
    setMariaAcademicStage('source');
    setMariaAcademicStatus(`Maria selected ${match.className} · ${divisionScreenLabel(match.division)} · ${match.subjectName}. Validating the existing source rules.`);
  }, [pendingMariaAcademic, mariaAcademicStage, assignments, fields]);
  ...
  useEffect(() => {
    if (!pendingMariaAcademic || mariaAcademicStage !== 'source' || !assignmentId) return;
    if (isTeachingDiary) {
      setSelectedMaterials([]); setChapters(''); setMariaAcademicStage('generate');
      setMariaAcademicStatus('Maria filled the factual diary inputs. The existing diary generator is ready to format them.');
      return;
    }
    if (usesTextbookCore) {
      if (!scopedMaterials.length) {
        setMariaAcademicStatus('Maria is waiting for the assigned Textbook to become AI Ready in the existing academic module.');
        return;
      }
      const sourceIds = scopedMaterials.map(item => item.id);
      // The existing assignment hydration effect remains authoritative. Maria waits
      // for that proven source selector to finish instead of racing/replacing it.
      if (selectedMaterials.join('|') !== sourceIds.join('|')) {
        setMariaAcademicStatus('Maria is waiting for the existing Textbook source selector to finish loading.');
        return;
      }
      const requested = (pendingMariaAcademic.chapters || []).map(String).map(value => value.trim()).filter(Boolean);
      if (!requested.length) { setChapters(''); setMariaAcademicStage('generate'); return; }
      if (!availableChapters.length) { setMariaAcademicStatus('Maria is waiting for the existing Textbook chapter index.'); return; }
      const first = requested[0];
      const key = normalizeMariaText(first);
      const exact = availableChapters.filter(option => normalizeMariaText(option.replace(/^\d+\s*[—-]\s*/, '')) === key);
      const loose = exact.length ? exact : availableChapters.filter(option => { const clean=normalizeMariaText(option.replace(/^\d+\s*[—-]\s*/, '')); return clean.includes(key) || key.includes(clean); });
      const unique = [...new Set(loose.map(option => option.replace(/^\d+\s*[—-]\s*/, '')))];
      if (unique.length !== 1) {
        setError(unique.length ? `Maria found more than one chapter matching “${first}”. Please select the exact chapter.` : `Maria could not find “${first}” in the assigned Textbook chapter index.`);
        setMariaAcademicStatus('Generation stopped safely before AI generation.'); setPendingMariaAcademic(null); setMariaAcademicStage(''); return;
      }
      setChapters(unique[0]); setMariaAcademicStage('generate');
      setMariaAcademicStatus(`Maria matched “${unique[0]}” to the existing Textbook index.`);
      return;
    }
    setSelectedMaterials(scopedMaterials.map(item => item.id));
    setChapters((pendingMariaAcademic.chapters || []).join(', '));
    setMariaAcademicStage('generate');
  }, [pendingMariaAcademic, mariaAcademicStage, assignmentId, isTeachingDiary, usesTextbookCore, scopedMaterials.map(item=>item.id).join('|'), availableChapters.join('|'), selectedMaterials.join('|')]);
  ...
  useEffect(() => {
    if (!pendingMariaAcademic || mariaAcademicStage !== 'generate' || busy) return;
    setMariaAcademicStage('running');
    setMariaAcademicStatus('Maria started the existing academic generator. It will stop at Teacher Review Draft; Save/Publish remain manual.');
    void generate().finally(() => { setPendingMariaAcademic(null); setMariaAcademicStage(''); });
  }, [pendingMariaAcademic, mariaAcademicStage, busy, assignmentId, selectedMaterials.join('|'), chapters, prompt, JSON.stringify(fieldValues)]);
  ...
  return (
    <section className="workspace">
      ...
      <div className="card">
        ...
        {mariaAcademicStatus && <div className="alert" style={{borderColor:'#ddd6fe',background:'#f5f3ff',color:'#5b21b6'}}><b>Maria:</b> {mariaAcademicStatus} <span style={{opacity:.75}}>Existing Save/Publish controls remain manual.</span></div>}
