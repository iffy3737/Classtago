...
export function CombinedQuestionPaperBuilder({group,currentAssignment,assignments,materials,exam,combinedTotalMarks,durationMinutes,onTotalMarksChange,onGenerated,onBusy,onError}:Props){
  ...
  const [generationStage,setGenerationStage]=useState('Preparing Combined Paper…');
  ...
  const generate=async()=>{
    ...
    onError('');
    setGenerating(true);
    setGenerationStage('Preparing Combined Paper…');
    ...
    try{
      ...
      for(let targetIndex=0;targetIndex<targets.length;targetIndex++){
        ...
        setGenerationStage(`Generating ${member.subjectName} section (${targetIndex+1}/${targets.length})…`);
      ...
      if(!group.currentTeacherHasAll){
        setGenerationStage('Preparing your Subject section for Clerk…');
      ...
      setGenerationStage('Combining all generated Subject sections…');
    ...
    }catch(e){onError(e instanceof Error?e.message:'Combined Question Paper generation failed.');}finally{setGenerating(false);setGenerationStage('Preparing Combined Paper…');setGenerationIndex(0);onBusy(false);}
  };
  ...
  return <div className="combined-paper-builder">
    ...
    {generating&&<div className="combined-generation-status" role="status" aria-live="polite"><div className="combined-generation-spinner" aria-hidden="true"/><div className="combined-generation-copy"><strong>{generationStage}</strong><small>AI is preparing the paper from the selected Subject Textbook. Please keep this screen open and do not press Generate again.</small></div>{generationIndex>0&&<span className="combined-generation-count">{generationIndex}/{group.currentTeacherHasAll?group.members.length:editableMembers.length}</span>}</div>}
    <div className="actions"><button className="btn primary" type="button" onClick={()=>void generate()} disabled={generating}>{generating?'Generating…':group.currentTeacherHasAll?'Generate Complete Combined Paper':'Generate My Subject Section for Clerk'}</button></div>
  </div>;
}
