...
export default function AcademicSetupModule({
  ...
  const addSubjectGroup = () => {
    // R2.5.93: Subject Group Master is reserved for faculty/curriculum grouping
    // such as Science, Commerce, Arts and Languages. Combined Question Paper
    // groups are configured separately inside the canonical Subject Master.
    ...
    if (selectedGroupSubjects.length < 1) {
      triggerToast('Select at least one Subject for this faculty/curriculum group.', 'error');
      return;
    }

    const codeBase = name.toUpperCase().replace(/[^A-Z0-9]+/g, '_').replace(/^_+|_+$/g, '').slice(0, 30) || 'GROUP';
    const newGroup: SubjectGroup = {
      id: `grp_${Date.now()}`,
      groupName: name,
      subjectIds: [...selectedGroupSubjects],
      groupCode: codeBase,
      combinedPaperEnabled: false
    };
    ...
    handleSaveSetup(updated, 'ADD_SUBJECT_GROUP', `Created faculty/curriculum Subject Group: ${name}`);
  };
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        {activeCategory === 'subjects' && (
          <div className="space-y-8">
            ...
            {/* Subject Groups — faculty/curriculum classification only. */}
            {showSubjectGroups && (<>
            <div className="bg-slate-50/50 border border-slate-200/80 rounded-2xl p-6 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-200 pb-3">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 text-[10px] font-medium text-slate-500">Faculty/curriculum grouping only — e.g. Science, Commerce, Arts, Languages. Combined Question Papers are configured in Subject Master.</p>
                  </div>
                </div>
              </div>
              ...
              {isFinalAuthority && (
                <div className="rounded-2xl border border-purple-200 bg-white p-4 shadow-sm">
                  <label className="text-xs font-bold text-slate-700">Group / Faculty Name
                    <input
                      type="text"
                  ...
                  <div className="mt-3">
                    <div className="mb-2 flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-wider text-slate-500">Subjects in this group</span><span className="text-[10px] font-bold text-slate-400">{selectedGroupSubjects.length} selected</span></div>
                  ...
                  <div className="mt-3 flex flex-wrap items-center justify-between gap-2">
                    <p className="text-[10px] text-slate-500">This grouping does not assign Teachers and does not create a Combined Question Paper.</p>
                    <button type="button" onClick={addSubjectGroup} className="flex cursor-pointer items-center gap-1 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white hover:bg-blue-700">
                      <Plus className="h-3.5 w-3.5" /><span>Add Group</span>
                    </button>
                  </div>
                </div>
              )}
                ...
                {setup.subjectGroups.filter((grp) => !grp.combinedPaperEnabled).map((grp) => (
                  <div key={grp.id} className="p-4 bg-white border border-slate-200 rounded-xl space-y-2 shadow-sm">
                    <div className="flex justify-between items-center border-b border-slate-100 pb-1.5">
                      ...
                      {isFinalAuthority && <button type="button" onClick={() => deleteSubjectGroup(grp.id)} className="text-slate-400 hover:text-rose-600 cursor-pointer"><Trash2 className="w-3 h-3" /></button>}
                    </div>
                    <div className="flex flex-wrap gap-1">
                      {grp.subjectIds.map(subId => {
                        const sObj = setup.subjects.find(s => s.id === subId);
                        return sObj ? <span key={subId} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-[10px] rounded font-semibold">{sObj.subjectName}</span> : null;
                      })}
                      {grp.subjectIds.length === 0 && <span className="text-[10px] text-slate-400 italic">No subjects mapped</span>}
                    </div>
                  </div>
                ))}
              </div>
