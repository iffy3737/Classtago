...
public class EdunixoDocumentScanActivity extends AppCompatActivity {
    ...
    private SaveResult savePdf(Uri source) throws Exception {
        ...
        String fileName = "EDUNIXO-Scan-" + stamp + ".pdf";
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            ContentValues values = new ContentValues();
            values.put(MediaStore.Downloads.DISPLAY_NAME, fileName);
            values.put(MediaStore.Downloads.MIME_TYPE, "application/pdf");
            values.put(MediaStore.Downloads.RELATIVE_PATH, Environment.DIRECTORY_DOWNLOADS + "/EDUNIXO");
            ...
            return new SaveResult(destination, fileName, "Downloads/EDUNIXO");
        }
        File root = getExternalFilesDir(Environment.DIRECTORY_DOCUMENTS);
        if (root == null) root = getFilesDir();
        File dir = new File(root, "EDUNIXO");
        if (!dir.exists() && !dir.mkdirs()) throw new IllegalStateException("EDUNIXO document folder could not be created.");
        ...
        return new SaveResult(Uri.fromFile(target), fileName, "EDUNIXO app documents");
    }
