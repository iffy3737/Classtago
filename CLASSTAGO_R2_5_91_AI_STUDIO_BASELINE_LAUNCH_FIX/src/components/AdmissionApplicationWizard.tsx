...
import { ArrowLeft, ArrowRight, Check, CheckCircle2, ClipboardCheck, FileText, KeyRound, Loader2, ScanLine, Search, ShieldCheck, Trash2, UploadCloud, X } from 'lucide-react';
...
import { requestSchoolOtp, verifyPublicOtp } from '../lib/otpClient';
...
export default function AdmissionApplicationWizard({ open, onClose, schoolSlug, schoolName, admissionTitle, admissionDescription, campaign, language }: Props) {
  ...
  const resetAdmissionOtp = () => { setOtpToken(''); setOtp(''); setOtpVerified(false); setOtpMasked(''); setOtpDelivery(''); };
  const update = (key:keyof FormState,value:any) => { setForm(current => ({...current,[key]:value})); if (key === 'mobile' || key === 'email') resetAdmissionOtp(); setError(''); };
  const sendAdmissionOtp = async () => {
    if (!/^\+?[0-9\s-]{7,18}$/.test(form.mobile.trim())) { setError('Enter a valid guardian mobile number before requesting OTP.'); return; }
    setOtpBusy(true); setError('');
    try { const result = await requestSchoolOtp(schoolSlug,form.mobile.trim(),'admission_verification',form.email.trim()); setOtpToken(result.challengeToken); setOtp(''); setOtpVerified(false); setOtpMasked(result.maskedMobile || 'guardian mobile'); setOtpDelivery(result.deliveryLabel || 'available OTP channel'); }
    catch(failure:any){ setError(failure?.message || 'Admission mobile OTP could not be sent.'); }
    finally { setOtpBusy(false); }
  };
  const verifyAdmissionOtp = async () => {
    if (!otpToken || otp.length !== 6) { setError('Enter the 6-digit admission OTP.'); return; }
    setOtpBusy(true); setError('');
    try { await verifyPublicOtp(otpToken,otp); setOtpVerified(true); }
    catch(failure:any){ setOtpVerified(false); setError(failure?.message || 'Admission OTP verification failed.'); }
    finally { setOtpBusy(false); }
  };
  ...
  const submit = async () => {
    ...
    if (!otpVerified || !otpToken) { setError('Verify the guardian contact with OTP before submitting this admission application.'); return; }
    const validation = validateStep();
    if (validation) { setError(validation); return; }
    setSubmitting(true);
    ...
    try {
      const response = await fetch(`/api/public/schools/${encodeURIComponent(schoolSlug)}/admission-applications`, {
        ...
        body:JSON.stringify({
          ...
          message:form.message, consent:true, website:form.website, otpChallengeToken:otpToken
        })
      });
  ...
  return <div className="fixed inset-0 z-[100] flex items-center justify-center bg-[#02040b]/88 p-2 backdrop-blur-xl sm:p-5" role="dialog" aria-modal="true" aria-label={campaignTitle}>
    <div className="max-h-[96vh] w-full max-w-5xl overflow-y-auto rounded-[2rem] border border-white/12 bg-[#0a0f21] shadow-[0_35px_120px_rgba(0,0,0,.72)]">
      ...
      {reference ? <div className="p-8 text-center sm:p-14">
        ...
        {step===4 && <div className="space-y-5"><div className="rounded-2xl border border-[var(--school-accent)]/20 bg-[var(--school-accent)]/10 p-5"><ClipboardCheck className="h-7 w-7 text-[var(--school-accent)]"/><h3 className="mt-4 text-xl font-black">{copy.reviewBeforeSubmit}</h3><p className="mt-2 text-xs leading-6 text-slate-400">{copy.applicationNotAdmission}</p></div><div className="grid gap-3 sm:grid-cols-2">{[[ui.studentFullName,form.studentName],[ui.dateOfBirth,form.dateOfBirth],[ui.guardianFullName,form.guardianName],[ui.mobileNumber,form.mobile],[ui.classApplyingFor,form.classApplying],[ui.address,form.addressLine],[copy.previousSchool,form.previousSchool||'—'],[copy.requiredDocuments,Object.keys(files).length]].map(([label,value])=><div key={String(label)} className="rounded-xl border border-white/10 bg-white/[0.035] p-4"><div className="text-[9px] font-black uppercase tracking-wider text-slate-500">{label}</div><div className="mt-2 text-sm font-bold text-white">{value}</div></div>)}</div><div className="rounded-2xl border border-cyan-300/20 bg-cyan-300/[.07] p-4"><div className="flex items-start gap-3"><KeyRound className="mt-0.5 h-5 w-5 shrink-0 text-cyan-300"/><div><div className="text-xs font-black text-white">Guardian contact OTP verification *</div><p className="mt-1 text-[10px] leading-5 text-slate-400">OTP uses the school's Classtago multi-channel security flow: SMS + WhatsApp when available, with Email as an optional backup. Routine admission updates do not consume the SIM SMS quota.</p></div></div>{!otpToken?<button type="button" onClick={()=>void sendAdmissionOtp()} disabled={otpBusy||!form.mobile.trim()} className="mt-3 rounded-xl bg-cyan-300 px-4 py-2.5 text-xs font-black text-slate-950 disabled:opacity-40">{otpBusy?'Sending OTP…':'Send Admission OTP'}</button>:otpVerified?<div className="mt-3 flex items-center gap-2 rounded-xl border border-emerald-300/20 bg-emerald-300/10 px-3 py-2.5 text-xs font-black text-emerald-200"><CheckCircle2 className="h-4 w-4"/>Guardian contact verified {otpMasked?`(${otpMasked})`:''}{otpDelivery?` · ${otpDelivery}`:''}</div>:<div className="mt-3 flex flex-col gap-2 sm:flex-row"><input inputMode="numeric" maxLength={6} value={otp} onChange={e=>setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} placeholder="6-digit OTP" className="min-h-11 flex-1 rounded-xl border border-white/10 bg-slate-950 px-3 font-mono text-xs font-black tracking-[.22em] text-white outline-none"/><button type="button" onClick={()=>void verifyAdmissionOtp()} disabled={otpBusy||otp.length!==6} className="rounded-xl bg-emerald-400 px-4 py-2.5 text-xs font-black text-slate-950 disabled:opacity-40">{otpBusy?'Verifying…':'Verify OTP'}</button><button type="button" onClick={()=>void sendAdmissionOtp()} disabled={otpBusy} className="rounded-xl border border-white/10 px-3 py-2.5 text-[10px] font-black text-slate-300 disabled:opacity-40">Resend</button></div>}{otpToken && !otpVerified && otpDelivery && <div className="mt-2 text-[9px] font-bold text-cyan-200/80">OTP sent/queued via: {otpDelivery}. Email is optional.</div>}</div><label className="flex items-start gap-3 rounded-xl border border-white/10 bg-white/[0.035] p-4"><input type="checkbox" checked={form.consent} onChange={e=>update('consent',e.target.checked)} className="mt-0.5 h-4 w-4 accent-cyan-400"/><span className="text-xs leading-6 text-slate-400">{ui.consent}</span></label></div>}
