      ...
      {leadEditorOpen && selectedLead && (
        <Modal title="Demo Follow-up" subtitle="Keep the latest contact result, next follow-up time and internal note. Delete the record only after the work is complete and the lead is no longer required." onClose={() => !saving && setLeadEditorOpen(false)}>
          <div className="space-y-5">
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-between"><button type="button" onClick={() => void deleteLead(selectedLead)} disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-extrabold text-rose-700 hover:bg-rose-100 disabled:opacity-50"><Trash2 className="h-4 w-4" />Delete completed lead</button><div className="flex flex-col-reverse gap-3 sm:flex-row"><button type="button" onClick={() => setLeadEditorOpen(false)} disabled={saving} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void saveLeadFollowUp()} disabled={saving} className={primaryButton}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save Follow-up</button></div></div>
          </div>
        </Modal>
      )}
