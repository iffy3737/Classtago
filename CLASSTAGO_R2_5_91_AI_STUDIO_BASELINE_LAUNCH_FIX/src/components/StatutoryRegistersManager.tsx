...
export default function StatutoryRegistersManager({
  ...
  const getFilteredData = () => {
      ...
      case 'result':
        ...
        raw = exams.map(ex => {
          ...
          return {
            ...
            totalAppeared: exMarks.length || 24, // fallback default
            passed: exMarks.length ? passed : 22,
            failed: exMarks.length ? failed : 2,
            passRate: exMarks.length ? Math.round((passed / exMarks.length) * 100) : 92,
            academicYear: ex.academicYear || '2026-27'
          };
        });
        break;
      case 'staff':
    ...
    raw = raw.filter(item => {
      const year = item.academicYear || item.date?.substring(0, 4) || '2026-27';
  ...
  const handleAddScholarship = (e: React.FormEvent) => {
    ...
    setNewScholarship({ name: 'Pre-Matric Minority Scholarship', grNumber: '', amount: '4000', academicYear: '2026-27', status: 'Pending', paymentDetails: 'Pending Direct Benefit Transfer' });
  ...
  const translations = {
    en: {
      ...
      subtitle: 'Official digital record room and verified regulatory logs of National High School, Taloda.',
      ...
      <div className={`grid grid-cols-1 gap-6 ${focusedMode ? '' : 'lg:grid-cols-4'}`}>
        ...
        <div className={focusedMode ? 'w-full' : 'lg:col-span-3'}>
          <div id="printable-register-area" className="bg-amber-50/20 p-6 md:p-8 rounded-2xl border border-amber-200/60 shadow-inner text-left relative min-h-[600px]">
            ...
            {/* Stamp Logo of National High School Taloda */}
            <div className="border-b-4 border-double border-slate-800 pb-4 mb-6 flex flex-col items-center text-center">
              ...
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest mt-1">
                Taloda, Dist. Nandurbar, Maharashtra | School Code: 220101
              </p>
                        ...
                        <tr key={item.id || idx} className="hover:bg-amber-50/10 font-mono text-[11px] text-slate-700">
                            ...
                            <>
                              ...
                              <td className="font-sans">
                                <span className="text-[10px] bg-slate-100 px-1.5 py-0.5 rounded font-bold">
                                  Class {item.admissionClassId === 'c3' ? '9' : '10'} A (2026-27)
                                </span>
                              </td>
                          ...
                          {activeReg === 'examination' && (
                            <>
                              ...
                              <td>{item.academicYear || '2026-27'}</td>
                            </>
                          )}
            ...
            <div className="mt-12 pt-8 border-t-2 border-dashed border-slate-400 grid grid-cols-1 md:grid-cols-3 gap-6 text-center text-xs">
              <div>
                ...
                <div className="text-[10px] text-slate-500 mt-1">National High School, Taloda</div>
              </div>
            ...
            <form onSubmit={handleAddInspection} className="p-6 space-y-4">
              <div className="space-y-1.5">
                ...
                <input
                  type="text"
                  required
                  placeholder="e.g. Block Education Officer, Taloda Tehsil"
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden text-left">
            <div className="bg-indigo-950 text-white p-4 flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] text-indigo-300 mt-0.5">National High School, Taloda General Register Audit Control</p>
              </div>
            ...
            <form onSubmit={handleApplyCorrection} className="p-6 space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <textarea
                  required
                  rows={2}
                  placeholder="e.g. Corrected as per Birth Certificate Reg No. 122/2012 issued by Taloda Municipal Council"
