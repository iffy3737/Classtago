...
import { subscribeMariaDynamicCommands, takePendingMariaDynamicCommands, type MariaHomeworkCommand } from '../../../lib/mariaClientBridge';
...
export function HomeworkBuilder({ assignments, materials, editingRecord, onEditingCleared, onSaved, onNavigate }: Props) {
  ...
  const [pendingMariaHomework, setPendingMariaHomework] = useState<MariaHomeworkCommand | null>(null);
  const [mariaHomeworkStage, setMariaHomeworkStage] = useState('');
  const [mariaHomeworkStatus, setMariaHomeworkStatus] = useState('');
  ...
  useEffect(() => {
    const accept = (command:any) => { if (command?.type === 'maria_homework') { setPendingMariaHomework(command as MariaHomeworkCommand); setMariaHomeworkStage('scope'); setMariaHomeworkStatus('Maria is preparing this request inside the existing Homework builder.'); } };
    const unsubscribe = subscribeMariaDynamicCommands(accept);
    for (const command of takePendingMariaDynamicCommands('maria_homework')) accept(command);
    return unsubscribe;
  }, []);

  useEffect(() => {
    if (!pendingMariaHomework || mariaHomeworkStage !== 'scope' || !assignments.length) return;
    const requestedId = String(pendingMariaHomework.assignmentId || '');
    const subject = normalizeMariaAcademicText(pendingMariaHomework.subjectName);
    const className = normalizeMariaAcademicText(pendingMariaHomework.className);
    const division = normalizeMariaAcademicText(pendingMariaHomework.division);
    const matches = assignments.filter(item => {
      if (requestedId && item.id === requestedId) return true;
      return (!subject || normalizeMariaAcademicText(item.subjectName) === subject)
        && (!className || normalizeMariaAcademicText(item.className) === className)
        && (!division || normalizeMariaAcademicText(item.division) === division);
    });
    if (matches.length !== 1) {
      setError(matches.length ? 'Maria found more than one matching Homework assignment. Please specify Class/Division.' : 'Maria could not find that subject in your current assigned Homework scope.');
      setMariaHomeworkStatus('Maria stopped before changing any Homework data.');
      setPendingMariaHomework(null); setMariaHomeworkStage(''); return;
    }
    const match = matches[0];
    setMode('ai'); setAssignmentId(match.id); setChapters([]); setResult(null); setEditableContent(''); setHomeworkTitle(''); setError(''); setSaveMessage(''); setNewAttachments([]); clearSavedIdentity();
    const difficultyRaw = normalizeMariaAcademicText(pendingMariaHomework.difficulty);
    setDifficulty(difficultyRaw === 'easy' ? 'Easy' : difficultyRaw === 'challenging' || difficultyRaw === 'hard' ? 'Challenging' : 'Balanced');
    const requestedMinutes = Number(pendingMariaHomework.targetMinutes || 30); const allowedMinutes=[15,20,30,45,60];
    setTargetMinutes(allowedMinutes.reduce((best,value)=>Math.abs(value-requestedMinutes)<Math.abs(best-requestedMinutes)?value:best,30));
    const requestedLanguage=String(pendingMariaHomework.languageCode||'auto').trim().toLowerCase();
    setLanguageCode(requestedLanguage==='auto'||FALLBACK_LANGUAGE_CATALOGUE.some(option=>String(option.code)===requestedLanguage)?requestedLanguage:'auto');
    setTeacherInstruction(String(pendingMariaHomework.instruction||'').trim());
    setMariaHomeworkStage('chapters'); setMariaHomeworkStatus(`Maria selected ${match.className} · ${divisionScreenLabel(match.division)} · ${match.subjectName}. Waiting for the existing Textbook chapter list.`);
  }, [pendingMariaHomework, mariaHomeworkStage, assignments]);

  useEffect(() => {
    if (!pendingMariaHomework || mariaHomeworkStage !== 'chapters' || !assignment || assignment.id !== (pendingMariaHomework.assignmentId || assignment.id)) return;
    if (!scopedTextbooks.length) { setError('Maria cannot generate this Homework because the existing Homework module has no Textbook for this Class + Subject.'); setMariaHomeworkStatus('No generation was attempted.'); setPendingMariaHomework(null); setMariaHomeworkStage(''); return; }
    if (!readyTextbooks.length && (preparingTextbooks.length || chapterLoading)) return;
    if (!readyTextbooks.length) { setError(generateBlocker || 'Textbook is not AI Ready yet.'); setMariaHomeworkStatus('No generation was attempted.'); setPendingMariaHomework(null); setMariaHomeworkStage(''); return; }
    if (chapterLoading) return;
    const requested=(pendingMariaHomework.chapters||[]).map(String).map(value=>value.trim()).filter(Boolean);
    if (!requested.length) { setChapters([]); setMariaHomeworkStage('generate'); setMariaHomeworkStatus('Maria will use the whole authorised Textbook in the existing Homework generator.'); return; }
    const matched:string[]=[];
    for (const requestedChapter of requested) {
      const key=normalizeMariaAcademicText(requestedChapter); const exact=chapterOptions.filter(option=>normalizeMariaAcademicText(option)===key); const loose=exact.length?exact:chapterOptions.filter(option=>normalizeMariaAcademicText(option).includes(key)||key.includes(normalizeMariaAcademicText(option)));
      const unique:string[]=[...new Set<string>(loose.map(String))]; if(unique.length!==1){setError(unique.length?`Maria found multiple chapter matches for “${requestedChapter}”. Please choose the exact chapter.`:`Maria could not find “${requestedChapter}” in the indexed Textbook chapters. Classtago did not fall back to the whole book.`);setMariaHomeworkStatus('Homework generation stopped safely before AI generation.');setPendingMariaHomework(null);setMariaHomeworkStage('');return;} matched.push(unique[0]);
    }
    setChapters([...new Set(matched)]); setMariaHomeworkStage('generate'); setMariaHomeworkStatus(`Maria matched ${matched.length} requested chapter${matched.length===1?'':'s'} to the existing Textbook index.`);
  }, [pendingMariaHomework, mariaHomeworkStage, assignment?.id, scopedTextbooks.length, readyTextbooks.length, preparingTextbooks.length, chapterLoading, chapterOptions.join('|'), generateBlocker]);
  ...
  const generate = async () => {
    ...
    try {
      ...
      if (!generatedContent) throw new Error('The Homework AI service returned no usable text. Classtago kept your selections ready; restart Preview once if this build was just uploaded.');
  ...
  useEffect(() => {
    if (!pendingMariaHomework || mariaHomeworkStage !== 'generate' || busy) return;
    if (generateBlocker) { setMariaHomeworkStatus(`Maria is waiting on the existing Homework rule: ${generateBlocker}`); return; }
    setMariaHomeworkStage('running'); setMariaHomeworkStatus('Maria started the existing Homework generator. It will stop at Preview; nothing is saved or published automatically.');
    void generate().finally(() => { setPendingMariaHomework(null); setMariaHomeworkStage(''); });
  }, [pendingMariaHomework, mariaHomeworkStage, generateBlocker, busy, chapters.join('|'), assignment?.id, selectedMaterials.join('|'), selectedTypes.map(x=>`${x.key}:${x.count}`).join('|'), languageCode, difficulty, targetMinutes, teacherInstruction]);
  ...
  const openWhatsApp = async () => {
    ...
    const url=`https://wa.me/?text=${encodeURIComponent(shareText)}`;const opened=window.open(url,'_blank');if(opened){try{opened.opener=null;}catch{}}else window.location.assign(url);setWhatsAppStatus(shareable.length?'This browser could not hand actual files to WhatsApp, so Classtago opened the safe link fallback.':'WhatsApp opened with the Homework pre-filled. Select your Broadcast List / contacts and tap Send.');
  };
  ...
  return <section id="homework-builder-top" className="workspace">
    ...
    <div className="card">
      ...
      {mode==='ai' && <>
        <div className="form-grid two">
          <label className="field"><span>Chapter / Unit</span><select value={chapters[0] || ''} onChange={e=>setChapters(e.target.value?[e.target.value]:[])} disabled={!readyTextbooks.length || chapterLoading}><option value="">{!assignment ? 'Select Class / Subject first' : !scopedTextbooks.length ? 'Upload textbook first' : !readyTextbooks.length && failedTextbooks.length ? 'Chapter preparation needs attention' : !readyTextbooks.length ? 'Preparing chapter list…' : chapterLoading ? 'Reading textbook chapters…' : 'Whole textbook'}</option>{chapterOptions.map(ch=><option key={ch} value={ch}>{ch}</option>)}</select>{!readyTextbooks.length && failedTextbooks.length > 0 && <small className="text-amber-700">The textbook is already uploaded. Complete AI setup or retry chapter preparation from Study Material.</small>}{!readyTextbooks.length && !failedTextbooks.length && preparingTextbooks.length > 0 && <small className="text-cyan-700">Classtago is preparing the chapter list from your uploaded textbook.</small>}{readyTextbooks.length>0 && !chapterLoading && !chapterOptions.length && <small className="text-amber-700">No chapter headings were detected. Homework will use the whole Textbook.</small>}</label>
      ...
      {mariaHomeworkStatus && <div className="mt-3 rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs font-semibold text-violet-900"><b>Maria:</b> {mariaHomeworkStatus} <span className="opacity-75">Existing Homework Save/Publish controls remain manual.</span></div>}
