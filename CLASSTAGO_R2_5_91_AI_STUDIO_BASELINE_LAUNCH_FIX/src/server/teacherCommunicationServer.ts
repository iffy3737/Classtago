...
export async function loadTeacherCommunicationScopes(activeUser: TeacherCommunicationActiveUser) {
  ...
  const channelReadiness={
    ...
    sms:false, // R2.5.65: SIM SMS is reserved for OTP/security/critical emergency flows.
...
export async function sendTeacherCommunicationServer(activeUser:TeacherCommunicationActiveUser,input:any){
  ...
  const audience=['students','parents','students_and_parents'].includes(String(input.audience))?String(input.audience):'students_and_parents'; const requestedChannels:string[]=[...new Set<string>((Array.isArray(input.channels)?input.channels:[]).map((v:any)=>String(v)))]; if(requestedChannels.includes('sms')) throw new Error('SMS is reserved for OTP, account security and critical/emergency school workflows. Use Website / App, WhatsApp or Email for routine Teacher communication.'); const channels:string[]=requestedChannels.filter((c:string)=>['website','whatsapp','email'].includes(c)); if(!channels.length) throw new Error('Select at least one delivery channel.');
  ...
  const created=await supabaseAdmin.from('edunixo_communication_messages').insert(payload).select('id').single(); if(created.error){if(missingTable(created.error)) throw new Error('Classtago Communication cloud setup is pending. Apply the current Classtago Supabase migration bundle, then reload.');throw created.error;} const messageId=String(created.data.id);
