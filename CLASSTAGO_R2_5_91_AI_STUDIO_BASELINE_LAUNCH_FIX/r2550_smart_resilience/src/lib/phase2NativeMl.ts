...
export async function recognizeAdmissionDocument(dataUrl: string, interfaceLanguage = 'en') {
  if (!Capacitor.isNativePlatform()) throw new Error('On-device OCR is available in the EDUNIXO Android app.');
...
export async function translateOnDevice(text: string, sourceLanguage: string, targetLanguage: string) {
  if (!Capacitor.isNativePlatform()) throw new Error('On-device translation is available in the EDUNIXO Android app.');
...
export async function transcribeOnDevice(languageTag = 'en-IN', preferOffline = true) {
  if (!Capacitor.isNativePlatform()) throw new Error('Voice transcription is available in the EDUNIXO Android app.');
