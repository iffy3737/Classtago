...
async function academicApiJson<T>(url: string, init?: RequestInit): Promise<T> {
  ...
  if (!contentType.includes('application/json')) {
    ...
    if (response.ok) {
      throw new Error('The updated EDUNIXO upload service is not active in this Preview yet. Restart Preview once, then try Upload Textbook again.');
    }
    throw new Error(`Teacher Academic request failed (${response.status}).`);
  }
  const payload = await response.json().catch(() => null);
  if (!payload || typeof payload !== 'object') {
    throw new Error('The EDUNIXO upload service returned an invalid response. Restart Preview once and try again.');
  }
...
async function waitForHomeworkTextbookSource(materialIds: string[], maxWaitMs = 210000): Promise<void> {
  ...
  while (Date.now() - started < maxWaitMs) {
    ...
    if (String(status?.featureBuild || '') !== 'R33.6') throw new Error('EDUNIXO Preview server and Homework page are on different builds. Restart Preview once after importing R33.6.');
    if (status?.ready) return;
    if (status?.status === 'failed' || status?.error) throw new Error(String(status?.error || 'Textbook AI preparation failed.'));
    await academicClientWait(1500);
  }
...
export async function generateAcademicContent(request: GenerationRequest): Promise<GenerationResult> {
  ...
  // deployments simple because the same server-side Gemini secret used by EDUNIXO
  ...
  if (request.taskType === 'homework') {
    ...
    if (String(aiStatus?.featureBuild || '') !== 'R33.6') {
      throw new Error('EDUNIXO Preview server and Homework page are on different builds. Restart Preview once after importing R33.6, then Generate Homework again.');
    }
    ...
    for (let attempt = 1; attempt <= 2; attempt += 1) {
      try {
        ...
        if (response.ok) {
          ...
          if (!content) throw new Error('The R33.6 server completed without Homework text. EDUNIXO did not save a blank result; this response was rejected safely.');
...
export async function saveQuestionPaper(paper: QuestionPaperDraft): Promise<string> {
  ...
  // legacy EDUNIXO projection. The server resolves that bridge safely so Clerk
...
export async function generateQuestionPaperDraft(request: GenerationRequest & { taskType: 'question-paper' }): Promise<QuestionPaperDraft> {
  ...
  const aiStatus = await getTeacherAcademicAiStatus();
  if (!aiStatus?.ready) throw new Error('School Gemini Free Tier service is not ready in this Preview.');
  if (request.materialIds.length) await waitForHomeworkTextbookSource(request.materialIds);
  const token = await academicAuthToken();
...
export async function regenerateQuestion(input: {
  ...
  if (input.paper.materialIds.length) await waitForHomeworkTextbookSource(input.paper.materialIds);
  const token=await academicAuthToken();
  let response:Response|null=null,payload:any={};
  for(let attempt=1;attempt<=2;attempt+=1){
...
export async function listAcademicRecords(kind: SavedAcademicRecord['kind']): Promise<import('../types/domain').AcademicRecordListItem[]> {
  if (kind === 'homework') {
    const db = requireSupabase();
    const { data, error } = await db.from('edunixo_academic_records').select('*').eq('kind', kind).order('updated_at', { ascending: false }).limit(50);
    if (error) throw error;
    return (data ?? []).map((row: any) => ({
      id: row.id, kind: row.kind, assignmentId: row.assignment_id, title: row.title, status: row.status, content: row.content,
...
export async function uploadAcademicAttachment(recordId: string, file: File): Promise<void> {
  ...
  const path = `${userData.user.id}/${recordId}/${crypto.randomUUID()}-${file.name}`;
  const { error: uploadError } = await db.storage.from('teacher-academic-attachments').upload(path, file, { upsert: false });
  if (uploadError) throw uploadError;
  const { error } = await db.from('edunixo_academic_record_attachments').insert({
    academic_record_id: recordId,
    owner_teacher_id: userData.user.id,
    file_name: file.name,
    storage_path: path,
...
export async function listAssignmentSubmissions(recordId: string): Promise<import('../types/domain').AssignmentSubmission[]> {
  const db = requireSupabase();
  const { data, error } = await db.from('edunixo_assignment_submissions').select('*').eq('academic_record_id', recordId).order('student_name');
  if (error) throw error;
  return (data ?? []).map((row: any) => ({
    id: row.id,
...
export async function reviewAssignmentSubmission(submissionId: string, review: { remarks?: string; marks?: number; grade?: string; checked: boolean }): Promise<void> {
  const db = requireSupabase();
  const { error } = await db.from('edunixo_assignment_submissions').update({
    remarks: review.remarks ?? null,
    marks: review.marks ?? null,
    grade: review.grade ?? null,
    status: review.checked ? 'checked' : 'submitted',
    checked_at: review.checked ? new Date().toISOString() : null,
  }).eq('id', submissionId);
  if (error) throw error;
}
