...
type StaffRole = 'teacher' | 'class_teacher' | 'clerk';
...
const STAFF_ROLES: StaffRole[] = ['teacher', 'class_teacher', 'clerk'];
...
export default function StaffAccountManager({ activeFeatureId = null }: StaffAccountManagerProps) {
  ...
  const callProvision = async (staff: User, password: string) => {
    ...
    const response = await fetch('/api/admin/provision-staff', {
      ...
      body: JSON.stringify({
        ...
        phone: staff.phone,
        shalarthId: staff.role === 'clerk' ? undefined : (staff.shalarthId || username),
        employeeCode: staff.role === 'clerk' ? (staff.employeeCode || username) : staff.employeeCode
      })
    });
  ...
  const applyStaffMasterSelection = (masterId: string) => {
    ...
    const inferredRole: StaffRole = /clerk|office|administrative/i.test(designation) ? 'clerk' : 'teacher';
  ...
  const showAccountList = activeFeatureId !== 'staff-account-create';
                    ...
                    <td data-label="Actions" className="p-4">
                      <div className="erp-row-actions flex flex-wrap justify-end gap-2">
                        {busy ? <Loader2 className="w-4 h-4 animate-spin text-indigo-600" /> : !cloud ? (
                          <>
                            {(showAllActions || activeFeatureId === 'staff-account-edit') && (
                            ...
                            {(showAllActions || activeFeatureId === 'staff-account-pending' || activeFeatureId === 'link-staff-login') && (
