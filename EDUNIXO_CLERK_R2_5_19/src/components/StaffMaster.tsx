...
export default function StaffMaster({
  ...
  const exportExcel = () => {
    ...
    const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'StaffMaster'); XLSX.writeFile(wb, 'EDUNIXO_Staff_Master.xlsx');
  };
  ...
  return <div className="space-y-5 text-left">
    <section className="edx-staff-master-hero relative overflow-hidden rounded-[1.75rem] border border-white/10 bg-[radial-gradient(circle_at_10%_15%,rgba(34,211,238,.19),transparent_32%),radial-gradient(circle_at_92%_4%,rgba(139,92,246,.24),transparent_34%),linear-gradient(135deg,#07182f,#080b1d_58%,#17122f)] p-5 text-white shadow-[0_28px_80px_rgba(2,6,23,.28)] sm:p-7 no-print">
      <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
        ...
        <button type="button" onClick={() => void loadRecords()} disabled={loading} className="inline-flex items-center justify-center gap-2 rounded-xl border border-white/15 bg-white/10 px-4 py-3 text-xs font-black hover:bg-white/15 disabled:opacity-50"><RefreshCw className={`h-4 w-4 ${loading?'animate-spin':''}`}/>Refresh Cloud</button>
      </div>
...
function Metric({icon:Icon,label,value}:{icon:any;label:string;value:number}) { return <div className="edx-staff-master-metric rounded-2xl border border-white/10 bg-white/7 p-4"><Icon className="h-4 w-4 text-cyan-200"/><div className="mt-3 text-2xl font-black">{value}</div><div className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-300">{label}</div></div>; }
