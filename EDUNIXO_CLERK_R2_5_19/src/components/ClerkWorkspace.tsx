...
export default function ClerkWorkspace({ lang, user, classes, onRefreshData, initialTab = 'dashboard', hideAdmissionNavigation = false, focusedMode = false, focusedTitle, activeFeatureId = null, onOpenAdmissionDesk, onBackToOverview }: ClerkWorkspaceProps) {
  ...
  // Helper generators for GR & Admission sequential numbers
  const getNextGrNumber = (currentList: AdmittedStudent[]) => {
    const existing = currentList.map(a => {
      const match = a.grNumber.match(/\d+/);
      return match ? parseInt(match[0], 10) : 0;
    });
    const maxNum = existing.length > 0 ? Math.max(...existing) : 100;
    return `GR2026${maxNum + 1}`;
  };

  const getNextAdmNumber = (currentList: AdmittedStudent[]) => {
    const existing = currentList.map(a => {
      const match = a.admissionNumber ? a.admissionNumber.match(/\d+/) : null;
      return match ? parseInt(match[0], 10) : 0;
    });
    const maxNum = existing.length > 0 ? Math.max(...existing) : 500;
    return `ADM2026${maxNum + 1}`;
  };
  ...
  useEffect(() => {
    ...
    setWizardData((prev: any) => ({
      ...prev,
      admissionNumber: prev.admissionNumber || getNextAdmNumber(loadedAdmissions),
      grNumber: prev.grNumber || getNextGrNumber(loadedAdmissions)
    }));
  ...
  const handleExecuteAdmission = async (e?: React.FormEvent) => {
    ...
    const nextAdm = getNextAdmNumber(proposalHistory);
    const nextGr = getNextGrNumber(proposalHistory);
