...
type HistoryRow = { id: string; studentId: string; action: string; summary: string; createdAt: string; beforeStatus?: string | null; afterStatus?: string | null; reason?: string | null };
          ...
          {section === 'history' && (selected ? <div className="space-y-5">
            {isClerk ? (
              ...
              <form onSubmit={async event => {
                ...
                const consequential = ['Transferred', 'Passout', 'Leaving', 'Archived'].includes(statusForm.status);
                if (consequential && !statusNote.trim()) {
                  setMessage({ type: 'error', text: 'Add an office reason before a transfer, pass-out, leaving or archive status is finalized.' });
                  return;
                }
                if (await requestActionConfirm({
                  title: 'Change student status?',
                  message: `Change ${selected.fullName}'s status to ${statusForm.status}? This is a Headmaster-only audited lifecycle action and never deletes the Student Master record.`,
                  confirmLabel: 'Change Status',
                  tone: 'warning'
                })) void saveSection('history', { ...statusForm, reason: statusNote.trim() || null });
              }} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
                <h2 className="text-xl font-black text-slate-950">Student Status & History</h2><p className="mt-2 text-xs text-slate-500">Final status changes are Headmaster-only, audited and never silently delete the Student Master record. The cloud Student Master keeps its active flag; exact lifecycle state and reason are preserved in the audit trail.</p>
                ...
                <div className="mt-5"><TextField label="Office reason / lifecycle note"><textarea rows={4} value={statusNote} onChange={e => setStatusNote(e.target.value)} placeholder="Required for transfer, pass-out, leaving and archive..." className="w-full rounded-xl border border-slate-200 px-4 py-3 text-sm font-semibold" /></TextField></div>
