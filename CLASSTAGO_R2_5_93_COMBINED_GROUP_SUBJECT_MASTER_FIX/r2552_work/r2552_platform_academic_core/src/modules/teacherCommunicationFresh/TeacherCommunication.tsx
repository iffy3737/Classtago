...
import { Bell, BookOpenCheck, CheckCircle2, Copy, Languages, Mail, MessageCircleMore, MessageSquareText, RefreshCw, Send, Share2, Smartphone, Sparkles, UsersRound } from 'lucide-react';
...
function FreeWhatsAppPanel({
  ...
  return <div className={`rounded-xl border border-emerald-200 bg-emerald-50 ${compact ? 'p-3' : 'p-4'}`}>
    ...
    <p className="mt-2 text-[9px] font-semibold leading-4 text-emerald-800/80">Free mode does not mark WhatsApp as Delivered inside EDUNIXO because the final Send happens in WhatsApp. True automatic bulk WhatsApp remains an optional school-provider mode.</p>
  </div>;
}
...
export default function TeacherCommunication({ context, loading, error, view, onNavigate }: Props) {
  ...
  const commonControls = <>
    ...
    <div className="rounded-xl bg-slate-50 p-3 text-[11px] leading-5 text-slate-600"><b>Channel readiness:</b> Website/App ready · My WhatsApp Free {senderIdentity?.mobileLinked ? 'ready' : 'needs Teacher mobile'} · {availableExternal.length ? `${availableExternal.join(', ')} Auto configured` : 'WhatsApp Auto / SMS / Email provider not configured'}.</div>
  </>;
  ...
  return <div className="space-y-5">
    ...
    {view !== 'school_notices' && <div className="flex items-start gap-2 text-[11px] font-semibold leading-5 text-slate-500"><CheckCircle2 className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600"/><span>Communication scope and recipients are revalidated on the server before sending. Teacher mobile/email are auto-linked from profile. Website/App is native EDUNIXO; My WhatsApp Free opens the Teacher's device WhatsApp without a provider; WhatsApp Auto/SMS/Email become selectable only when the school provider is configured server-side.</span></div>}
  </div>;
}
