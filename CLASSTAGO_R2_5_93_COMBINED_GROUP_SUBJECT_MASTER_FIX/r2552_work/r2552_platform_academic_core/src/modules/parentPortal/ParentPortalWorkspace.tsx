...
export default function ParentPortalWorkspace({ user, module, feature, onNavigate }: Props) {
  ...
  const submitService = async (type: 'certificate'|'library'|'profile_correction'|'support') => {
    ...
    try { const out = await submitParentServiceRequest(studentId, { type, title: requestTitle, details: requestDetails }); setRequests(prev => [out.request, ...prev]); setRequestTitle(''); setRequestDetails(''); setMessage('Request submitted to the school office.'); }
  ...
  return <section className="edx-parent-portal edx-theme-safe overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-50 shadow-xl">{header}<div className="p-5 sm:p-7">{error&&<div className="mb-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-xs font-bold text-rose-800">{error}</div>}{message&&module.id!=='pa-leave'&&module.id!=='pa-certificate-requests'&&module.id!=='pa-profile'&&<div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 text-xs font-bold">{message}</div>}{content}<div className="mt-7 flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-[11px] leading-6 text-slate-500"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600"/><span>Privacy boundary: Parent access is derived only from Headmaster-approved, school-scoped Parent-child link records. Marks, attendance, Result Book, Teacher drafts, other students and administrative edit/approval controls remain inaccessible.</span><button onClick={()=>void refresh()} className="ml-auto shrink-0 rounded-lg border border-slate-200 p-2 text-slate-600" title="Refresh"><RefreshCw className="h-4 w-4"/></button></div></div></section>;
}
