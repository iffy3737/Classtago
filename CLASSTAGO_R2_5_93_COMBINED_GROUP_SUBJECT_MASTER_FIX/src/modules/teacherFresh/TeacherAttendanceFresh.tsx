...
import { subscribeMariaDynamicCommands, takePendingMariaDynamicCommands, type MariaAttendanceCommand } from '../../lib/mariaClientBridge';
...
function DailyAttendance({ context }: { context: TeacherCloudContext }) {
  ...
  const [pendingMaria,setPendingMaria]=useState<MariaAttendanceCommand|null>(null);
  ...
  useEffect(() => {
    const onSync=(event:any)=>{ if(event?.detail?.kind==='attendance_save' && event?.detail?.recordKey===cacheKey && event?.detail?.ok) { setMessage('Offline attendance synced safely to Classtago cloud.'); if(navigator.onLine) void load(); } };
  ...
  useEffect(()=>{
    const accept=(command:any)=>{if(command?.type!=='maria_attendance')return;setPendingMaria(command as MariaAttendanceCommand);setMariaStatus(command.statusMessage||'Maria is preparing the existing Daily Attendance sheet. Save remains manual.');};
    const unsubscribe=subscribeMariaDynamicCommands(accept);
    for(const command of takePendingMariaDynamicCommands('maria_attendance'))accept(command);
    return unsubscribe;
  },[]);
  ...
  useEffect(()=>{
    if(!pendingMaria)return;
    const norm=(value:unknown)=>String(value||'').normalize('NFKC').toLowerCase().replace(/[^a-z0-9\u0900-\u097f\u0600-\u06ff]+/g,' ').trim();
    const requestedClass=norm(pendingMaria.className), requestedDivision=norm(pendingMaria.division);
    const candidates=scopes.filter(item=>(!requestedClass||norm(item.className)===requestedClass)&&(!requestedDivision||norm(item.division)===requestedDivision));
    if((requestedClass||requestedDivision)&&candidates.length!==1){setError(candidates.length?'Maria found more than one Class Teacher attendance scope. Please specify Class/Division.':'Maria could not find that Class/Division in your Class Teacher assignment.');setMariaStatus('Maria stopped safely before changing attendance.');setPendingMaria(null);return;}
    const target=candidates[0]||scope;
    if(target&&target.id!==scopeId){setScopeId(target.id);setMariaStatus(`Maria selected ${target.className} · ${target.division}. Loading the existing attendance sheet.`);return;}
    if(!target){setError('No Class Teacher attendance scope is available.');setPendingMaria(null);return;}
    if(pendingMaria.command==='open'){setMariaStatus('Maria opened the existing Daily Attendance sheet. No attendance was changed.');setPendingMaria(null);return;}
    if(busy||!roster.length)return;
    if(!directEditAllowed){setError('Maria cannot edit this sheet because the existing attendance rules currently lock it.');setMariaStatus('Existing holiday/date/lock rules remain authoritative.');setPendingMaria(null);return;}
    const activeRows=roster.filter(student=>isActiveOn(student,date));
    const resolveStudent=(name?:string,roll?:string)=>{
      const r=norm(roll); if(r){const m=activeRows.filter(x=>norm(x.rollNumber)===r||norm(x.grNumber)===r);if(m.length===1)return m[0];}
      const n=norm(name);if(n){let m=activeRows.filter(x=>norm(x.fullName)===n);if(m.length===1)return m[0];m=activeRows.filter(x=>norm(x.fullName).includes(n)||n.includes(norm(x.fullName)));if(m.length===1)return m[0];}
      return null;
    };
    if(pendingMaria.command==='mark_all_present'){
      setValues(v=>({...v,...Object.fromEntries(activeRows.map(student=>[student.id,'P']))}));
      setMariaStatus(`Maria filled Present for ${activeRows.length} active Student(s). Review the sheet; Save is still manual.`);
    }else if(pendingMaria.command==='clear'){
      setValues({});setMariaStatus('Maria cleared the on-screen draft only. Nothing was saved.');
    }else if(pendingMaria.command==='mark_absent'||pendingMaria.command==='mark_present'){
      const names=pendingMaria.studentNames||[], rolls=pendingMaria.rollNumbers||[];const chosen=new Map<string,AttendanceStudent>();
      for(let i=0;i<Math.max(names.length,rolls.length);i++){const student=resolveStudent(names[i],rolls[i]);if(student)chosen.set(student.id,student);}
      if(!chosen.size){setError('Maria could not uniquely match the requested Student(s) in this roster.');setMariaStatus('No attendance was changed.');setPendingMaria(null);return;}
      const status=pendingMaria.command==='mark_absent'?'A':'P';setValues(v=>({...v,...Object.fromEntries([...chosen.keys()].map(id=>[id,status]))}));setMariaStatus(`Maria filled ${status==='A'?'Absent':'Present'} for ${chosen.size} Student(s). Review before Save.`);
    }else if(pendingMaria.command==='enter_sequence'){
      const statuses=(pendingMaria.statuses||[]).filter(x=>x==='P'||x==='A');if(!statuses.length){setError('Maria did not receive a valid P/A sequence.');setPendingMaria(null);return;}
      const count=Math.min(statuses.length,activeRows.length);setValues(v=>({...v,...Object.fromEntries(activeRows.slice(0,count).map((student,i)=>[student.id,statuses[i]]))}));setMariaStatus(`Maria filled ${count} attendance value(s) top-to-bottom. Review the existing sheet; Save remains manual.`);
    }
    setPendingMaria(null);
  },[pendingMaria,scopes,scopeId,scope,busy,roster,date,directEditAllowed]);
  ...
  const save = async()=>{
    ...
    try{
      ...
      if (!online) {
        ...
        setMessage('Attendance saved to Classtago cloud. Historical edits require Correction Request.');
      }
  ...
  return <div className="space-y-5" data-testid="daily-attendance-complete">
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      {otherEditors>0&&<div className="mb-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-800">Another Classtago session is currently editing this same attendance sheet. You can continue, but review before saving; server conflict protection remains active.</div>}
      {mariaStatus&&<div className="mb-3 rounded-xl border border-violet-200 bg-violet-50 p-3 text-xs font-bold text-violet-900"><b>Maria:</b> {mariaStatus}</div>}
