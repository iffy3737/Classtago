...
const featureTitle: Record<string, string> = {
  ...
  'sms-gateway': 'Inbuilt SMS Gateway',
};
...
export default function HeadmasterCommunicationCloudWorkspace({ user, activeFeatureId = 'school-notices' }: Props) {
  ...
  if (feature === 'sms-gateway') return <SmsGatewayControlPanel user={user} />;
  ...
  const useTemplate = (id: string) => {
    ...
    if (['whatsapp', 'email'].includes(mapped)) setChannels([mapped]);
    if (mapped === 'sms' || mapped === 'notice' || mapped === 'in-app') setChannels(['website']);
  };
  ...
  const sendMessage = async () => {
    ...
    if (channels.includes('sms') && priority !== 'urgent') {
      setError('SMS is reserved for critical/emergency communication. Set Priority to Urgent or use Website / In-App, WhatsApp or Email.');
      return;
    }
  ...
  const Composer = ({ allowSchedule = true }: { allowSchedule?: boolean }) => <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2">
      ...
      <div><div className="text-[10px] font-black uppercase tracking-wide text-slate-500">Channels</div><div className="mt-2 flex flex-wrap gap-2">{channelOptions.map(([key, label]) => { const ready = data.channelReadiness?.[key] !== false; const policyReady = key !== 'sms' || priority === 'urgent'; return <button type="button" key={key} disabled={!ready || !policyReady} onClick={() => toggle(key, channels, setChannels)} className={`rounded-xl border px-3 py-2 text-xs font-black disabled:cursor-not-allowed disabled:opacity-40 ${channels.includes(key) ? 'border-violet-300 bg-violet-50 text-violet-800' : 'border-slate-200 text-slate-500'}`}>{label}{!ready ? ' · Not configured' : key === 'sms' && !policyReady ? ' · Urgent only' : ''}</button>; })}</div><p className="mt-2 text-[10px] leading-relaxed text-amber-700">SIM SMS is reserved for OTP, security and genuinely critical/emergency messages. Routine notices should use Website / In-App, WhatsApp or Email.</p></div>
    </div>
