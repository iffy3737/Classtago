...
type CommunicationTab = 'dashboard' | 'notices' | 'in_app' | 'announcements' | 'templates' | 'scheduled' | 'history' | 'smtp_settings';
...
export default function SmartCommunicationHub({ lang, user, onRefreshData, activeFeatureId = null, focusedMode = false, focusedTitle = 'Communication Hub' }: SmartCommunicationHubProps) {
  ...
  const [templates, setTemplates] = useState<MessageTemplate[]>(() => {
    ...
    return [
      {
        id: 't_admission',
        name: 'Admission Approval Greeting',
        category: 'Admission',
        subject: 'Welcome to National High School, Taloda',
        body: 'Dear Parent, Congratulations! The admission of your child {student_name} to Class {class_name} has been approved. Your GR Number is {gr_number}. Use the student registration flow with the verified GR number; credentials are never sent in notices.',
        bodyUr: 'محترم والدین، مبارک ہو! آپ کے بچے {student_name} کا کلاس {class_name} میں داخلہ منظور کر لیا گیا ہے۔ آپ کا جی آر نمبر {gr_number} ہے۔',
        bodyHi: 'प्रिय अभिभावक, बधाई हो! आपके बच्चे {student_name} का कक्षा {class_name} में प्रवेश स्वीकृत हो गया है। आपका जीआर नंबर {gr_number} है।'
      },
      {
        id: 't_fee',
        name: 'Pending Fee Alert',
        category: 'Fee Reminder',
        subject: 'URGENT: Outstanding School Fee Reminder',
        body: 'Dear Parents, this is a friendly reminder that school fees installment of ₹{amount} is due for your ward {student_name}. Kindly settle before {due_date} and keep the fee ledger up to date. Principal, NHS Taloda.',
        bodyUr: 'محترم والدین، یہ ایک دوستانہ یاد دہانی ہے کہ آپ کے بچے {student_name} کے لیے اسکول کی فیس کی قسط {amount} روپے واجب الادا ہے۔',
        bodyHi: 'प्रिय अभिभावक, यह एक अनुस्मारक है कि आपके बच्चे {student_name} की स्कूल फीस की किस्त ₹{amount} देय है।'
      },
      {
        id: 't_attendance',
        name: 'Absent Student Alert',
        category: 'Attendance Alert',
        subject: 'Daily Student Absence Notice',
        body: 'Dear Parent, your ward {student_name} was found ABSENT today {date} without prior leave approval. Please submit a leave application or contact class teacher {teacher_name}.',
        bodyUr: 'محترم والدین، آپ کا بچہ {student_name} آج {date} کو بغیر پیشگی رخصت کے غیر حاضر پایا گیا ہے۔',
        bodyHi: 'प्रिय अभिभावक, आपका बच्चा {student_name} आज {date} को बिना पूर्व सूचना के अनुपस्थित पाया गया है।'
      },
      {
        id: 't_exam',
        name: 'Exam Schedule Out',
        category: 'Exam Reminder',
        subject: 'Academic Examination Schedule Published',
        body: 'Hello Students & Parents, the date sheet for {exam_name} is now declared. Exam begins on {date}. View date sheets and download hall tickets on the portal.',
        bodyUr: 'ہیلو طلباء اور والدین، {exam_name} کا امتحانی شیڈول اب جاری کر دیا گیا ہے۔',
        bodyHi: 'प्रिय छात्र और अभिभावक, {exam_name} का परीक्षा कार्यक्रम घोषित कर दिया गया है।'
      }
    ];
  });
  ...
  const handleSendBulkAnnouncement = (e: React.FormEvent) => {
    e.preventDefault();
    if (!bulkMessageText) {
    ...
    if (isScheduled) {
      ...
      // Execute instantly (Free WhatsApp manually formats, Free SMTP mock, SMS optional mock)
      ...
      if (bulkChannel === 'WhatsApp') {
        ...
        if (bulkRecipientType === 'individual') {
          ...
          if (ind?.phone) contactPhone = ind.phone;
        }
      ...
      {activeTab === 'notices' && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 animate-fade-in text-left">
          ...
          <div className={`${canDraftNotice ? 'lg:col-span-2' : 'lg:col-span-3'} space-y-6`}>
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              <div className="border-b border-slate-200 bg-slate-50/50 px-5 py-4">
                <h3 className="font-bold text-slate-900 text-sm font-sans">NHS Digital Board Circulars</h3>
              </div>
              ...
              <div>
                ...
                <div className="flex flex-col gap-1.5">
                  {[
                    ...
                    { id: 'SMS', label: 'SMS Gateway Broadcast (OPTIONAL)', desc: 'Requires integration gateway' },
                  ].map(ch => (
