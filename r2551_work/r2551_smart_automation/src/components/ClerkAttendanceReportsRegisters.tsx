...
export default function ClerkAttendanceReportsRegisters({ lang, user, activeFeatureId = 'cl-attendance-overview' }: Props) {
  ...
  const setup = useMemo(() => ({
    ...
    schoolProfile: { schoolName: cloud?.school?.schoolName || cloud?.school?.school_name || 'National High School, Taloda', address: cloud?.school?.address || 'Taloda, Dist. Nandurbar', udiseCode: cloud?.school?.udiseCode || cloud?.school?.udise_code || '' },
  ...
  const catalogueClassNames = useMemo(() => Array.from(new Set(classes.map(c => c.className))).sort((a,b)=>a.localeCompare(b, undefined, { numeric: true })), [classes]);
  ...
  const schoolHeader = (subtitle: string) => <div className="border-b-2 border-slate-900 pb-3 text-center">
    <div className="text-lg font-black uppercase tracking-wide">{setup.schoolProfile.schoolName || 'National High School, Taloda'}</div>
    <div className="mt-1 text-[9px] font-semibold text-slate-500">{setup.schoolProfile.address || 'Taloda, Dist. Nandurbar'} · U-DISE: {setup.schoolProfile.udiseCode || '—'}</div>
