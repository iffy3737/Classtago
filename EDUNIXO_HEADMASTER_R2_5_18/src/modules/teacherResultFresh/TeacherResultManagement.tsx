...
function SubjectMarksPage({context,onNavigate}:{context:TeacherCloudContext;onNavigate?:Props['onNavigate']}) {
  ...
  const reload=async()=>{
    ...
    }catch(e:any){setErr(e?.message||'Unable to load Subject Marks List.');}
  ...
  return <div className="space-y-5">
    ...
    {template&&<>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white p-4"><p className="text-[10px] text-slate-500">Allowed special codes: A, AB, ML, EX, WH, NA. Calculated / other-term / other-teacher cells stay locked.</p><div className="flex gap-2"><button onClick={()=>void reload()} className="inline-flex items-center gap-1 rounded-xl border border-slate-300 px-3 py-2 text-xs font-bold"><RefreshCw className="h-3.5 w-3.5"/>Refresh</button><button disabled={busy||locked||!backendReady} onClick={()=>void save(false)} className="rounded-xl border border-slate-300 px-4 py-2 text-xs font-black disabled:opacity-40">Save Draft</button><button disabled={busy||locked||!backendReady||!roster.length} onClick={()=>void save(true)} className="inline-flex items-center gap-1 rounded-xl bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-40"><Send className="h-3.5 w-3.5"/>{list?.status==='returned'?'Resubmit to Class Teacher':'Send to Class Teacher'}</button></div></div>
    </>}
...
function ResultBookPage({context,onNavigate}:{context:TeacherCloudContext;onNavigate?:Props['onNavigate']}){
  ...
  const reload=async()=>{if(!scope)return;setBusy(true);try{const r=await buildOrLoadResultBook(context,scope,term);setBook(r.book);setStatuses(r.statuses);setComplete(r.complete);setErr('')}catch(e:any){setErr(e?.message||'Result Book could not be built.')}finally{setBusy(false)}};
  ...
  return <div className="space-y-5">
    ...
    <div className="flex justify-end"><button disabled={busy||!complete||book?.status==='sent_to_progress_card'} onClick={()=>void send()} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40"><FileCheck2 className="h-4 w-4"/>Send to Progress Card & Clerk</button></div>{msg&&<Banner tone="emerald">{msg}</Banner>}{err&&<Banner tone="rose">{err}</Banner>}
  </div>
}
...
function ProgressCardPage({context,onNavigate}:{context:TeacherCloudContext;onNavigate?:Props['onNavigate']}){
  ...
  const[err,setErr]=useState('');
  useEffect(()=>{if(scope)void loadProgressBatch(context,scope,term).then(setBatch).catch((e:any)=>setErr(e?.message||'Progress Card batch unavailable.'))},[scope?.id,term]);
