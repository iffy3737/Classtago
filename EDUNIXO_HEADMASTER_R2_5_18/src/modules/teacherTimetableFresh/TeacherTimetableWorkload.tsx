...
export default function TeacherTimetableWorkload({ context, loading, error, view }: { context: TeacherCloudContext | null; loading: boolean; error?: string; view: TeacherTimetableView }) {
  ...
  const reload = async () => {
    ...
    try { const next = await loadTeacherTimetableData(context); setData(next); if (!classScope && next.classTeacherScopes[0]) setClassScope(`${next.classTeacherScopes[0].className}|${next.classTeacherScopes[0].division}`); }
    catch (e:any) { setLoadError(e?.message || 'Unable to load timetable data.'); }
  ...
  return <div className="space-y-5">
    ...
    {view === 'my_timetable' && <><div className="grid gap-3 sm:grid-cols-3"><Stat icon={<CalendarDays/>} label="Weekly periods" value={String(workload.total)}/><Stat icon={<Users/>} label="Assigned scopes" value={String(workload.scopes.length)}/><Stat icon={<Clock3/>} label="Feed" value={data?.source === 'cloud' ? 'Published Cloud' : 'Smart Timetable'}/></div><WeeklyGrid rows={data?.regularRows || []}/></>}
