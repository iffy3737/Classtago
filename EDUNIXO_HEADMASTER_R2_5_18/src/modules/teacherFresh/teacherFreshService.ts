...
export async function loadTeacherCloudContext(identity: { userId: string; appUserId?: string; employeeCode?: string; shalarthId?: string; name?: string }): Promise<TeacherCloudContext> {
  ...
  return {
    ...
    schoolName: asText((school as any)?.school_name, 'EDUNIXO School'),
...
export async function saveDailyAttendance(input: {
  ...
  if (input.date !== today) throw new Error('Past attendance cannot be silently edited. Use Attendance Correction Request.');
...
export async function createAttendanceCorrectionRequest(input: {
  ...
  const { error } = await supabase.from('edunixo_attendance_correction_requests').insert({
    school_id: input.context.schoolId,
    academic_year_id: input.assignment.academicYearId,
    class_id: input.assignment.classId,
    division_id: input.assignment.divisionId || null,
    student_id: input.studentId,
    attendance_date: input.date,
    current_status: input.currentStatus,
    requested_status: input.requestedStatus,
    reason: input.reason.trim(),
    requested_by_user_id: input.context.userId,
    requested_by_teacher_id: input.context.teacherRecordId,
    status: 'pending',
  });
  if (error) {
    if (isMissingRelation(error)) throw new Error('Attendance Correction workflow backend is not installed yet. Apply the included Teacher R3 Supabase migration before using this workflow.');
    throw error;
  }
}

export async function listMyAttendanceCorrectionRequests(userId: string): Promise<AttendanceCorrectionRequest[]> {
  const { data, error } = await supabase.from('edunixo_attendance_correction_requests')
    .select('id, student_id, attendance_date, current_status, requested_status, reason, status, created_at')
    .eq('requested_by_user_id', userId).order('created_at', { ascending: false }).limit(100);
  if (error) {
    if (/does not exist|schema cache/i.test(String((error as any).message || ''))) return [];
    throw error;
  }
...
export async function loadTeacherDashboardCloudFeed(context: TeacherCloudContext, date: string): Promise<import('./types').TeacherDashboardCloudFeed> {
  ...
  const timetableSource = await firstReadableSchoolTable(
    ['edunixo_published_timetable_entries', 'school_timetable_entries', 'class_timetable', 'timetable_entries'],
    context.schoolId,
  );
  const timetableRows = timetableSource.rows.filter((row: any) => {
    const teacherMatch = [row.teacher_id, row.teacher_record_id].some(v => String(v || '') === context.teacherRecordId)
      || String(row.teacher_user_id || row.user_id || '') === context.userId
      || assignmentIds.has(String(row.subject_teacher_id || row.teacher_assignment_id || ''));
  ...
  const substituteSource = await firstReadableSchoolTable(['edunixo_substitute_duty_items'], context.schoolId);
  if (substituteSource.rows.length) {
    const substituteRows = substituteSource.rows.filter((row: any) => {
      ...
      return String(row.substitute_teacher_user_id || '') === context.userId
        || String(row.substitute_teacher_record_id || '') === context.teacherRecordId
        || String(row.substitute_teacher_name || '').trim().toLowerCase() === context.teacherName.trim().toLowerCase();
    });
