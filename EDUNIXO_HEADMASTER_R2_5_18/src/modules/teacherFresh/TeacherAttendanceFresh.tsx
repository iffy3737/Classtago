...
function DailyAttendance({ context }: { context: TeacherCloudContext }) {
  ...
  useEffect(() => { if (!scope) { setRoster([]); setValues({}); return; } let cancelled=false; (async()=>{ setBusy(true); setError(''); setMessage(''); try { const [r,c,e] = await Promise.all([loadAttendanceRoster(scope), getCalendarDay(context.schoolId,date), loadAttendanceForDate(scope,date)]); if(cancelled)return; setRoster(r); setCalendar(c); const next:Record<string,'P'|'A'>={}; (e as AttendanceEntry[]).forEach(x=>next[x.studentId]=x.status); setValues(next); } catch(err:any){ if(!cancelled)setError(err?.message||'Unable to load attendance.'); } finally { if(!cancelled)setBusy(false); } })(); return()=>{cancelled=true}; },[scopeId,date,context.schoolId]);
  ...
  const save = async()=>{ if(!scope)return; setBusy(true); setError(''); setMessage(''); try{ await saveDailyAttendance({context,assignment:scope,date,values,calendarDay:calendar}); setMessage('Attendance saved to EDUNIXO cloud. Historical edits now require Correction Request.'); }catch(e:any){setError(e?.message||'Save failed.');}finally{setBusy(false);} };
  ...
  return <div className="space-y-5" data-testid="daily-attendance-complete">
    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-[11px] text-slate-500">Daily Teacher marking is P/A only. Admission/leaving-date inactive rows are excluded automatically.</p><button disabled={busy||!scope||!directEditAllowed||unmarked>0||!active.length} onClick={save} className="inline-flex items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40"><Save className="h-4 w-4"/>Save Attendance</button></div>
