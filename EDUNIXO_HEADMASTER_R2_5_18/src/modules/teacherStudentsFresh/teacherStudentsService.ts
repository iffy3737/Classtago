...
export async function loadRecognitionRatings(
  ...
  if (response.error) {
    if (missingTable(response.error)) return { ratings: [], backendReady: false };
    throw response.error;
  }
  return { ratings: (response.data || []).map(ratingFromRow), backendReady: true };
}
...
export async function saveRecognitionRating(input: {
  ...
  const payload = {
    ...
    academic_year: input.assignment.academicYear || input.context.academicYear,
    class_id: input.assignment.classId,
    class_name: input.assignment.className,
    division_id: input.assignment.divisionId || null,
    division_name: input.assignment.division || 'No Division',
    subject_id: input.assignment.subjectId,
    subject_name: input.assignment.subjectName,
    teacher_user_id: input.context.userId,
    teacher_record_id: input.context.teacherRecordId,
    teacher_name: input.context.teacherName,
    student_id: input.student.id,
    student_name: input.student.fullName,
    gr_number: input.student.grNumber || null,
    ...
    total_points: scoreTotal(scores),
    updated_at: new Date().toISOString(),
  };
  const response = await supabase.from('edunixo_student_recognition_ratings').upsert(payload, {
    onConflict: 'school_id,academic_year_id,class_id,division_key,subject_id,teacher_user_id,student_id,period_type,period_key',
  }).select('*').single();
  if (response.error) {
    if (missingTable(response.error)) throw new Error('My Students Recognition cloud table is not installed yet. Run the R5 My Students Supabase setup to save stars.');
    throw response.error;
  }
  return ratingFromRow(response.data);
}
