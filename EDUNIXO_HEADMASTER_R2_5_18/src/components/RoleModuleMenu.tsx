...
export default function RoleModuleMenu({
      ...
      .map(category => {
        ...
        const modules = category.modules
          .map(module => {
            ...
            if (teacherRoadmapRole) return { ...module, features: [...module.features] };

            const features = module.features.filter(feature =>
              canAccess(feature.targetTab || module.tab)
            );
  ...
  }, [catalogue, canAccess, role, teacherRoadmapRole]);
  ...
  const brandInfoContent: Record<NonNullable<typeof brandInfo>, { title: string; body: string }> = {
    faq: {
      title: copy.faq,
      body: 'EDUNIXO keeps each school workspace isolated. Access, modules and data visibility follow the signed-in role, school membership and active platform permissions.'
    },
    about: {
      title: copy.about,
      body: 'EDUNIXO is a multi-school education operations platform designed to bring academic, administrative, communication and school-management workflows into one secure mobile-first workspace.'
    },
    terms: {
      title: copy.terms,
    ...
    privacy: {
      title: copy.privacy,
      body: 'EDUNIXO is designed around school-isolated access and role-based visibility. Personal and institutional information should only be processed for authorised school operations and approved platform services.'
    },
    grievance: {
      title: copy.grievance,
      body: 'For account, access, data or service concerns, use the authorised school or EDUNIXO support channel configured for your institution. Do not share passwords or secret keys in support messages.'
    },
    'privacy-center': {
        ...
        <div className="fixed inset-0 z-[2147483600] flex items-end justify-center bg-slate-950/55 p-3 backdrop-blur-sm sm:items-center" onClick={() => setBrandInfo(null)}>
          <section className="w-full max-w-md overflow-hidden rounded-[1.8rem] border border-white/70 bg-white shadow-[0_30px_100px_rgba(15,23,42,.3)]" onClick={event => event.stopPropagation()}>
            <div className="flex items-start justify-between gap-4 bg-gradient-to-br from-indigo-50 via-white to-cyan-50 p-5">
              <div>
                <p className="text-[9px] font-black uppercase tracking-[0.25em] text-indigo-600">EDUNIXO</p>
