/**
 * EDUNIXO premium mobile launcher.
...
export default function MobilePlatformGateway({ lang, onLoginSuccess, onSchoolSelected }: MobilePlatformGatewayProps) {
  ...
  useEffect(() => {
    if (mode !== 'discover' || normalizedQuery.length < 2) {
      setSchools([]);
      setError('');
      return;
    }
    ...
    <main className="edx-gateway-shell edx-app-launcher-shell">
      <section className="edx-gateway-hero edx-app-launcher-hero">
        ...
        <div className="edx-gateway-search-card edx-app-school-search">
          ...
          <small>Search only active schools connected to EDUNIXO.</small>
        </div>
      </section>
      ...
      <section className="edx-gateway-results edx-app-school-results" aria-live="polite">
        ...
        {!error && normalizedQuery.length >= 2 && !loading && schools.length === 0 && (
      ...
      <section className="edx-app-launcher-trust" aria-label="EDUNIXO security">
      ...
      <div className="edx-app-launcher-foot">EDUNIXO · Secure school operating system</div>
    </main>
  );
}
