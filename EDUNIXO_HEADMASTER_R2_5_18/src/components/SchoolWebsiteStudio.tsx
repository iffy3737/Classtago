...
async function prepareSchoolImage(file: File, kind: 'hero' | 'gallery' | 'logo'): Promise<PreparedSchoolImage> {
  ...
  if (file.size > 10 * 1024 * 1024) throw new Error('Choose an image smaller than 10 MB. EDUNIXO will optimize it before upload.');
...
export default function SchoolWebsiteStudio({ userRole, userName, initialTab = 'identity', allowedTabs, focusedMode = false, focusedTitle, activeFeatureId }: SchoolWebsiteStudioProps) {
  ...
  const normalizedAllowedTabs = useMemo<StudioTab[]>(() => {
    const defaultTabs: StudioTab[] = ['identity', 'media', 'content', 'contact', 'design', 'preview', 'admissions', 'applications', 'confirmation'];
    const source: StudioTab[] = allowedTabs?.length ? allowedTabs : defaultTabs;
    return Array.from(new Set<StudioTab>(source.filter(tab => tab !== 'confirmation' || userRole === 'headmaster')));
  ...
  const workspaceTitle = focusedTitle || (websiteDesignPage ? 'Website Design' : 'Admission Campaigns');
  const workspaceDescription = websiteDesignPage
    ? 'Edit the public website page-by-page. Save and publish controls remain at the bottom of every page.'
    : 'Manage campaigns and submitted applications from one focused admission workspace.';

  if (loading) return <div className="flex min-h-[520px] flex-col items-center justify-center rounded-3xl border border-slate-200 bg-white p-8 text-center shadow-sm"><Loader2 className="h-9 w-9 animate-spin text-cyan-600" /><h2 className="mt-4 text-xl font-black text-slate-950">Opening {focusedTitle || 'Website Design'}…</h2><p className="mt-2 text-sm text-slate-500">Loading the school draft, media and secure permissions.</p></div>;
    ...
    <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-950 shadow-2xl">
      <div className="relative overflow-hidden border-b border-white/10 px-5 py-6 text-white sm:px-7">
        ...
        <div className="relative flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-4"><div className="grid h-13 w-13 shrink-0 place-items-center rounded-2xl border border-white/10 bg-white/5 text-cyan-300"><WandSparkles className="h-7 w-7" /></div><div><div className="flex flex-wrap items-center gap-2"><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">EDUNIXO Signature Experience</p><span className="rounded-full border border-emerald-300/20 bg-emerald-300/10 px-2 py-1 text-[9px] font-black uppercase tracking-wider text-emerald-300">Cloud connected</span></div><h1 className="mt-2 text-2xl font-black tracking-tight sm:text-3xl">{workspaceTitle}</h1><p className="mt-2 max-w-3xl text-xs leading-6 text-slate-400">{focusedMode ? workspaceDescription : 'Shape the school identity and public information page-by-page. Draft changes remain private until the Headmaster publishes them.'}</p></div></div>
        ...
        <div className="w-full max-w-lg overflow-hidden rounded-[1.75rem] border border-white/10 text-white shadow-[0_32px_100px_rgba(2,6,23,.7)]" style={{background:'radial-gradient(circle at 0% 0%, rgba(34,211,238,.16), transparent 36%), radial-gradient(circle at 100% 0%, rgba(139,92,246,.16), transparent 38%), linear-gradient(145deg,#07172d,#090d1f 60%,#181235)',color:'#ffffff'}}>
          <div className="relative overflow-hidden border-b border-white/10 p-6 sm:p-7">
            ...
            <div className="relative flex items-start gap-4">
              ...
              <div><p className="text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">Headmaster publishing control</p><h2 id="publish-confirm-title" className="mt-2 text-xl font-black tracking-tight">{dirty ? 'Save changes and publish website?' : 'Publish the saved draft?'}</h2><p className="mt-3 text-xs leading-6 text-slate-400">{dirty ? 'The latest edits will first be saved permanently to the school cloud and then published. The current live version will remain available in revision history.' : 'The saved private draft will become the live public school website. The current live version will remain available in revision history.'}</p></div>
            </div>
          </div>
          ...
          {websiteDesignPage && <div className="mt-8 rounded-[1.5rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-2 text-xs leading-5 text-slate-500">Save Draft stores all Website Design pages privately. Publish Website is a separate Headmaster action and creates revision history.</p>
              </div>
  ...
  return <div id={focusFeatureId === 'media-archive-restore' ? 'edx-studio-media-archive-restore' : focusFeatureId === 'media-usage-audit' ? 'edx-studio-media-usage-audit' : 'edx-studio-media-library'} className="scroll-mt-24 overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm">
    ...
    {archiveCandidate && <div className="fixed inset-0 z-[100] grid place-items-center bg-slate-950/75 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-label="Archive school media">
      <div className="w-full max-w-md overflow-hidden rounded-[1.75rem] border border-white/10 bg-white shadow-2xl">
        <div className="bg-[radial-gradient(circle_at_top_left,rgba(244,63,94,.18),transparent_38%),#0f172a] p-6 text-white"><div className="flex items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-rose-500/15 text-rose-300"><Archive className="h-6 w-6" /></div><div><div className="text-[10px] font-black uppercase tracking-[0.2em] text-rose-300">Recoverable archive</div><h4 className="mt-2 text-xl font-black">Archive this media asset?</h4><p className="mt-2 text-xs leading-6 text-slate-300">The cloud original will be preserved and can be restored. EDUNIXO will block the action if the image is still used by a saved draft or the live website.</p></div></div></div>
