...
export default function AcademicSetupModule({
  ...
  const addSubjectGroup = () => {
    ...
    handleSaveSetup(updated, 'ADD_SUBJECT_GROUP', `Created Subject Group: ${name}`);
  };
  ...
  const toggleExamTerm = (id: string) => {
    if (!isHeadmaster) return;
    const updated = {
      ...setup,
      examTerms: setup.examTerms.map(t => t.id === id ? { ...t, isActive: !t.isActive } : t)
    };
    handleSaveSetup(updated, 'TOGGLE_EXAM_TERM', `Toggled exam term status`);
  };
        ...
        <div className="flex items-center gap-3 bg-slate-800/80 border border-slate-700/60 px-4 py-2.5 rounded-xl text-xs no-print">
          ...
          <div>
            ...
            <div className="font-bold text-slate-200">
              {userRole === 'clerk' ? `${userName} (Clerk Full Access)` : (isHeadmaster ? 'Headmaster (Write Access)' : `${userName} (Read Only)`) }
            </div>
          </div>
        </div>
      </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
                {setup.examTerms.map((term) => (
                  ...
                  >
                    ...
                    <button
                      disabled={!isHeadmaster}
                      onClick={() => toggleExamTerm(term.id)}
                      className={`text-[9px] font-bold px-1.5 py-0.5 rounded cursor-pointer ${
