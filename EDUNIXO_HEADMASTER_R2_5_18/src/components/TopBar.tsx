...
import { LogOut, ShieldAlert, GraduationCap, Database, Globe2, Sparkles, UserCircle2 } from 'lucide-react';
        ...
        <div className="edx-mobile-topbar-inner">
          <div className="edx-mobile-brand-lockup">
            ...
            <div className="min-w-0">
              <p className="edx-mobile-brand-eyebrow">EDUNIXO</p>
      ...
      <div className="mx-auto flex max-w-[96rem] flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div className="flex items-center justify-between gap-4">
          <button className="flex min-w-0 items-center gap-3 text-left" onClick={() => onNavigate(user?.role === 'super_admin' ? 'erp' : 'website')}>
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2"><h1 className="truncate text-sm font-black uppercase tracking-tight text-white sm:text-base">{user?.role === 'super_admin' ? 'EDUNIXO ERP' : (schoolName || t.schoolName)}</h1><span className="hidden rounded-full border border-emerald-300/20 bg-emerald-300/10 px-2 py-1 text-[8px] font-black uppercase tracking-wider text-emerald-300 sm:inline">Secure cloud</span></div>
