...
export default function DashboardOverview({ lang, user, onRefreshData }: DashboardOverviewProps) {
  ...
  const roleModuleCatalogue = useMemo(
    () => getRoleModuleCatalogue(user.role, isClassTeacher),
  ...
  useEffect(() => {
    ...
    const loadModuleEntitlements = async () => {
      ...
      try {
        ...
        console.error('Plan entitlements could not be loaded. Access is restricted until verification succeeds.', error);
        if (!cancelled) {
          // Fail closed: a temporary network/API error must never reveal paid or
          // role-restricted operational modules. Dashboard/security remain open.
          setEntitlementEnforced(true);
          setAccessState('error');
          setEntitlementError(error?.message || 'Plan access could not be verified.');
  ...
  const openRoleModuleById = (moduleId: string, featureId?: string) => {
    ...
    if (legacyRoute) {
      openRoleModuleById(legacyRoute.moduleId, featureId || legacyRoute.featureId);
      return;
    }
    const module = allRoleModules.find(item => item.id === moduleId);
    if (!module) {
      console.warn(`EDUNIXO route is not visible for this account: ${moduleId}${featureId ? ` / ${featureId}` : ''}`);
      return;
    }
    const feature = featureId ? module.features.find(item => item.id === featureId) : null;
    if (featureId && !feature) {
      console.warn(`EDUNIXO feature route is not visible for this account: ${moduleId} / ${featureId}`);
      return;
    }
  ...
  const renderSecurityPasswordPage = () => (
    <div className="mx-auto max-w-2xl animate-fade-in text-left" data-security-feature={activeFeatureId || 'security-change-password'}>
      <section className="overflow-hidden rounded-[1.75rem] border border-white/10 bg-white shadow-[0_30px_90px_rgba(15,23,42,.16)]">
        <header className="relative overflow-hidden bg-[radial-gradient(circle_at_12%_0%,rgba(34,211,238,.22),transparent_36%),radial-gradient(circle_at_92%_0%,rgba(139,92,246,.24),transparent_38%),linear-gradient(135deg,#07172d,#080b1c_56%,#17112e)] px-6 py-7 text-white sm:px-8">
  ...
  return (
    <div
      className="erp-dashboard-root edx-global-page-shell space-y-8 print:bg-white print:p-0"
    ...
    >
      ...
      {!showShortcutHub && hasFocusedWorkspace && (
        <div className="edx-operational-canvas edx-theme-safe">
          {showCommandCenterFocusedWorkspace && (
      ...
      {activeRoleModule && activeRoleFeature && activeRoleModule.id !== 'hm-student-master' && (
        <RoleFeatureContextHeader
          module={activeRoleModule}
          feature={activeRoleFeature}
          onBack={returnToActiveModuleRoot}
        />
      )}
      ...
      {activeTab === 'website_studio' && (user.role === 'headmaster' || user.role === 'clerk') && (
        <div className="animate-fade-in text-left">
          <SchoolWebsiteStudio userRole={user.role} userName={user.name} />
        </div>
      )}
