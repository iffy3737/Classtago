...
function Workload({ data }: { data: AnalyticsPayload['staffWorkload'] }) {
  ...
  return <Panel title="Staff Workload Analysis" text={data.assignmentsAvailable ? `Assignment sources: ${(data.sourceTables || []).join(', ') || 'school assignments'}.` : 'Staff records are live, but assignment tables are not connected yet.'}><MiniTable title="Academic assignments" headers={['Staff','Subjects','Class Teacher','Total']} rows={data.rows.map(x => [x.name,String(x.subjectAssignments),String(x.classTeacherAssignments),String(x.totalAssignments)])} /></Panel>;
}
