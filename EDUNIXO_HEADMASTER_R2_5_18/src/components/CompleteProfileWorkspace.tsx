...
const STAFF_ROLES = new Set(['headmaster', 'clerk', 'teacher', 'class_teacher', 'super_admin']);

function safe(value: unknown): string { return String(value ?? '').trim(); }

export default function CompleteProfileWorkspace({ user }: { user: User }) {
  ...
  const completion = useMemo(() => {
    const required: Array<string | undefined> = [fullName, phone, photoDataUrl || photoUrl, details.gender, details.dob, details.address, details.motherTongue];
    if (isStaff) required.push(details.designation, details.qualification, details.joiningDate);
    if (user.role === 'parent') required.push(details.emergencyContactPhone);
    const complete = required.filter(value => safe(value)).length;
    return Math.round((complete / Math.max(1, required.length)) * 100);
  }, [fullName, phone, photoDataUrl, photoUrl, details, isStaff, user.role]);
  ...
  const identityId = user.role === 'teacher' || user.role === 'class_teacher' ? (user.shalarthId || user.username)
    : user.role === 'clerk' ? (user.employeeCode || user.username)
    : user.role === 'student' ? (user.grNumber || user.username)
    : user.username;

  return (
    <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-xl">
      <div className="bg-[radial-gradient(circle_at_10%_10%,rgba(34,211,238,.17),transparent_32%),linear-gradient(135deg,#071426,#0b1026_55%,#17112e)] p-6 text-white sm:p-8">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4"><div className="grid h-12 w-12 place-items-center rounded-2xl border border-white/15 bg-white/10"><UserRound className="h-6 w-6 text-cyan-300" /></div><div><div className="text-[10px] font-black uppercase tracking-[.18em] text-cyan-300">Personal account</div><h2 className="mt-1 text-2xl font-black">My Complete Profile</h2><p className="mt-1 text-xs text-slate-300">Complete your permitted personal and professional details. Login identity and role remain protected.</p></div></div>
      ...
      <form onSubmit={saveProfile} className="space-y-7 bg-slate-50 p-5 sm:p-7">
        ...
        <ProfileSection title="Profile Photo & Contact" subtitle="Your photo and permitted contact details are shared across the ERP header and dashboard.">
          <div className="grid gap-5 lg:grid-cols-[180px_1fr]">
            ...
            <div className="grid gap-4 sm:grid-cols-2"><Field label="Full Name *"><input value={fullName} onChange={e => setFullName(e.target.value)} disabled={!fullNameEditable} required className="edx-complete-profile-input" /></Field><Field label="Mobile Number"><input type="tel" value={phone} onChange={e => setPhone(e.target.value)} className="edx-complete-profile-input" /></Field><div className="sm:col-span-2"><Field label="Residential Address"><textarea rows={3} value={details.address} onChange={e => patch('address', e.target.value)} className="edx-complete-profile-input" /></Field></div></div>
          </div>
        </ProfileSection>

        <ProfileSection title="Personal Details" subtitle="These details are your personal profile. Official school identifiers stay read-only.">
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <Field label="Father's Name"><input value={details.fatherName} onChange={e => patch('fatherName', e.target.value)} className="edx-complete-profile-input" /></Field>
            <Field label="Mother's Name"><input value={details.motherName} onChange={e => patch('motherName', e.target.value)} className="edx-complete-profile-input" /></Field>
            <Field label="Gender"><select value={details.gender} onChange={e => patch('gender', e.target.value)} className="edx-complete-profile-input"><option value="">Select</option><option>Male</option><option>Female</option><option>Other</option></select></Field>
            <Field label="Date of Birth"><input type="date" value={details.dob} onChange={e => patch('dob', e.target.value)} className="edx-complete-profile-input" /></Field>
            <Field label="Blood Group"><select value={details.bloodGroup} onChange={e => patch('bloodGroup', e.target.value)} className="edx-complete-profile-input"><option value="">Select</option>{BLOOD_GROUPS.map(item => <option key={item}>{item}</option>)}</select></Field>
