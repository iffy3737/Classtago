...
export async function ensureHeadmasterTeachingProfile(activeUser: TeacherCommunicationActiveUser) {
  ...
  const ids = [profile.employee_code, profile.shalarth_id]
  ...
  const created = await supabaseAdmin.from('teachers').insert({
    school_id: activeUser.schoolId,
    user_id: activeUser.userId,
    shalarth_id: profile.shalarth_id || null,
    employee_id: profile.employee_code || null,
...
async function loadStudents(activeUser:TeacherCommunicationActiveUser, scope:TeacherCommunicationScope){
  ...
  const userIds=[...new Set(scoped.map((r:any)=>String(r.user_id||r.auth_user_id||'')).filter(Boolean))];
  const [users,profiles]=await Promise.all([
    userIds.length?supabaseAdmin.from('users').select('id,email,phone_number,status,is_active').in('id',userIds):Promise.resolve({data:[],error:null} as any),
    userIds.length?supabaseAdmin.from('user_profile_extensions').select('user_id,preferred_language').in('user_id',userIds):Promise.resolve({data:[],error:null} as any)
  ]);
  ...
  return scoped.map((st:any)=>{ const userId=String(st.user_id||st.auth_user_id||'')||undefined; const u:any=userId?um.get(userId):null; const p:any=userId?pm.get(userId):null; return {
    ...
    parentName:String(st.father_name||st.mother_name||st.guardian_name||st.parent_name||'')||undefined, parentMobile:String(st.parent_mobile||st.guardian_mobile||st.primary_contact_number||'')||undefined, parentEmail:String(st.parent_email||st.guardian_email||'')||undefined,
...
export async function sendTeacherCommunicationServer(activeUser:TeacherCommunicationActiveUser,input:any){
  ...
  if(audience==='parents' && channels.every(c=>c==='website')) throw new Error('Parent/Guardian Website delivery will activate with the Parent Portal. Select a configured external channel for parent-only communication.');
  ...
  let websiteCreated=0; if(channels.includes('website')){const rows=recipientRows.filter(r=>r.audience_kind==='student'&&r.recipient_user_id).map(r=>({school_id:activeUser.schoolId,recipient_user_id:r.recipient_user_id,recipient_student_id:r.recipient_student_id,title,body,notification_type:type,source_message_id:messageId,source_key:`teacher-communication:${messageId}:${r.recipient_user_id}`,is_read:false,created_at:new Date().toISOString()}));if(rows.length){const nr=await supabaseAdmin.from('edunixo_user_notifications').insert(rows);if(nr.error)throw nr.error;websiteCreated=rows.length;}}
