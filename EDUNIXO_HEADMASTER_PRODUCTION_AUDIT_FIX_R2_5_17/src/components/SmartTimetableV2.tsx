...
export default function SmartTimetableV2({
  ...
  const [setup, setSetup] = useState<V2SchoolSetup>(() => {
    ...
    return {
      academicYear:
        academicSetup.academicYears?.find((y) => y.isActive)?.year || "2026-27",
  ...
  const mapTeachingAssignmentsToWorkload = (payload: any): V2WorkloadRow[] => {
    ...
    return subjectRows
      .map((row: any, index: number) => {
        ...
        const isClassTeacher = teacherMatchesClassTeacher && !classTeacherPriorityUsed.has(scope);
        if (isClassTeacher) classTeacherPriorityUsed.add(scope);

        return {
          id: String(row?.cloudId || row?.id || `assignment_${index}`),
          teacherName,
          className,
          division,
          subjectName,
          ...
          remarks: isClassTeacher
            ? 'Class Teacher priority · Synced from Teaching Assignments'
  ...
  const handleGenerateTimetable = () => {
    ...
    const logTimeline = [
      ...
      },
      {
        prg: 65,
        msg: t(
          "Assigning first period of each division to their respective Class Teacher...",
          "प्रत्येक श्रेणी की पहली अवधि उनके संबंधित कक्षा शिक्षक को आवंटित की जा रही है...",
          "ہر کلاس کی پہلی مدت ان کے متعلقہ کلاس ٹیچر کو الاٹ کی جا رہی ہے...",
        ),
      },
      {
        prg: 80,
  ...
  return (
    <div
      ...
      className="bg-slate-900 border border-slate-800 text-slate-100 rounded-3xl p-4 md:p-8 space-y-6 shadow-2xl relative overflow-hidden font-sans"
    >
          ...
          <div className="space-y-5">
                  ...
                  <tbody className="divide-y divide-slate-800/70">
                    {assignmentCloudLoading && !workload.length ? (
                      ...
                      <tr key={row.id} className="transition hover:bg-slate-900/55">
                        ...
                        <td className="px-4 py-3.5">
                          {row.isClassTeacher ? (
                            <span className="rounded-full border border-amber-400/20 bg-amber-400/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-amber-300">Class Teacher</span>
                          ) : (
                            <span className="text-[11px] font-bold text-slate-500">Subject Teacher</span>
                          )}
                        </td>
                        <td className="px-4 py-3.5">
            ...
            <div className="flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-950/60 p-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-start gap-2 text-[11px] leading-relaxed text-slate-400">
                ...
                <span>No duplicate timetable workload entry. Changes belong to <strong className="text-slate-200">Academic Setup → Teaching Assignments</strong> and automatically flow here.</span>
              </div>
              <button
                onClick={() => setActiveTab("generate")}
              ...
              >
                <div className="text-center border-b-2 border-slate-900 pb-4 space-y-1">
                  <h2 className="font-extrabold text-xl tracking-wide uppercase text-slate-900">
                    {academicSetup.schoolProfile?.schoolName || 'National High School, Taloda'}
                  </h2>
                  <p className="text-xs font-sans text-slate-600 font-bold">
                    {[academicSetup.schoolProfile?.address, academicSetup.schoolProfile?.taluka, academicSetup.schoolProfile?.district]
                      .filter(Boolean).join(', ') || 'Taloda, Dist. Nandurbar'} | Academic Year {setup.academicYear}
                  </p>
      ...
      <div className="border-t border-slate-850 pt-4 flex flex-col sm:flex-row items-center justify-between gap-4 text-[10px] font-mono text-slate-500">
        <p>
          National High School Taloda ERP • V2 isolated sandbox engine v2.0-beta
        </p>
