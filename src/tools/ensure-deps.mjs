...
if (stampMatches && missing.length === 0) {
  console.log('[Classtago] Dependencies ready.');
  process.exit(0);
}

if (missing.length) {
  console.log(`[Classtago] Installing missing project dependencies: ${missing.join(', ')}`);
} else {
  console.log('[Classtago] Package definition changed; refreshing dependencies.');
}
...
if (result.error) {
  console.error('[Classtago] Dependency installation could not start:', result.error.message);
  process.exit(1);
}
if (result.status !== 0) {
  console.error(`[Classtago] Dependency installation failed with exit code ${result.status}.`);
  process.exit(result.status ?? 1);
}

const stillMissing = required.filter((name) => !packageAvailable(name));
if (stillMissing.length) {
  console.error(`[Classtago] Install completed but packages are still unavailable: ${stillMissing.join(', ')}`);
  process.exit(1);
}

writeFileSync(stamp, hash, 'utf8');
console.log('[Classtago] Dependencies installed and verified.');
