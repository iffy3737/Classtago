...
if (!existsSync(android)) {
  console.log('[Classtago Android] Creating native Android project from installed Capacitor template.');
  run(['cap', 'add', 'android']);
} else if (!existsSync(wrapperJar)) {
  if (existsSync(backup)) throw new Error(`Safety stop: ${backup} already exists.`);
  console.log('[Classtago Android] Hydrating trusted Gradle wrapper files from installed Capacitor template.');
...
console.log('[Classtago Android] Android project hydrated and synchronized.');
