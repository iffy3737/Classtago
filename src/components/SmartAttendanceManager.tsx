...
export default function SmartAttendanceManager({ lang, user, onRefreshData, activeFeatureId = null }: SmartAttendanceManagerProps) {
  ...
  const activeYear = useMemo(() => {
    return academicSetup?.academicYears?.find((y: any) => y.isActive)?.year || '';
  }, [academicSetup]);
          ...
          <div className="space-y-1">
            ...
            <p className="text-xs text-slate-300 leading-relaxed max-w-xl">
              {isUrdu 
                ...
                : "Automatically synced with the school timetable. Select classes or periods below to take attendance."}
            </p>
          </div>
  ...
  const handleGenerateMonthlyRegister = () => {
    ...
    const activeYearStr = monAcademicYear || activeYear;
  ...
  const handleExportExcel = async () => {
    try {
      ...
      subHeaderCell.value = `Govt. of Maharashtra Approved | UDISE Code: ${academicSetup?.schoolProfile.udiseCode || 'Not configured'}`;
  ...
  const handleExportMonthlyExcel = async (monData: any) => {
    ...
    try {
      ...
      subHeaderRow.height = 20;
      const subHeaderCell = worksheet.getCell('A2');
      subHeaderCell.value = `Govt. of Maharashtra Approved | UDISE Code: ${academicSetup?.schoolProfile?.udiseCode || 'Not configured'} | ${academicSetup?.schoolProfile?.address || 'School address not configured'}`;
      subHeaderCell.font = { name: 'Arial', size: 9, italic: true, color: { argb: 'FF334155' } };
      subHeaderCell.fill = lightFill;
      subHeaderCell.alignment = { vertical: 'middle', horizontal: 'center' };
      ...
      <div className="space-y-6 text-left">
                  ...
                  <tbody>
                    <tr>
                      <td style={{ border: 'none', textAlign: 'center' }}>
                        ...
                        <h1 className="text-base font-black uppercase text-slate-950 tracking-wide m-0">{academicSetup?.schoolProfile?.schoolName || "School"}</h1>
        ...
        <div className="print-only-container hidden print:block space-y-8 bg-white p-8">
          <div className="border-b-2 border-slate-950 pb-4 flex flex-col items-center text-center space-y-1">
            <h1 className="text-xl font-black uppercase text-slate-950 tracking-wide">{academicSetup?.schoolProfile?.schoolName || "School"}</h1>
