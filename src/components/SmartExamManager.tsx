...
export default function SmartExamManager({ lang, user, onRefreshData }: SmartExamManagerProps) {
  ...
  const loadDatabaseState = () => {
    ...
    const configuredYears = Array.isArray(setup?.academicYears)
      ? setup.academicYears
          .map((y: any) => typeof y === 'string' ? y : String(y?.yearCode || y?.year || y?.label || ''))
          .filter(Boolean)
      : [];
    const examYears = LocalERPDatabase.getExaminations().map((e: any) => String(e.academicYear || '')).filter(Boolean);
    const uniqueYears = Array.from(new Set([...configuredYears, ...examYears]));
    setAcademicYears(uniqueYears);
    const activeConfiguredYear = Array.isArray(setup?.academicYears)
      ? setup.academicYears.find((y: any) => y?.isActive || y?.active)
      : null;
    const resolvedActiveYear = String(activeConfiguredYear?.year || activeConfiguredYear?.yearCode || activeConfiguredYear?.label || uniqueYears[0] || '');
    setActiveAcademicYear(resolvedActiveYear);
  ...
  const handleCopyPreviousYearExams = async () => {
    if (!canEdit) return notifyError("Unauthorized operation");
    const previousYear = academicYears.filter(y => y !== activeAcademicYear).sort().reverse()[0] || '';
    if (!previousYear) return notifyError('No previous academic year is configured.');
    const sourceYear = await requestActionConfirm({ title: 'Copy previous-year examinations?', message: `Copy examination structures from preceding academic year '${previousYear}' to current session?`, confirmLabel: 'Copy Structure', tone: 'warning' });
    if (sourceYear) {
      const previousExams = examinations.filter(e => e.academicYear === previousYear);
      if (previousExams.length === 0) {
        notifyError(`No examinations are configured for ${sourceYear}. Nothing was copied.`);
        return;
      }
      ...
      {activeTab === 'overview' && (
        <div className="space-y-6 animate-fade-in no-print">
          ...
          <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-4 flex gap-3 text-xs text-blue-800 leading-relaxed">
            ...
            <div>
              <span className="font-bold block text-blue-900 mb-0.5">Classtago Examination Module Connectivity</span>
        ...
        <div className="space-y-6 animate-fade-in print:space-y-0">
          ...
          <div className={`p-8 border border-slate-200 rounded-2xl bg-white shadow-sm max-w-4xl mx-auto print:border-none print:shadow-none print:p-0 print:max-w-none ${
            ...
            <div className="flex justify-between items-center text-[10px] text-slate-400 font-mono border-b border-slate-100 pb-2 mb-4">
              <span>{academicSetup?.schoolProfile?.schoolName || 'School'}</span>
