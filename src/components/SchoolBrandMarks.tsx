...
export function SchoolBrandMark({
  schoolName = 'School',
  managementName = 'School Management',
...
export function SchoolSealMark({ schoolName = 'School', location = 'Location not configured', className = '' }: SchoolSealMarkProps) {
  const rawId = useId().replace(/:/g, '');
  ...
  return (
    <svg viewBox="0 0 220 220" className={className} role="img" aria-label={`${schoolName} official seal`}>
      ...
      <text x="110" y="91" textAnchor="middle" fill="currentColor" fontSize="12" fontWeight="900">{schoolName.length > 24 ? schoolName.slice(0, 24).toUpperCase() : schoolName.toUpperCase()}</text>
      <text x="110" y="116" textAnchor="middle" fill="currentColor" fontSize="11" fontWeight="800">OFFICIAL SCHOOL SEAL</text>
      <text x="110" y="139" textAnchor="middle" fill="currentColor" fontSize="10" fontWeight="800">{location.length > 24 ? location.slice(0, 24).toUpperCase() : location.toUpperCase()}</text>
    </svg>
  );
}
