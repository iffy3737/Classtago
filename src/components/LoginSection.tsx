...
import { passkeyFeatureEnabled, passkeySupportedOnDevice, signInWithPasskey } from '../lib/passkey';
...
export default function LoginSection({ lang, onLoginSuccess, schoolId, schoolCode, schoolName }: LoginSectionProps) {
  ...
  const handlePasskeyLogin = async () => {
    ...
    try {
      ...
      if (!restored.user) throw new Error(restored.error?.message || 'Passkey sign-in succeeded but the Classtago school profile could not be resolved.');
      if (schoolId && restored.schoolId && String(restored.schoolId) !== String(schoolId)) {
        await AuthService.logout();
        throw new Error('This passkey belongs to a different school workspace. Open that school website and try again.');
      }
      onLoginSuccess(restored.user);
    } catch (error:any) { setErrorMsg(error?.message || 'Passkey sign-in failed.'); }
  ...
  const handleParentSignup = async (e: React.FormEvent) => {
    ...
    if (!parentMobile || !parentGrNumber || !parentPassword || !parentConfirmPassword) {
      setErrorMsg('Ward GR/PEN, registered Parent Mobile and both password fields are required.');
      return;
    }
    if (parentPassword.length < 6 || parentPassword.length > 72) {
      setErrorMsg('Password must contain 6 to 72 characters.');
      return;
    }
    ...
    setIsLoading(true);
    try {
      const response = await fetch('/api/public/register-parent-account', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          childIdentifier: parentGrNumber.trim().toUpperCase(),
          parentMobile: parentMobile.trim(),
          childDob: parentStudentDob || null,
          password: parentPassword,
          schoolCode: schoolCode || undefined
        })
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Parent registration failed.');

      setRegistrationNotice({
        title: 'Parent signup request created',
        message: payload.message || 'Your verified Parent account request is waiting for Headmaster approval.',
        requestCode: payload.requestCode || payload.requestId || undefined
      });
      setMode('login');
      setUsername(payload.username || `P${parentMobile.replace(/\D/g, '')}-${String(schoolCode || 'NHS01').toUpperCase()}`);
      setPassword('');
      setParentPassword('');
      setParentConfirmPassword('');
    } catch (error: any) {
      setErrorMsg(error?.message || 'Parent registration failed.');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="max-w-md mx-auto space-y-6 py-6 font-sans text-left">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl p-8 space-y-6">
          ...
          <p className="text-xs text-slate-500 leading-relaxed text-center">
            Hello, <strong className="text-slate-800">{pendingChangeUser.name}</strong>. In compliance with Classtago security guidelines, you must update your temporary credential keys on first-time access.
          </p>
      ...
      <div className="max-w-md mx-auto space-y-6 py-4 font-sans text-left">
        <div className="bg-white rounded-2xl border border-slate-200 shadow-xl p-6 space-y-6">
          ...
          <form onSubmit={handleParentSignup} className="space-y-4">
            ...
            >
              {isLoading ? 'Verifying Student Master…' : 'Sign Up and Await Headmaster Approval'}
            </button>
          </form>
        </div>
      </div>
    ...
    <div className="edx-auth-premium" data-edx-surface="premium-school-login-r257">
      <div className="edx-auth-premium-card">
        ...
        <div className="edx-auth-premium-head">
          ...
          <div className="edx-auth-premium-eyebrow"><ShieldCheck className="w-3.5 h-3.5" /> Classtago SECURE SCHOOL ACCESS</div>
          <h2>{t.erpPortal}</h2>
          <p>{schoolName || 'Classtago School'}</p>
        ...
        <form onSubmit={handleLoginSubmit} className="edx-auth-premium-form">
          ...
          {passkeyFeatureEnabled && passkeySupportedOnDevice() && <button type="button" disabled={isLoading} onClick={()=>void handlePasskeyLogin()} className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-cyan-300/30 bg-cyan-50 px-4 py-3 text-xs font-black text-cyan-800"><KeyRound className="h-4 w-4"/>Sign in with Passkey / Biometric</button>}
        </form>
        ...
        <div className="edx-auth-premium-signup">
          ...
          <div className="edx-auth-premium-signup-grid">
            ...
            <button type="button" onClick={() => { setErrorMsg(''); setMode('parent_signup'); }}>
              <span className="edx-auth-premium-mini-icon edx-auth-premium-mini-violet"><UserPlus className="w-4 h-4" /></span>
              <span><b>Parent Sign Up</b><small>Child GR + registered parent mobile</small></span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
