...
export default function PremiumSchoolPortal({
  ...
  const [mariaReceptionOpen,setMariaReceptionOpen]=useState(false);
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      ...
      if (section === 'admissions') {
        if (school.admissionsEnabled !== false && school.admissionsOpen === true) setAdmissionOpen(true);
        else document.getElementById('contact')?.scrollIntoView({ behavior:'smooth', block:'start' });
        return;
      }
      const allowed = new Set(['about','academics','facilities','notices','gallery','contact']);
      if (allowed.has(section)) document.getElementById(section)?.scrollIntoView({ behavior:'smooth', block:'start' });
    };
    window.addEventListener('edunixo_public_school_action', handler as EventListener);
    return () => window.removeEventListener('edunixo_public_school_action', handler as EventListener);
  }, [school.admissionsEnabled, school.admissionsOpen]);
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-between gap-x-6 gap-y-2 px-5 py-2.5 text-[10px] font-bold lg:px-8">
          <div className="flex min-w-0 flex-wrap items-center gap-x-5 gap-y-2">
            <button type="button" onClick={onBackToPlatform} className="inline-flex items-center gap-1.5 uppercase tracking-[0.18em] text-[var(--school-accent)] transition hover:text-white">
              <GraduationCap className="h-3.5 w-3.5" /> Classtago School Network
            </button>
      ...
      <header className="edx-public-dark-surface sticky top-0 z-40 border-b border-white/10 bg-[#050816]/80 backdrop-blur-2xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
          ...
          <nav className="hidden items-center gap-6 lg:flex">
            ...
            <button onClick={onBackToPlatform} className="text-xs font-bold text-slate-400 transition hover:text-white">Classtago</button>
          </nav>
