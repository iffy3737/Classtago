...
import { canonicalTemplateKeyFromLegacy, loadLegacyClerkMasterTemplates, publishLegacyClerkMasterTemplate } from '../modules/teacherResultFresh/clerkResultMasterBridge';
...
export default function ResultManagement({
  ...
  const activeAcademicYear = setup?.academicYears?.find((y: any) => y.isActive)?.year || setup?.academicYears?.[0]?.year || '';
  ...
  const examinations = React.useMemo(() => {
    // Production integrity: never fabricate examination masters when the school
    // has not configured them. Existing workflows can still use real active
    // examinations from the local/cloud-backed academic setup.
    return LocalERPDatabase.getExaminations().filter(e => e.status === 'Active');
  }, [setup]);
  ...
  const getSubjectLockStates = () => {
    // Production integrity: lock state is evidence of an actual workflow action.
    // Never create synthetic Draft/Approved/Pending records merely to populate
    // the dashboard or print queue.
    const _ = refreshCounter;
    return LocalERPDatabase.getSubjectLockStates();
  };

  const headmasterResultStats = React.useMemo(() => {
    const states = LocalERPDatabase.getSubjectLockStates();
    const completed = states.filter((s: any) => String(s.status || '').toLowerCase() === 'approved').length;
    const draft = states.filter((s: any) => String(s.status || '').toLowerCase() === 'draft').length;
    const returned = states.filter((s: any) => {
      const status = String(s.status || '').toLowerCase();
      return status === 'returned' || status === 'returned for correction';
    }).length;
    const pending = states.filter((s: any) => {
      const status = String(s.status || '').toLowerCase();
      return status === 'pending review' || status === 'submitted' || status === 'resubmitted';
    }).length;
    const actionable = states.length;
    const finished = completed;
    const progress = actionable > 0 ? Math.round((finished / actionable) * 100) : 0;
    return { completed, draft, pending, returned, progress };
  }, [refreshCounter, setup]);
  ...
  const getCustomizedTemplateForTeacher = (baseTemplate: any, alloc: any, exam: any) => {
    ...
    const acadYear = alloc?.academicYear || activeAcademicYear;
  ...
  const handleGenerateMarkList = (alloc: any, exam: any) => {
    ...
    const lockId = `lock_${activeAcademicYear}_${exam.id}_${alloc.className.replace(/\s+/g, '_')}_${alloc.divisionName.replace(/\s+/g, '_')}_${alloc.subjectName.replace(/\s+/g, '_')}`;
    ...
    const entries: StudentMarkEntry[] = students.map(student => {
      ...
      return {
        id: `mark_${activeAcademicYear}_${exam.id}_${alloc.className.replace(/\s+/g, '_')}_${alloc.divisionName.replace(/\s+/g, '_')}_${alloc.subjectName.replace(/\s+/g, '_')}_${student.id}`,
        academicYear: activeAcademicYear,
        examId: exam.id,
        classId: alloc.className,
        division: alloc.divisionName,
  ...
  const handleBulkSendToResultBook = () => {
    ...
    ctClassAllocations.forEach((alloc: any) => {
      ctActiveExams.forEach((examId) => {
        const lockId = `lock_${activeAcademicYear}_${examId}_${alloc.className.replace(/\s+/g, '_')}_${alloc.divisionName.replace(/\s+/g, '_')}_${alloc.subjectName.replace(/\s+/g, '_')}`;
  ...
  const performApproval = (lockId: string, allocation: any, cardId: string, isReplacing: boolean) => {
    ...
    try {
      ...
      const newPrintableMarkList = {
        id: `pml_${activeAcademicYear}_${examId}_${allocation.className.replace(/\s+/g, '_')}_${allocation.divisionName.replace(/\s+/g, '_')}_${allocation.subjectName.replace(/\s+/g, '_')}`,
        academicYear: activeAcademicYear,
        className: allocation.className,
        divisionName: allocation.divisionName,
        subjectName: allocation.subjectName,
    ...
    try {
      ...
      approvedEntries.forEach(entry => {
        ...
        const rbIndex = studentResultBooks.findIndex((rb: any) => 
          rb.academicYear === activeAcademicYear &&
  ...
  const handleSaveMarks = (submitAsFinal: boolean) => {
    ...
    const lockId = `lock_${activeAcademicYear}_${examId}_${classId.replace(/\s+/g, '_')}_${division.replace(/\s+/g, '_')}_${subjectName.replace(/\s+/g, '_')}`;
    ...
    const updatedEntries = markListStudents.map(student => {
      ...
      if (isHindiMarathi) {
        ...
        return {
          id: `mark_${activeAcademicYear}_${examId}_${classId.replace(/\s+/g, '_')}_${division.replace(/\s+/g, '_')}_${subjectName.replace(/\s+/g, '_')}_${student.id}`,
          academicYear: activeAcademicYear,
          examId,
          classId,
          division,
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center border-b border-slate-100 pb-3">
          ...
          <span className="text-xs bg-slate-100 text-slate-500 font-bold px-2.5 py-1 rounded-md">
            {isUrdu ? `تعلیمی سال: ${activeAcademicYear}` : `Academic Year: ${activeAcademicYear}`}
          </span>
        </div>
  ...
  const [printableProgressCards, setPrintableProgressCards] = useState<any[]>([]);
  const [progressCardsLoading, setProgressCardsLoading] = useState(false);
  const [progressCardsError, setProgressCardsError] = useState('');
  ...
  useEffect(() => {
    if (clerkTab === 'ledgers') {
      ...
      if (activeLedgerCategory === 'progress_cards' && isClerk) {
        let cancelled = false;
        setProgressCardsLoading(true); setProgressCardsError('');
        (async()=>{
          try {
            const { data: { session } } = await supabase.auth.getSession();
            const token = session?.access_token;
            const response = await fetch('/api/clerk/progress-card-print-queue',{headers:token?{Authorization:`Bearer ${token}`}:{},cache:'no-store'});
            const payload=await response.json().catch(()=>({}));
            if(!response.ok) throw new Error(payload.error||'Progress Card Print Queue could not be loaded.');
  ...
  const handleDownloadResultBook = (rb: any) => {
    try {
      ...
      XLSX.writeFile(wb, `ResultBook_${rb.className}_${rb.divisionName}_${activeAcademicYear}.xlsx`);
    } catch (e) {
      console.error(e);
    }
  };
  ...
  const ctTargetClass = classTeacherAssignment?.className || '';
  const ctTargetDivision = classTeacherAssignment?.divisionName || '';
  const ctClassAllocations = (setup?.subjectAllocations || []).filter(
    (a: any) => a.className === ctTargetClass && a.divisionName === ctTargetDivision
  );
  
  const ctActiveExams = selectedCTTerm === 'term_1'
    ? examinations.slice(0, 2).map(e => e.id)
    : examinations.slice(-2).map(e => e.id);
  ...
  const ctTotalAllocated = ctClassAllocations.length * ctActiveExams.length;
      ...
      if (isClassTeacher && ctTargetClass && ctTargetDivision) {
        ...
        students.forEach(student => {
          const hasRecord = studentResultBooks.some(
            (rb: any) => rb && rb.studentId === student.id && rb.academicYear === activeAcademicYear
          );
          if (!hasRecord) {
            // Prepare a blank Result Book record without reproducing the master sheet template (saves several megabytes)
            const newRecord = {
              id: `rb_${activeAcademicYear}_${student.id}`,
              studentId: student.id,
              studentName: student.name,
        ...
        if (updated) {
          ...
          try {
            LocalERPDatabase.addAuditLog(
              ...
              'Result Book',
              `Automatically prepared blank Result Book records for ${students.length} students of Class: ${ctTargetClass} Div: ${ctTargetDivision} for ${activeAcademicYear}.`
            );
          } catch (auditError) {
  ...
  const defaultTemplatesList = useMemo(() => clerkDefaultMasterTemplates.map((template:any) => {
    const copy = JSON.parse(JSON.stringify(template));
    if (activeAcademicYear) {
      if (copy.academicYear === '2026-27') copy.academicYear = activeAcademicYear;
      if (Array.isArray(copy.sections)) copy.sections = copy.sections.map((section:any) => ({ ...section, text: String(section?.text || '').replaceAll('2026-27', activeAcademicYear) }));
    }
    return copy;
  }), [activeAcademicYear]);
  ...
  // Canonical cloud Result Master wins over browser cache on every Clerk session.
  // localStorage remains only a fast draft/cache surface for this legacy editor.
  useEffect(() => {
    if (!isClerk) return;
    let cancelled = false;
    void loadLegacyClerkMasterTemplates().then((cloud) => {
      if (cancelled || !cloud.ok || !cloud.templates.length) return;
      const base = migrateTemplatesList(defaultTemplatesList).map((item: any) => JSON.parse(JSON.stringify(item)));
      for (const cloudTemplate of cloud.templates) {
        const cloudKey = canonicalTemplateKeyFromLegacy(cloudTemplate);
        if (!cloudKey) continue;
        const index = base.findIndex((item: any) => canonicalTemplateKeyFromLegacy(item) === cloudKey);
        if (index >= 0) base[index] = JSON.parse(JSON.stringify(cloudTemplate));
        else base.push(JSON.parse(JSON.stringify(cloudTemplate)));
      }
      const migrated = migrateTemplatesList(base);
      setTemplatesList(migrated);
      try { localStorage.setItem('erp_master_templates', JSON.stringify(migrated)); } catch { /* cache only */ }
    }).catch(() => undefined);
    return () => { cancelled = true; };
  }, [isClerk]);
  ...
  const handleSaveMasterTemplateChanges = async () => {
    if (!selectedTemplate || !editingTemplate) return;

    // Cloud commit is authoritative. Do not show a local success when the canonical
    // Result Master could not be saved for other devices / Teacher Result.
    const cloudResult = await publishLegacyClerkMasterTemplate(editingTemplate);
    if (!cloudResult.ok) {
      setUploadSuccess(false);
      window.alert(`Result Master was NOT saved: ${cloudResult.message}`);
      return;
    }

    // Create a local backup only after the canonical cloud commit succeeds.
    ...
    localStorage.setItem('erp_master_templates', JSON.stringify(updatedList));
    if (LocalERPDatabase.saveMarkListTemplate) LocalERPDatabase.saveMarkListTemplate(editingTemplate);

    LocalERPDatabase.addAuditLog(
      user.id, user.name, user.role, "Edit Master Template", "Result Management",
      `Clerk saved canonical cloud Master Template changes for: ${editingTemplate.name} (${editingTemplate.id}). Backup auto-created.`
    );
  ...
  const t = {
    en: {
      ...
      common: {
        ...
        verified: "Verified",
        classLabel: "Class",
        sectionTitle: `Academic Year ${activeAcademicYear} Print Center Overview`
      },
      headmaster: {
    ...
    hi: {
      ...
      common: {
        ...
        verified: "सत्यापित",
        classLabel: "कक्षा",
        sectionTitle: `शैक्षणिक वर्ष ${activeAcademicYear} प्रिंट सेंटर अवलोकन`
      },
      headmaster: {
    ...
    ur: {
      ...
      common: {
        ...
        verified: "تصدیق شدہ",
        classLabel: "کلاس",
        sectionTitle: `تعلیمی سال ${activeAcademicYear} کا پرنٹ سینٹر کا جائزہ`
      },
      headmaster: {
              ...
              <tbody>
                ...
                <tr className="bg-white border-b border-slate-200">
                  ...
                  <td colSpan={template?.headers?.length || 1} className="border-r border-slate-200 px-4 text-center font-mono text-slate-400 text-[10px] py-1 bg-white">
                    {template?.sections?.[2]?.text || `Class Division: ${classId} - ${division} | Academic Year: ${activeAcademicYear} | Teacher Signature Register`}
                  </td>
                </tr>
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              <div className="space-y-3 font-sans text-xs text-slate-600">
                {activeLockStates.length === 0 ? (
                  <div className="p-3 bg-slate-50 rounded-lg border border-dashed border-slate-200 text-slate-400">
                    {isUrdu ? "ابھی کوئی حقیقی تعلیمی سرگرمی ریکارڈ نہیں ہوئی۔" : "No recorded academic activity yet."}
                  </div>
                ) : (
                  activeLockStates
                    .slice()
                    .sort((a: any, b: any) => String(b.lockedAt || b.updatedAt || '').localeCompare(String(a.lockedAt || a.updatedAt || '')))
                    .slice(0, 5)
                    .map((state: any) => (
                      <div key={state.id} className="p-3 bg-slate-50 rounded-lg flex justify-between items-center gap-3 text-left">
                        <span className="truncate">{state.subjectId || state.subjectName || "Subject"} · {state.classId || "Class"}{state.division ? `-${state.division}` : ''} ({state.status || "Recorded"})</span>
                        <span className="text-[10px] text-slate-400 shrink-0">{state.lockedAt || state.updatedAt ? new Date(state.lockedAt || state.updatedAt).toLocaleString() : "Recorded"}</span>
                      </div>
                    ))
                )}
              </div>
      ...
      {isClassTeacher && (
        <div className="space-y-6 animate-fade-in" id="class-teacher-dashboard-view">
          {(() => {
            ...
            const ctTotalAllocated = classAllocations.length * ctActiveExams.length;
            const ctPendingCount = Math.max(0, ctTotalAllocated - (ctSubmittedCount + ctDraftCount + ctReturnedCount + ctApprovedCount));
            const ctProgressPercent = (ctTotalAllocated > 0 && !isNaN(ctApprovedCount) && !isNaN(ctTotalAllocated))
              ? Math.round((ctApprovedCount / ctTotalAllocated) * 100) 
              : 0;
                      ...
                      return (
                        <div className="space-y-4">
                          {filteredItems.map((item: any) => {
                            const cardId = item.state?.id || `lock_${activeAcademicYear}_${item.allocation.examId}_${item.allocation.className.replace(/\s+/g, '_')}_${item.allocation.divisionName.replace(/\s+/g, '_')}_${item.allocation.subjectName.replace(/\s+/g, '_')}`;
                                  ...
                                  <div className="space-y-2 text-left w-full md:w-auto">
                                    <div className="flex items-center gap-1.5 flex-wrap">
                                      <span className="bg-slate-100 text-slate-600 text-[9px] font-black uppercase px-2.5 py-0.5 rounded-md border border-slate-200 font-sans">
                                        {isUrdu ? `تعلیمی سال: ${activeAcademicYear}` : `AY: ${activeAcademicYear}`}
                                      </span>
                                      <span className="bg-slate-100 text-slate-600 text-[9px] font-black uppercase px-2.5 py-0.5 rounded-md border border-slate-200 font-sans">
                                        ...
                                        <table className="w-full border-collapse font-sans text-xs">
                                          ...
                                          <tbody>
                                            ...
                                            <tr className="bg-white border-b border-slate-200">
                                              <td className="bg-slate-100 text-slate-500 font-mono text-[9px] text-center border-r border-slate-300 font-bold">3</td>
                                              <td colSpan={template?.headers?.length || 1} className="border-r border-slate-200 px-4 text-center font-mono text-slate-400 text-[10px] py-1 bg-white">
                                                {template?.sections?.[2]?.text || `Class Division: ${item.allocation.className} - ${item.allocation.divisionName} | Academic Year: ${activeAcademicYear} | Teacher Signature Register`}
                                              </td>
                                            </tr>
                                      ...
                                      <div className="flex flex-col sm:flex-row justify-end items-center gap-3 pt-4 border-t border-slate-200/60">
                                        ...
                                        {item.status === 'Approved' ? (
                                          ...
                                          <>
                                            <button type="button"
                                              onClick={() => {
                                                const lockId = item.state?.id || `lock_${activeAcademicYear}_${item.allocation.examId}_${item.allocation.className.replace(/\s+/g, '_')}_${item.allocation.divisionName.replace(/\s+/g, '_')}_${item.allocation.subjectName.replace(/\s+/g, '_')}`;
          ...
          {clerkTab === 'ledgers' && (
            <div className="space-y-4">
              ...
              {activeLedgerCategory === 'progress_cards' && (
                <div className="space-y-4">
                  {progressCardsError && <div className="rounded-xl border border-rose-200 bg-rose-50 p-4 text-xs font-bold text-rose-700">{progressCardsError}</div>}
                  {progressCardsLoading ? (
                    <div className="rounded-2xl border bg-white p-10 text-center text-sm font-bold text-slate-500"><RefreshCw className="mx-auto mb-3 h-6 w-6 animate-spin text-indigo-600"/>Loading finalized Progress Cards…</div>
                  ) : printableProgressCards.length === 0 ? (
                    <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center"><CheckCircle2 className="mx-auto h-8 w-8 text-slate-300"/><h5 className="mt-3 font-extrabold text-slate-700">No finalized Progress Cards are ready for print</h5><p className="mt-1 text-xs text-slate-500">Only Progress Card batches formally published and locked by the Headmaster appear here.</p></div>
                  ) : (
                    <div className="overflow-x-auto rounded-2xl border bg-white"><table className="w-full text-left text-xs"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-wide text-slate-500"><tr><th className="p-3">Academic Year</th><th className="p-3">Class</th><th className="p-3">Division</th><th className="p-3">Term</th><th className="p-3">Students</th><th className="p-3">Finalized</th><th className="p-3">Status</th></tr></thead><tbody>{printableProgressCards.map((row:any)=><tr key={row.id} className="border-t"><td className="p-3 font-bold">{row.academicYear}</td><td className="p-3 font-black">{row.className}</td><td className="p-3">{row.divisionName}</td><td className="p-3">{String(row.term||'—').replaceAll('_',' ')}</td><td className="p-3">{row.studentCount}</td><td className="p-3">{row.updatedAt ? new Date(row.updatedAt).toLocaleString() : '—'}</td><td className="p-3"><span className="rounded-full bg-emerald-100 px-2.5 py-1 text-[9px] font-black text-emerald-700">FINAL · LOCKED</span></td></tr>)}</tbody></table></div>
                  )}
                </div>
              )}
            ...
            >
              <div className="flex justify-between items-start w-full">
                <div className="space-y-1">
                  ...
                  <span className="text-2xl font-black text-emerald-600 block">{headmasterResultStats.completed}</span>
                </div>
            ...
            >
              <div className="flex justify-between items-start w-full">
                <div className="space-y-1">
                  ...
                  <span className="text-2xl font-black text-amber-500 block">{headmasterResultStats.draft}</span>
                </div>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all ${
            ...
            >
              <div className="flex justify-between items-start w-full">
                <div className="space-y-1">
                  ...
                  <span className="text-2xl font-black text-rose-600 block">{headmasterResultStats.pending}</span>
                </div>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all ${
            ...
            >
              <div className="flex justify-between items-start w-full">
                <div className="space-y-1">
                  ...
                  <span className="text-2xl font-black text-orange-500 block">{headmasterResultStats.returned}</span>
                </div>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all ${
            ...
            >
              <div className="flex justify-between items-start w-full">
                <div className="space-y-1">
                  ...
                  <span className="text-2xl font-black text-indigo-600 block">{headmasterResultStats.progress}%</span>
                </div>
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center border transition-all ${
