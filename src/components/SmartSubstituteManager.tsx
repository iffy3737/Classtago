...
export default function SmartSubstituteManager({
  ...
  const autoPreparedLeaveKeys = useRef<Set<string>>(new Set());
  ...
  const approvedLeavesList = useMemo(() => {
    ...
    return list.filter((l) => {
      ...
      if (!['teacher','class_teacher'].includes(String(l?.applicantRole || '').toLowerCase())) return false;
  ...
  // a canonical timetable has affected periods. Headmaster review/Approve remains
  // mandatory; this never silently changes the published timetable.
  useEffect(() => {
    if (!approvedLeavesList.length || !timetable.length || !selectedDate) return;
    const pending = approvedLeavesList.find((leave:any) => {
      const leaveId = String(leave?.id || '').trim();
      const teacherName = String(leave?.applicantName || '').trim();
      if (!leaveId || !teacherName) return false;
      const key = `${leaveId}:${selectedDate}`;
      if (autoPreparedLeaveKeys.current.has(key)) return false;
      if (adjustments.some(adj => adj.leaveApplicationId === leaveId && adj.date === selectedDate && adj.status !== 'Cancelled')) return false;
      return timetable.some((cell:any) => cell.day === selectedDay && String(cell.teacherName || '').toLowerCase().trim() === teacherName.toLowerCase());
    });
    if (!pending) return;
    const leaveId = String(pending.id);
    autoPreparedLeaveKeys.current.add(`${leaveId}:${selectedDate}`);
    runAutoSubstituteEngine([{ name:String(pending.applicantName), reason:(pending.category || 'Leave') as any, customReason:String(pending.reason || '') || undefined }], selectedDate, selectedDay, excludedTeachers, leaveId, { strictCanonical:true, automatic:true });
  }, [approvedLeavesList, timetable, selectedDate, selectedDay, adjustments, excludedTeachers]);
              ...
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400 font-medium">
                    {t(
                      "Approved leave with published timetable periods automatically prepares a workload-balanced Draft. You can also click a leave to rebuild/review manually.",
