...
export default function TimetableEngineDashboard({ lang, user, onTimetableChange }: TimetableEngineDashboardProps) {
  ...
  const initialAcademicSetup = LocalERPDatabase.getAcademicSetup();
  const initialAcademicYear = initialAcademicSetup.academicYears?.find((year: any) => year.isActive)?.year || initialAcademicSetup.academicYears?.[0]?.year || '';
  const initialClassName = LocalERPDatabase.getClasses()?.[0]?.className || '';
  const initialDivisionName = LocalERPDatabase.getClasses()?.[0]?.divisionName || '';
