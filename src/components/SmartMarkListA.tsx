...
export default function SmartMarkListA({ 
  ...
  const loadDatabase = () => {
    ...
    // Resolve the session only from configured academic-year data; never invent a year.
    if (initialAcademicYear) {
      setSelectedAcademicYear(initialAcademicYear);
    } else if (!selectedAcademicYear) {
      const configuredYear = academicSetup?.academicYears?.find((y: any) => y.isActive)?.year
        || academicSetup?.academicYears?.[0]?.year
        || '';
  ...
  const exportToWord = () => {
    ...
    const wordHTML = `
      ...
      <body>
        <h2>{academicSetup?.schoolProfile?.schoolName || 'School'}</h2>
          ...
          <div>
            ...
            >
              {(LocalERPDatabase.getAcademicSetup()?.academicYears || []).map((year: any) => (
                <option key={year.id || year.year} value={year.year}>{year.year}</option>
              ))}
            </select>
          </div>
      ...
      {activePattern && activeTemplate ? (
        <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="print-only">
            <PrintLetterhead lang={activeTemplate.isRTL ? 'ur' : 'en'} subtitle={academicSetup?.schoolProfile?.schoolName || 'Academic Assessment Register'} />
