...
export default function SmartLeaveManager({ lang, user, onRefreshData, activeFeatureId = null, focusedMode = false, focusedTitle = 'Leave Management' }: SmartLeaveManagerProps) {
  ...
  const [newHolidayAcademicYear, setNewHolidayAcademicYear] = useState('');
  const [holidayReminderEnabled, setHolidayReminderEnabled] = useState(true);
  const [holidayChannels, setHolidayChannels] = useState({ website: true, email: true, sms: false, whatsapp: true });
  ...
  useEffect(() => {
    ...
    holidays.filter(holiday => holiday.reminderEnabled !== false && holiday.startDate === tomorrowKey).forEach(holiday => {
      ...
      const channels = holiday.notificationChannels || { website: true, email: true, sms: false, whatsapp: true };
  ...
  const configuredAcademicYears = useMemo(() => {
    const setup = LocalERPDatabase.getAcademicSetup();
    return Array.from(new Set((setup?.academicYears || []).map((item: any) => String(item?.year || item?.yearCode || '').trim()).filter(Boolean)));
  }, []);
  const activeAcademicYear = configuredAcademicYears.find((year: string) => {
    const setup = LocalERPDatabase.getAcademicSetup();
    return (setup?.academicYears || []).some((item: any) => String(item?.year || item?.yearCode || '').trim() === year && item?.isActive);
  }) || configuredAcademicYears[0] || '';

  useEffect(() => {
    if (!newHolidayAcademicYear && activeAcademicYear) setNewHolidayAcademicYear(activeAcademicYear);
    if (!reportYear && activeAcademicYear) setReportYear(activeAcademicYear.slice(0, 4));
  }, [activeAcademicYear]);
  ...
  const handleAddHoliday = async (e: React.FormEvent) => {
    ...
    setHolidayChannels({ website: true, email: true, sms: false, whatsapp: true });
  };
  ...
  const handleEditHoliday = (h: Holiday) => {
    ...
    setHolidayChannels(h.notificationChannels || { website: true, email: true, sms: false, whatsapp: true });
  };
  ...
  const syncApprovedLeaveToAttendance = (leave: LeaveApplication) => {
    ...
    const activeYear = currentSetup.academicYears.find((y: any) => y.isActive)?.year || currentSetup.academicYears[0]?.year || '';
  ...
  const handleExportToExcel = () => {
    ...
    link.setAttribute("download", `CLST_Leave_Report_${reportType}_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <div className="bg-slate-900 text-white rounded-2xl p-6 shadow-md flex flex-col md:flex-row justify-between items-start md:items-center gap-4 no-print">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <span className="text-slate-400 font-mono text-xs">School</span>
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white rounded-xl border border-slate-200 p-4 shadow-sm gap-3 no-print">
            ...
            <div className="flex items-center gap-3 text-xs">
              <span className="text-slate-400 font-bold">
                Academic Year: <span className="text-indigo-600 font-mono font-extrabold">{activeAcademicYear || 'Not configured'}</span>
              </span>
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="border-b border-slate-100 bg-slate-50/50 px-5 py-4 flex justify-between items-center">
                  ...
                  <span className="text-[9px] font-mono font-bold bg-indigo-50 text-indigo-600 px-2 py-0.5 rounded uppercase">
                    Academic Year: {activeAcademicYear || 'Not configured'}
                  </span>
                </div>
                    ...
                    <div className="rounded-xl border border-indigo-100 bg-indigo-50/60 p-3 space-y-2">
                      ...
                      {holidayReminderEnabled && (
                        <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-slate-700">
                          {(['website', 'email', 'whatsapp'] as const).map(channel => (
                      ...
                      <p className="text-[9px] text-slate-500">ERP dashboard reminders work automatically. Email and WhatsApp use configured server providers. SIM SMS is reserved for OTP/security/critical emergencies and is not used for routine holiday reminders.</p>
                    </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm flex flex-wrap gap-4 items-end text-xs no-print">
            ...
            {reportType === 'yearly' && (
              <div>
                ...
                >
                  {configuredAcademicYears.map((year) => (
                    <option key={year.slice(0, 4)} value={year.slice(0, 4)}>{year}{year === activeAcademicYear ? ' (Active)' : ''}</option>
                  ))}
                </select>
              </div>
            )}
          ...
          <div id="leave-register-print" className={`bg-white rounded-2xl border border-slate-200 shadow-md p-8 text-xs font-sans space-y-6 print-container`}>
            ...
            <div className="border-b border-slate-200 pb-4 text-center">
              ...
              <p className="text-[9px] text-slate-400 font-mono mt-0.5">
                Generated Date/Time: {new Date().toLocaleString('en-US', { timeZone: 'Asia/Kolkata' })} | Active Academic Year: {activeAcademicYear || 'Not configured'}
              </p>
            </div>
