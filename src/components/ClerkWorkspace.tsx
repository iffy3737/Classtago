...
import ClerkAttendanceReportsRegisters from './ClerkAttendanceReportsRegisters';
...
export default function ClerkWorkspace({ lang, user, classes, onRefreshData, initialTab = 'dashboard', hideAdmissionNavigation = false, focusedMode = false, focusedTitle, activeFeatureId = null, onOpenAdmissionDesk, onBackToOverview }: ClerkWorkspaceProps) {
  const activeYear = LocalERPDatabase.getAcademicSetup()?.academicYears?.find(y => y.isActive)?.year || '';
  ...
  const downloadBulkAdmissionTemplate = () => {
    try {
      ...
      anchor.href = url;
      anchor.download = 'Classtago_Bulk_Admission_Template_R4_9_6.xlsx';
  ...
  const handleBulkAdmissionWorkbook = async (file: File | null) => {
    ...
    try {
      if (!/\.(xlsx|xls)$/i.test(file.name)) throw new Error('Choose the Classtago .xlsx admission template.');
  ...
  const handleAdmissionDocumentFile = async (document: any, file: File | null) => {
    ...
    const currentTotal = Object.entries(admissionDocumentFiles)
      ...
      .reduce((sum, [, item]) => sum + Number((item as any)?.size || 0), 0);
  ...
  const handleExecuteAdmission = async (e?: React.FormEvent) => {
    ...
    // In Classtago, a class that has no division is a valid academic scope. The UI label is
    ...
    <div className="space-y-6">
      ...
      {clerkTab === 'dashboard' && (
        <div className="space-y-6 text-left">
          <RoleHeroCard
            name={user.name}
            role="clerk"
            workspaceLabel="Clerk Workspace"
            subtitle="Production metrics from the school cloud. Demo admissions are never seeded here."
            stats={[
              { value: dashboardLoading && !cloudDashboard ? '\u2026' : cloudDashboard?.kpis.admittedStudents == null ? '\u2014' : cloudDashboard.kpis.admittedStudents, label: 'Students' },
              { value: dashboardLoading && !cloudDashboard ? '\u2026' : cloudDashboard?.kpis.admissionQueue == null ? '\u2014' : cloudDashboard.kpis.admissionQueue, label: 'Admissions' },
              { value: dashboardLoading && !cloudDashboard ? '\u2026' : cloudDashboard?.kpis.feeClearanceRatio == null ? '\u2014' : `${cloudDashboard.kpis.feeClearanceRatio}%`, label: 'Fee Clearance' },
              { value: dashboardLoading && !cloudDashboard ? '\u2026' : cloudDashboard?.kpis.readyForHeadmaster == null ? '\u2014' : cloudDashboard.kpis.readyForHeadmaster, label: 'Ready for HM' },
            ]}
                ...
                <div className="bg-slate-50 border border-slate-100 rounded-xl p-4 text-[11px] space-y-2 max-h-[350px] overflow-y-auto">
                  <div id={`print-gr-card-${selectedCertStudent.grNumber}`} className="bg-white p-4 shadow-sm border border-slate-200/50 rounded">
                    ...
                    <div className="text-center border-b border-slate-300 pb-2 mb-3">
                      <h2 className="font-extrabold text-slate-800 text-xs tracking-tight uppercase">{LocalERPDatabase.getAcademicSetup()?.schoolProfile?.schoolName || 'School'}</h2>
                      <p className="text-[9px] text-slate-500">{LocalERPDatabase.getAcademicSetup()?.schoolProfile?.address || 'School address not configured'}</p>
      ...
      {clerkTab === 'attendance' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
          <ClerkAttendanceReportsRegisters lang={lang} user={user} activeFeatureId={activeFeatureId} />
        </div>
      )}
      ...
      {clerkTab === 'profile' && (
        <div className="max-w-md mx-auto bg-white rounded-2xl border border-slate-200 p-6 space-y-6 text-left">
          ...
          <div className="space-y-2 text-xs">
            ...
            <p><strong>Mobile Number:</strong> {user.phone || 'Not configured'}</p>
