...
export default function Phase6AppShield({userId}:{userId:string}){
  ...
  return <div className="fixed inset-0 z-[10000] grid place-items-center bg-slate-950/95 p-5 backdrop-blur-xl" role="dialog" aria-modal="true"><div className="w-full max-w-sm rounded-[2rem] border border-white/10 bg-white p-6 text-center shadow-2xl"><div className="mx-auto grid h-16 w-16 place-items-center rounded-2xl bg-emerald-50 text-emerald-700"><Fingerprint className="h-8 w-8"/></div><p className="mt-5 text-[10px] font-black uppercase tracking-[.18em] text-emerald-700">Classtago App Shield</p><h2 className="mt-1 text-2xl font-black text-slate-950">Device verification required</h2><p className="mt-2 text-xs leading-5 text-slate-500">The app was locked after being away. Verify with fingerprint, face or your device credential to continue.</p>{error&&<div className="mt-4 rounded-xl border border-rose-200 bg-rose-50 p-3 text-xs font-bold text-rose-800">{error}</div>}<button disabled={busy} onClick={()=>void unlock()} className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 py-3 text-xs font-black text-white disabled:opacity-50">{busy?<Loader2 className="h-4 w-4 animate-spin"/>:<ShieldCheck className="h-4 w-4"/>}Unlock Classtago</button></div></div>;
}
