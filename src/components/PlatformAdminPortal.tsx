...
import PlatformAcademicCore from './PlatformAcademicCore';
import PlatformSmsGatewayControlPanel from './PlatformSmsGatewayControlPanel';

type PlatformTab = 'dashboard' | 'schools' | 'leads' | 'modules' | 'plans' | 'subscriptions' | 'academic' | 'sms';
...
const COPY: Record<Language, Record<string, string>> = {
  en: {
    ...
    dashboard: 'Dashboard', schools: 'Schools', modules: 'Modules', plans: 'Plans', subscriptions: 'Subscriptions', academicCore: 'Academic Core',
  ...
  hi: {
    ...
    dashboard: 'डैशबोर्ड', schools: 'स्कूल', modules: 'मॉड्यूल', plans: 'प्लान', subscriptions: 'सब्सक्रिप्शन', academicCore: 'अकादमिक कोर',
  ...
  ur: {
    ...
    dashboard: 'ڈیش بورڈ', schools: 'اسکول', modules: 'ماڈیول', plans: 'پلان', subscriptions: 'سبسکرپشن', academicCore: 'اکیڈمک کور',
...
function dateTimeLocalInput(value?: string | null): string {
  if (!value) return '';
  const date = new Date(value);
...
export default function PlatformAdminPortal({ lang, user }: PlatformAdminPortalProps) {
  ...
  const [demoLeads, setDemoLeads] = useState<LeadRow[]>([]);
  ...
  const adminLevel = String(overview?.admin?.adminLevel || '').toLowerCase();
  const canManagePlans = adminLevel === 'super_admin' || adminLevel === 'billing_admin';
  const canManageSubscriptions = canManagePlans;
  const canManageLeads = adminLevel === 'super_admin' || adminLevel === 'support_admin';
  ...
  const loadTab = useCallback(async (targetTab: PlatformTab, forceOverview = false) => {
    ...
    try {
      ...
      if (targetTab === 'leads') setDemoLeads((await platformRequest<{ leads: LeadRow[] }>('/api/platform/institution-leads?kind=demo')).leads);
  ...
  const filteredDemoLeads = useMemo(() => filterRows<LeadRow>(demoLeads, query, (row: LeadRow) => `${row.institutionName} ${row.contactName} ${row.designation || ''} ${row.mobile} ${row.email} ${row.referenceCode} ${row.status} ${row.city || ''} ${row.state || ''}`), [demoLeads, query]);
  ...
  const tabs: Array<{ key: PlatformTab; label: string; icon: React.ElementType }> = [
    ...
    { key: 'leads', label: 'Demo Follow-up', icon: PhoneCall },
    ...
    { key: 'academic', label: c.academicCore || 'Academic Core', icon: BookOpenCheck },
    { key: 'sms', label: 'SMS / OTP Gateway Setup', icon: KeyRound }
  ];

  const openLeadFollowUp = (lead: LeadRow) => {
    setError(''); setSuccess('');
    setSelectedLead(lead);
    setLeadStatus(['new','contacted','follow_up','qualified','converted','closed'].includes(lead.status) ? lead.status : 'new');
    setLeadNote(lead.lastFollowUpNote || '');
    setLeadNextFollowUp(dateTimeLocalInput(lead.nextFollowUpAt));
  ...
  const saveLeadFollowUp = async () => {
    ...
    try {
      await platformRequest(`/api/platform/institution-leads/${selectedLead.id}/follow-up`, {
        method: 'POST',
        body: JSON.stringify({
          status: leadStatus,
          note: leadNote.trim(),
          nextFollowUpAt: leadNextFollowUp ? new Date(leadNextFollowUp).toISOString() : null
        })
      });
      setLeadEditorOpen(false);
      setSuccess('Demo follow-up updated successfully.');
      await loadTab('leads', true);
    } catch (requestError: any) {
      setError(requestError?.message || 'Demo follow-up could not be saved.');
    } finally { setSaving(false); }
  };

  const deleteLead = async (lead: LeadRow) => {
    const confirmed = window.confirm(`Delete ${lead.institutionName} / ${lead.contactName} from the Demo Follow-up list? This removes the saved lead record permanently.`);
    if (!confirmed) return;
    setSaving(true); setError(''); setSuccess('');
    try {
      await platformRequest(`/api/platform/institution-leads/${lead.id}`, { method: 'DELETE' });
      if (selectedLead?.id === lead.id) setLeadEditorOpen(false);
      setSuccess('Demo lead deleted.');
      await loadTab('leads', true);
    } catch (requestError: any) {
      setError(requestError?.message || 'Demo lead could not be deleted.');
    } finally { setSaving(false); }
  };
    ...
    <div className="space-y-6">
      ...
      {loading && !overview ? <LoadingPanel /> : (
        <>
          ...
          {tab === 'academic' && <PlatformAcademicCore />}
          {tab === 'sms' && <PlatformSmsGatewayControlPanel />}
          {tab !== 'dashboard' && tab !== 'academic' && tab !== 'sms' && (
            <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
              <div className="flex flex-col gap-3 border-b border-slate-200 p-4 lg:flex-row lg:items-center lg:justify-between lg:px-6">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-500">{tab === 'leads' ? 'Every OTP-verified live-demo contact appears here for follow-up, qualification and cleanup after completion.' : 'Live Platform Core data with controlled Super Admin actions.'}</p>
                </div>
                <div className="flex flex-col gap-2 sm:flex-row">
                  {(tab === 'plans' && canManagePlans) && <ActionButton onClick={() => void openNewPlan()} icon={Plus} label="Create Plan" />}
                  {(tab === 'subscriptions' && canManageSubscriptions) && <ActionButton onClick={() => void openNewSubscription()} icon={Plus} label="Assign Plan" disabled={publishedPlans.length === 0} />}
              ...
              {loading ? <div className="flex min-h-56 items-center justify-center"><Loader2 className="h-7 w-7 animate-spin text-blue-600" /></div> : (
                <div className="overflow-x-auto">
                  {tab === 'schools' && <SchoolsTable rows={filteredSchools} noData={c.noData} onAssign={openNewSubscription} onAccess={openAccessSummary} canAssign={canManageSubscriptions && publishedPlans.length > 0} />}
                  {tab === 'leads' && <DemoLeadsTable rows={filteredDemoLeads} noData={c.noData} onFollowUp={openLeadFollowUp} onDelete={deleteLead} saving={saving} />}
      ...
      {leadEditorOpen && selectedLead && (
        <Modal title="Demo Follow-up" subtitle="Keep the latest contact result, next follow-up time and internal note. Delete the record only after the work is complete and the lead is no longer required." onClose={() => !saving && setLeadEditorOpen(false)}>
          <div className="space-y-5">
            <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
              <Metric label="Institution" value={selectedLead.institutionName} />
              <Metric label={selectedLead.leadSourceKind === 'live_demo' ? 'Demo Role' : 'Contact'} value={selectedLead.leadSourceKind === 'live_demo' ? (selectedLead.contactName || selectedLead.designation || 'Demo User') : selectedLead.contactName} />
              <Metric label="Mobile / WhatsApp" value={selectedLead.mobile} />
              <Metric label="Email" value={selectedLead.email || 'Optional · not provided'} />
              <Metric label="Reference" value={selectedLead.referenceCode} />
              <Metric label="Demo requested" value={formatDate(selectedLead.createdAt)} />
            </div>
            <div className="grid gap-4 md:grid-cols-2">
              <Field label="Follow-up status"><select value={leadStatus} onChange={event => setLeadStatus(event.target.value)} className={inputClass}><option value="new">New</option><option value="contacted">Contacted</option><option value="follow_up">Follow-up needed</option><option value="qualified">Qualified</option><option value="converted">Converted</option><option value="closed">Closed</option></select></Field>
              <Field label="Next follow-up"><input type="datetime-local" value={leadNextFollowUp} disabled={leadStatus === 'converted' || leadStatus === 'closed'} onChange={event => setLeadNextFollowUp(event.target.value)} className={inputClass} /></Field>
            </div>
            <Field label="Follow-up note"><textarea rows={5} value={leadNote} onChange={event => setLeadNote(event.target.value)} className={`${inputClass} min-h-28`} placeholder="Example: Demo completed; call management on Monday regarding plan selection." /></Field>
            {selectedLead.message && <div className="rounded-2xl border border-cyan-200 bg-cyan-50 p-4 text-xs leading-6 text-cyan-900"><strong>Original demo/request note:</strong> {selectedLead.message}</div>}
            {selectedLead.requestedModules?.length > 0 && <div><div className="text-[10px] font-extrabold uppercase tracking-wider text-slate-500">Interested modules</div><div className="mt-2 flex flex-wrap gap-2">{selectedLead.requestedModules.map(module => <span key={module} className="rounded-full bg-blue-50 px-2.5 py-1 text-[10px] font-bold text-blue-700">{module}</span>)}</div></div>}
            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-between"><button type="button" onClick={() => void deleteLead(selectedLead)} disabled={saving || !canManageLeads} className="inline-flex items-center justify-center gap-2 rounded-xl border border-rose-200 bg-rose-50 px-4 py-2.5 text-xs font-extrabold text-rose-700 hover:bg-rose-100 disabled:opacity-50"><Trash2 className="h-4 w-4" />Delete completed lead</button><div className="flex flex-col-reverse gap-3 sm:flex-row"><button type="button" onClick={() => setLeadEditorOpen(false)} disabled={saving} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void saveLeadFollowUp()} disabled={saving || !canManageLeads} className={primaryButton}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save Follow-up</button></div></div>
          </div>
        </Modal>
      )}
      ...
      {planEditorOpen && (
        <Modal title={planForm.id ? 'Edit Plan' : 'Create Plan'} subtitle="Main modules are bundled through plans. Platform Core modules and required dependencies are included automatically." onClose={() => !saving && setPlanEditorOpen(false)}>
          <div className="space-y-6">
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end"><button type="button" onClick={() => setPlanEditorOpen(false)} disabled={saving} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void savePlan()} disabled={saving || !canManagePlans} className={primaryButton}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save Plan</button></div>
          </div>
        </Modal>
      )}
      ...
      {subscriptionEditorOpen && (
        <Modal title={subscriptionForm.id ? 'Edit School Subscription' : 'Assign Plan to School'} subtitle="A current subscription activates its plan modules automatically. Assigning a new current plan safely cancels the older current subscription." onClose={() => !saving && setSubscriptionEditorOpen(false)}>
          <div className="space-y-6">
            ...
            <div className="flex flex-col-reverse gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end"><button type="button" onClick={() => setSubscriptionEditorOpen(false)} disabled={saving} className={secondaryButton}>Cancel</button><button type="button" onClick={() => void saveSubscription()} disabled={saving || !canManageSubscriptions || publishedPlans.length === 0} className={primaryButton}>{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Check className="h-4 w-4" />}Save Subscription</button></div>
          </div>
        </Modal>
      )}
                ...
                <section className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {accessSummary.subscriptionContext.subscription.status !== 'active' && <button type="button" disabled={saving || !canManageSubscriptions} onClick={() => void changeSubscriptionStatus(accessSummary.subscriptionContext!.subscription!.id, 'active')} className={primaryButton}><PlayCircle className="h-4 w-4" />Reactivate</button>}
                      {accessSummary.subscriptionContext.subscription.status !== 'past_due' && <button type="button" disabled={saving || !canManageSubscriptions} onClick={() => void changeSubscriptionStatus(accessSummary.subscriptionContext!.subscription!.id, 'past_due')} className={secondaryButton}>Past due</button>}
                      {accessSummary.subscriptionContext.subscription.status !== 'suspended' && <button type="button" disabled={saving || !canManageSubscriptions} onClick={() => void changeSubscriptionStatus(accessSummary.subscriptionContext!.subscription!.id, 'suspended')} className={secondaryButton}><PauseCircle className="h-4 w-4" />Suspend</button>}
                      {accessSummary.subscriptionContext.subscription.status !== 'expired' && <button type="button" disabled={saving || !canManageSubscriptions} onClick={() => void changeSubscriptionStatus(accessSummary.subscriptionContext!.subscription!.id, 'expired')} className={secondaryButton}>Mark expired</button>}
                      {accessSummary.subscriptionContext.subscription.status !== 'cancelled' && <button type="button" disabled={saving || !canManageSubscriptions} onClick={() => void changeSubscriptionStatus(accessSummary.subscriptionContext!.subscription!.id, 'cancelled')} className={secondaryButton}>Cancel</button>}
                    </div>
                  </div>
  ...
  return <section className="edx-platform-growth" data-edx-surface="platform-growth-r257">
    <div className="edx-platform-growth-hero">
      <div className="edx-platform-growth-copy">
        <div className="edx-platform-growth-chip"><Crown className="h-4 w-4" /> Classtago PLATFORM BUSINESS CENTER</div>
        <h3>Brand, subscriptions and growth — arranged like a premium platform.</h3>
        <p>Manage the Classtago school network, published plans, subscriptions and promotional surfaces from one command view. Operational numbers below are live Platform Core records.</p>
    ...
    <div className="edx-platform-campaign-head">
      <div><span>Classtago ADS & BANNERS</span><h4>Premium platform campaign surfaces</h4></div>
      <p>Brand presentation only — no fake school or subscription data.</p>
    </div>
    <div className="edx-platform-campaign-grid">
      <article className="edx-platform-campaign edx-platform-campaign-violet"><div className="edx-platform-campaign-icon"><Megaphone className="h-5 w-5" /></div><span>Multi-school cloud</span><h5>One Classtago. Every school.</h5><p>Independent school workspaces, one platform identity and centrally governed access.</p></article>
      <article className="edx-platform-campaign edx-platform-campaign-cyan"><div className="edx-platform-campaign-icon"><WandSparkles className="h-5 w-5" /></div><span>Classtago AI</span><h5>Academic intelligence, built in.</h5><p>Teaching workflows, planning, homework and question-paper intelligence inside the school workspace.</p></article>
    ...
    <div className="edx-platform-plan-head"><div><span>LIVE PUBLISHED PLANS</span><h4>Plans currently available in Classtago</h4></div><button type="button" onClick={onOpenPlans}>Manage all plans <ArrowRight className="h-4 w-4" /></button></div>
    <div className="edx-platform-plan-grid">
      {livePlans.length ? livePlans.map((plan, index) => <article key={plan.id} className={`edx-platform-plan-card ${index === 0 ? 'edx-platform-plan-featured' : ''}`}>
        ...
        <h5>{plan.planName}</h5>
        <p>{plan.description || 'Classtago platform subscription plan.'}</p>
...
function DemoLeadsTable({ rows, noData, onFollowUp, onDelete, saving }: { rows: LeadRow[]; noData: string; onFollowUp: (lead: LeadRow) => void; onDelete: (lead: LeadRow) => Promise<void>; saving: boolean }) {
  return <table className="min-w-full text-left text-sm"><TableHead columns={['Demo contact', 'Institution', 'Demo date', 'Status', 'Follow-up', 'Action']} /><tbody className="divide-y divide-slate-100">{rows.map(row => <tr key={row.id} className="hover:bg-slate-50/70"><td className="px-5 py-4"><p className="font-extrabold text-slate-900">{row.leadSourceKind === 'live_demo' ? (row.contactName || row.designation || 'Demo User') : row.contactName}</p><p className="mt-1 text-[11px] font-semibold text-slate-600">{row.mobile}</p><p className="text-[10px] text-slate-400">{row.email || 'Email not provided'}</p></td><td className="px-5 py-4"><p className="font-extrabold text-slate-900">{row.institutionName}</p><p className="mt-1 text-[10px] text-slate-500">{[row.city,row.state].filter(Boolean).join(', ') || row.institutionType}</p><p className="mt-1 font-mono text-[9px] text-slate-400">{row.referenceCode}</p></td><td className="px-5 py-4 text-xs font-semibold text-slate-600">{formatDate(row.createdAt)}<div className="mt-1 text-[9px] font-black uppercase tracking-wide text-cyan-700">{row.leadSourceKind === 'demo_setup' ? 'Setup after demo' : 'Live demo'}</div></td><td className="px-5 py-4"><StatusBadge value={row.status || 'new'} /></td><td className="px-5 py-4"><div className="max-w-xs"><p className="line-clamp-2 text-[11px] font-semibold text-slate-600">{row.lastFollowUpNote || 'No follow-up recorded yet.'}</p>{row.nextFollowUpAt && <p className="mt-1 flex items-center gap-1 text-[9px] font-bold text-amber-700"><Clock3 className="h-3 w-3" />Next: {new Date(row.nextFollowUpAt).toLocaleString('en-IN')}</p>}</div></td><td className="px-5 py-4"><div className="flex flex-wrap gap-2"><button type="button" onClick={() => onFollowUp(row)} className={smallButton}><PhoneCall className="h-3.5 w-3.5" />Follow-up</button><button type="button" disabled={saving} onClick={() => void onDelete(row)} className="inline-flex min-h-9 items-center justify-center gap-1.5 rounded-lg border border-rose-200 bg-rose-50 px-3 py-1.5 text-[11px] font-extrabold text-rose-700 hover:bg-rose-100 disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" />Delete</button></div></td></tr>)}{rows.length === 0 && <EmptyRow columns={6} message={noData} />}</tbody></table>;
}
