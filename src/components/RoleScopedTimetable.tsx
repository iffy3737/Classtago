...
export default function RoleScopedTimetable({ user, embedded = false }: RoleScopedTimetableProps) {
  ...
  useEffect(() => {
    const shouldUseCloud = user.role === 'teacher' || user.role === 'class_teacher' || user.role === 'student' || user.role === 'parent';
    if (!shouldUseCloud) { setCloudRows(null); setCloudError(''); return; }
    let cancelled = false;
    setCloudError('');
    const query = user.role === 'parent' && selectedChild?.id ? `?studentId=${encodeURIComponent(selectedChild.id)}` : '';
    supabase.auth.getSession().then(async ({ data }) => {
      const token = data.session?.access_token || '';
      if (!token) throw new Error('Secure session unavailable.');
      return fetch(`/api/role/timetable${query}`, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store' });
    }).then(async response => { const payload = await response.json().catch(() => ({})); if (!response.ok) throw new Error(payload?.error || 'Timetable could not be loaded.'); return payload; })
      .then(payload => { if (!cancelled) setCloudRows(Array.isArray(payload?.rows) ? payload.rows : []); })
      .catch(error => { if (!cancelled) { setCloudRows([]); setCloudError(error?.message || 'Timetable could not be loaded.'); } });
    return () => { cancelled = true; };
  }, [user.role, user.id, selectedChild?.id]);
  ...
  }, [timetable, user, selectedChild, cloudRows]);
  ...
  return (
    <section className={`${embedded ? '' : 'rounded-[1.75rem] border border-slate-200 bg-white p-5 shadow-sm sm:p-7'} text-left`}>
      ...
      {cloudError && <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs font-semibold text-amber-900">{cloudError}</div>}
