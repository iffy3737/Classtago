...
import { publishMariaDynamicCommand, requestMariaNavigation, type MariaDynamicClientCommand } from '../lib/mariaClientBridge';
import { isPhase2NativeMlAvailable, requestNativeMicrophonePermission } from '../lib/phase2NativeMl';
...
export default function RealtimeVoiceAssistant({ lang, publicSchoolSlug, publicSchoolName }: RealtimeVoiceAssistantProps) {
  ...
  const startMic = async () => {
    ...
    if (!navigator.mediaDevices?.getUserMedia) {
      throw new Error('Microphone access is unavailable in this browser/preview. Open Classtago in a normal HTTPS browser or the Classtago Android app.');
    }
    // Capacitor Android can have RECORD_AUDIO denied at the OS layer even when
    // the WebView is otherwise ready. Ask the native permission bridge first so
    // getUserMedia receives the correct WebView grant instead of a generic
    // NotAllowedError/Permission denied result. Web builds keep the normal
    // browser permission prompt.
    if (isPhase2NativeMlAvailable()) {
      const permission = await requestNativeMicrophonePermission();
      if (!permission?.granted) {
        throw new Error('Microphone permission is denied for Classtago. Open Android Settings → Apps → Classtago → Permissions → Microphone and allow it, then start Maria again.');
      }
    }
    const context = await ensureAudio();
    ...
    try {
      ...
      if (name === 'NotAllowedError' || name === 'PermissionDeniedError' || message.includes('permission denied') || message.includes('not allowed')) {
        throw new Error(isPhase2NativeMlAvailable()
          ? 'Microphone permission is blocked for Classtago. Open Android Settings → Apps → Classtago → Permissions → Microphone, allow access, then start Maria again.'
          : 'Microphone permission is blocked. Allow microphone access for the current Classtago/AI Studio site in browser site settings, then start Maria again.');
      }
      if (name === 'SecurityError') {
        throw new Error('Microphone access is blocked by the current preview/browser security policy. Open Classtago from its HTTPS app URL or the Classtago Android app and start Maria there.');
      }
      throw error;
    }
  ...
  const handleClientAction = (action: any) => {
    ...
    if (action?.type === 'navigate' && action?.moduleId) {
      const moduleId = String(action.moduleId);
      const featureId = action.featureId ? String(action.featureId) : undefined;
      // Voice navigation is an instruction to open the authorised module, not a
      // proposal that requires a second tap. DashboardOverview remains the
      // authority: it resolves the canonical module and ignores anything the
      // signed-in role is not entitled to open.
      requestMariaNavigation(moduleId, featureId);
      setClientAction(null);
      setActionStatus(String(action.statusMessage || `Maria opened ${String(action.label || 'the requested Classtago module')}.`));
      return;
    }
  ...
  const runClientAction = async (confirmationMode: 'confirm' | 'cancel' = 'confirm') => {
    ...
    if (action.type === 'navigate') {
      requestMariaNavigation(String(action.moduleId), action.featureId ? String(action.featureId) : undefined);
      return;
    }
