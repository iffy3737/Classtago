...
export default function ContinuousResultBook({
  ...
  const configuredAcademicYear = (() => {
    try {
      ...
      return setup?.academicYears?.find((y: any) => y.isActive)?.year || setup?.academicYears?.[0]?.year || '';
    } catch {
      return '';
    }
  })();
  ...
  useEffect(() => {
    try {
      ...
      const match = locks.find((l: any) => l.className === ctTargetClass && l.divisionName === ctTargetDivision && l.academicYear === configuredAcademicYear);
  ...
  const getCompletionStatus = () => {
    try {
      ...
      const total = classAllocations.length;
      ...
      console.error(e);
      return { approved: 0, total: 0, isComplete: false };
    }
  };
  ...
  const handleSendCompleteResultBook = () => {
    try {
      const locks = JSON.parse(localStorage.getItem('nhs_erp_result_book_locks') || '[]');
      const existingIdx = locks.findIndex((l: any) => l.className === ctTargetClass && l.divisionName === ctTargetDivision && l.academicYear === configuredAcademicYear);
      
      const lockObj = {
        id: `lock_rb_${configuredAcademicYear}_${ctTargetClass.replace(/\s+/g, '_')}_${ctTargetDivision.replace(/\s+/g, '_')}`,
      ...
      const classStudentBooks = allStudentBooks.filter((rb: any) => 
        ...
        rb.academicYear === configuredAcademicYear
      );
      ...
      const newPrintableRB = {
        id: `prb_${configuredAcademicYear}_${ctTargetClass.replace(/\s+/g, '_')}_${ctTargetDivision.replace(/\s+/g, '_')}`,
        academicYear: configuredAcademicYear,
        className: ctTargetClass,
        divisionName: ctTargetDivision,
  ...
  const generateDefaultTemplate = () => {
    ...
    setCell(2, 0, `Academic Year: ${configuredAcademicYear} | Permanent Master Student Registry Template`, 's', undefined, false, 'center', '475569', 9);
    ...
    try {
      ...
      if (storedStr) {
        ...
        const match = studentBooks.find(
          (rb: any) => rb.studentId === student.id && rb.academicYear === configuredAcademicYear
        );
