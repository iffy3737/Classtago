...
export default function ParentPortalWorkspace({ user, module, feature, onNavigate }: Props) {
  ...
  return <section className="edx-parent-portal edx-theme-safe overflow-hidden rounded-[2rem] border border-slate-200 bg-slate-50 shadow-xl">{header}
      <div className="p-5 sm:p-7 pt-5">
        <RoleHeroCard
          name={user.name}
          role="parent"
          workspaceLabel="Parent Workspace"
          subtitle={profile ? `Viewing ${profile.fullName} · ${profile.className}` : 'Verified child access.'}
          stats={[
            { value: snapshot?.attendance?.rate == null ? '—' : `${snapshot.attendance.rate}%`, label: 'Attendance' },
            { value: currentHomework.length, label: 'Homework' },
            { value: snapshot?.results?.rows?.length || 0, label: 'Results' },
          ]}
          ctaLabel="Open Child Attendance"
          onCtaClick={() => onNavigate?.('pa-attendance')}
        />

      <ModulesPreviewCard
        title="Modules"
        items={[
          { id: 'attendance', label: 'Child Attendance', subtitle: 'Daily attendance · Monthly view', icon: MI_Cal, gradient: 'linear-gradient(135deg, #FF3473, #D91B5C)', route: 'pa-attendance' },
          { id: 'homework', label: 'Child Homework', subtitle: 'Assigned work · Study material', icon: MI_Book, gradient: 'linear-gradient(135deg, #FFA202, #E08800)', route: 'pa-homework-material' },
          { id: 'results', label: 'Child Results', subtitle: 'Marks · Progress cards', icon: MI_Bar, gradient: 'linear-gradient(135deg, #8B5CF6, #6D28D9)', route: 'pa-results-progress' },
          { id: 'fees', label: 'Child Fees & Receipts', subtitle: 'Ledger · Payment history', icon: MI_Fee, gradient: 'linear-gradient(135deg, #14B8A6, #0F766E)', route: 'pa-fees' }
        ]}
      ...
      <div className="px-5 sm:px-7 pb-5">{mariaStatus&&<div className="mb-5 rounded-2xl border border-violet-200 bg-violet-50 p-4 text-xs font-bold leading-5 text-violet-900"><b>Maria:</b> {mariaStatus}</div>}{error&&<div className="mb-5 rounded-2xl border border-rose-200 bg-rose-50 p-4 text-xs font-bold text-rose-800">{error}</div>}{message&&module.id!=='pa-leave'&&module.id!=='pa-certificate-requests'&&module.id!=='pa-profile'&&<div className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 text-xs font-bold">{message}</div>}{content}<div className="mt-7 flex items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-[11px] leading-6 text-slate-500"><ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-emerald-600"/><span>Privacy boundary: Parent access is derived only from Headmaster-approved, school-scoped Parent-child link records. Marks, attendance, Result Book, Teacher drafts, other students and administrative edit/approval controls remain inaccessible.</span><button onClick={()=>void refresh()} className="ml-auto shrink-0 rounded-lg border border-slate-200 p-2 text-slate-600" title="Refresh"><RefreshCw className="h-4 w-4"/></button></div></div></section>;
}
