...
export async function loadTeacherCloudContext(identity: { userId: string; appUserId?: string; employeeCode?: string; shalarthId?: string; name?: string }): Promise<TeacherCloudContext> {
  ...
  // Canonical assignment rows are authoritative, but defensive de-duplication prevents
  // duplicate teacher scopes when a migration/re-sync temporarily contains the same
  // Class + Division + Subject assignment more than once. Never collapse distinct subjects.
  const assignmentKeys = new Set<string>();
  rows = rows.filter((row:any) => {
    const key = [row.school_id, row.academic_year_id, row.class_id, row.division_id || '', row.subject_id, row.teacher_id].map(v => String(v || '')).join('|');
    if (assignmentKeys.has(key)) return false;
    assignmentKeys.add(key);
    return true;
  });
  const initialCloudSubjectRowCount = rows.length;
  const classTeacherRowsRaw = ((classTeacherRead.rows || []) as any[]).filter((row:any) => row.is_active !== false && row.active !== false);
  const classTeacherKeys = new Set<string>();
  const classTeacherRows = classTeacherRowsRaw.filter((row:any) => {
    const key = [row.school_id, row.academic_year_id, row.class_id, row.division_id || '', row.teacher_id].map(v => String(v || '')).join('|');
    if (classTeacherKeys.has(key)) return false;
    classTeacherKeys.add(key);
    return true;
  });
