...
import { subscribeMariaDynamicCommands, takePendingMariaDynamicCommands, type MariaMarkEntryCommand } from '../../lib/mariaClientBridge';
...
function Header({title,subtitle,badge,onBack}:{title:string;subtitle:string;badge:string;onBack?:()=>void}) {
  return <header className="cs-rm-header">
    <div className="cs-rm-header-top">
      <div className="min-w-0 flex-1">
        {onBack && <button onClick={onBack} className="cs-rm-back-btn"><ArrowLeft className="h-3.5 w-3.5"/>Back</button>}
        <p className="cs-rm-eyebrow">Classtago Result Management</p>
        <h1 className="cs-rm-title">{title}</h1>
        <p className="cs-rm-subtitle">{subtitle}</p>
      </div>
      <span className="cs-rm-badge">{badge}</span>
    </div>
  </header>;
}
...
function SubjectMarksPage({context,onNavigate}:{context:TeacherCloudContext;onNavigate?:Props['onNavigate']}) {
  ...
  // Maria is an optional voice-cursor layer only. The canonical Mark List rows,
  // validation, Save Draft and Send-to-Class-Teacher functions below remain unchanged.
  const markSheetRef=useRef<HTMLDivElement|null>(null);
  const [mariaMarkActive,setMariaMarkActive]=useState(false);
  const [mariaDirection,setMariaDirection]=useState<'vertical'|'horizontal'>('vertical');
  const [mariaAnchor,setMariaAnchor]=useState<MariaMarkCellAnchor|null>(null);
  const [mariaMarkStatus,setMariaMarkStatus]=useState('');
  const [pendingMariaOpen,setPendingMariaOpen]=useState<MariaMarkEntryCommand|null>(null);
  const mariaAnchorRef=useRef<MariaMarkCellAnchor|null>(null);
  const mariaDirectionRef=useRef<'vertical'|'horizontal'>('vertical');
  const mariaLastFilledRef=useRef<MariaMarkCellAnchor|null>(null);
  const pendingMariaControlsRef=useRef<MariaMarkEntryCommand[]>([]);
  const mariaCommandHandlerRef=useRef<(command:MariaMarkEntryCommand)=>void>(()=>undefined);
  ...
  const resetMariaMarkEntry=()=>{setMariaMarkActive(false);setMariaAnchor(null);mariaAnchorRef.current=null;setMariaMarkStatus('');setPendingMariaOpen(null);mariaLastFilledRef.current=null;pendingMariaControlsRef.current=[]};
  const resetSheet=()=>{resetMariaMarkEntry();setScopeId('');setTemplate(null);setRoster([]);setRows({});setList(null);setMessage('');setErr('')};
  ...
  const reload=async()=>{
    ...
    try{
      const [{template:t,mappingSource:m,backendReady:mapReady},students,ownedLists]=await Promise.all([resolveResultTemplate(scope),loadResultRoster(scope),loadOwnedSubjectLists(scope,context)]);
      setTemplate(t);setMappingSource(m);setRoster(students);setBackendReady(mapReady&&ownedLists.backendReady);const active=ownedLists.lists[0]||null;setList(active);
      ...
      await putPhase1Cache(localScope,'subject_marks',`${scope.id}:${term}`,{template:t,mappingSource:m,roster:students,rows:map,list:active,backendReady:mapReady&&ownedLists.backendReady,cachedAt:new Date().toISOString()});
    }catch(e:any){
  ...
  useEffect(()=>{const onSync=(event:any)=>{if(event?.detail?.kind==='result_subject_draft'&&event?.detail?.recordKey===cacheKey&&event?.detail?.ok){setMessage('Offline Mark List draft synced safely to Classtago cloud.');if(navigator.onLine)void reload();}};const onRealtime=(event:any)=>{if(event?.detail?.table==='edunixo_result_subject_lists'&&navigator.onLine&&scope)void reload();};window.addEventListener('edunixo_phase1_sync',onSync as EventListener);window.addEventListener('edunixo_realtime_change',onRealtime as EventListener);return()=>{window.removeEventListener('edunixo_phase1_sync',onSync as EventListener);window.removeEventListener('edunixo_realtime_change',onRealtime as EventListener);};},[cacheKey,scope?.id,term]);

  const normalizeMariaText=(value:any)=>String(value||'').normalize('NFKC').toLowerCase().replace(/[^a-z0-9\u0900-\u097f\u0600-\u06ff]+/g,' ').trim();
  const mariaInputs=():HTMLInputElement[]=>Array.from(markSheetRef.current?.querySelectorAll<HTMLInputElement>('input[data-maria-mark-cell="true"]:not(:disabled)')||[]) as HTMLInputElement[];
  const mariaAnchorFromInput=(input:HTMLInputElement):MariaMarkCellAnchor=>({
    cellKey:String(input.dataset.mariaCellKey||''),studentId:String(input.dataset.mariaStudentId||''),studentName:String(input.dataset.mariaStudentName||''),rollNumber:String(input.dataset.mariaRollNumber||''),markKey:String(input.dataset.mariaMarkKey||''),headLabel:String(input.dataset.mariaHeadLabel||''),maxMarks:input.dataset.mariaMaxMarks===''||input.dataset.mariaMaxMarks==null?undefined:Number(input.dataset.mariaMaxMarks),kind:String(input.dataset.mariaKind||'mark'),rowIndex:Number(input.dataset.mariaRowIndex||0),columnIndex:Number(input.dataset.mariaColumnIndex||0)
  });
  const setMariaCursor=(next:MariaMarkCellAnchor|null,focus=true)=>{
    mariaAnchorRef.current=next;setMariaAnchor(next);
    if(next&&focus){const input=mariaInputs().find(node=>node.dataset.mariaCellKey===next.cellKey);input?.focus({preventScroll:false});}
  };
  const setMariaDirectionSafe=(next:'vertical'|'horizontal')=>{mariaDirectionRef.current=next;setMariaDirection(next)};
  const nextMariaInput=(anchor:MariaMarkCellAnchor,direction:'vertical'|'horizontal',delta=1)=>{
    const candidates=mariaInputs().map(input=>({input,anchor:mariaAnchorFromInput(input)})).filter(item=>direction==='vertical'?item.anchor.markKey===anchor.markKey:item.anchor.rowIndex===anchor.rowIndex).sort((a,b)=>direction==='vertical'?(a.anchor.rowIndex-b.anchor.rowIndex||a.anchor.columnIndex-b.anchor.columnIndex):(a.anchor.columnIndex-b.anchor.columnIndex||a.anchor.rowIndex-b.anchor.rowIndex));
    const index=candidates.findIndex(item=>item.anchor.cellKey===anchor.cellKey);return index>=0?candidates[index+delta]?.input:null;
  };
  const normalizeMariaMark=(value:any)=>{const raw=String(value??'').trim();const key=normalizeMariaText(raw);if(!key)return'';if(['absent','ab','gair hazir','गैर हाजिर','अनुपस्थित'].includes(key))return'AB';if(['medical leave','ml'].includes(key))return'ML';if(['exempt','ex'].includes(key))return'EX';if(['withheld','wh'].includes(key))return'WH';if(['not applicable','na'].includes(key))return'NA';return raw.toUpperCase();};
  const validateMariaMark=(anchor:MariaMarkCellAnchor,value:any)=>{
    const clean=normalizeMariaMark(value);if(!clean)return{ok:false as const,message:'No mark was recognised.'};
    if(anchor.kind!=='grade'&&!specialCodes.includes(clean)&&Number.isNaN(Number(clean)))return{ok:false as const,message:`“${clean}” is not a valid mark or Classtago special code.`};
    if(anchor.maxMarks!=null&&!Number.isNaN(Number(clean))&&Number(clean)>anchor.maxMarks)return{ok:false as const,message:`${anchor.headLabel} maximum is ${anchor.maxMarks}. ${clean} was not entered; please correct it.`};
    return{ok:true as const,value:clean};
  };
  const startMariaAtInput=(input:HTMLInputElement|null,direction:'vertical'|'horizontal',status:string)=>{if(!input){setMariaMarkStatus('Maria could not find that editable Mark List cell. You can tap any editable cell and say “start from here”.');return false;}setMariaDirectionSafe(direction);setMariaMarkActive(true);const anchor=mariaAnchorFromInput(input);setMariaCursor(anchor);setMariaMarkStatus(status);return true;};
  const executeMariaMarkControl=(command:MariaMarkEntryCommand)=>{
    const inputs=mariaInputs();if(!inputs.length){pendingMariaControlsRef.current=[...pendingMariaControlsRef.current,command].slice(-12);setMariaMarkStatus('Maria is waiting for the existing Mark List to finish loading…');return;}
    if(locked){setMariaMarkActive(false);setMariaMarkStatus('This Mark List is locked by the existing Result workflow, so Maria cannot edit it.');return;}
    if(command.command==='start_vertical'){
      const wanted=normalizeMariaText(command.headLabel);const matches=inputs.filter(input=>{const label=normalizeMariaText(input.dataset.mariaHeadLabel);return wanted?label===wanted||label.includes(wanted)||wanted.includes(label):false;});
      const target=matches.sort((a,b)=>Number(a.dataset.mariaRowIndex||0)-Number(b.dataset.mariaRowIndex||0))[0]||null;
      startMariaAtInput(target,'vertical',`${command.headLabel||'Selected column'} ready. Speak marks in order; Maria will move top to bottom.`);return;
    }
    if(command.command==='start_horizontal'){
      const wantedName=normalizeMariaText(command.studentName);const wantedRoll=normalizeMariaText(command.rollNumber);let matches=inputs.filter(input=>wantedRoll&&normalizeMariaText(input.dataset.mariaRollNumber)===wantedRoll);
      if(!matches.length&&wantedName){const exact=inputs.filter(input=>normalizeMariaText(input.dataset.mariaStudentName)===wantedName);matches=exact.length?exact:inputs.filter(input=>normalizeMariaText(input.dataset.mariaStudentName).includes(wantedName));}
      const studentIds=[...new Set(matches.map(input=>input.dataset.mariaStudentId||''))].filter(Boolean);if(studentIds.length>1){setMariaMarkStatus('More than one Student matched that name. Please say the Roll Number.');return;}
      const target=matches.sort((a,b)=>Number(a.dataset.mariaColumnIndex||0)-Number(b.dataset.mariaColumnIndex||0))[0]||null;
      startMariaAtInput(target,'horizontal',`${command.studentName||`Roll ${command.rollNumber||''}`} ready. Speak marks in order; Maria will move sideways.`);return;
    }
    if(command.command==='start_here'){
      const anchor=mariaAnchorRef.current;const target=anchor?inputs.find(input=>input.dataset.mariaCellKey===anchor.cellKey)||null:null;const direction=command.direction||mariaDirectionRef.current;
      if(!anchor||!target){setMariaMarkStatus('Tap the exact editable Mark List cell first, then say “Maria, start from here”.');return;}
      startMariaAtInput(target,direction,`Starting from ${anchor.studentName} · ${anchor.headLabel}, moving ${direction==='vertical'?'down':'sideways'}.`);return;
    }
    if(command.command==='set_direction'){const direction=command.direction||mariaDirectionRef.current;setMariaDirectionSafe(direction);setMariaMarkStatus(`Maria will continue ${direction==='vertical'?'top to bottom':'sideways'}.`);return;}
    if(command.command==='pause'){setMariaMarkActive(false);setMariaMarkStatus('Maria voice mark entry paused. Manual Mark Entry remains available.');return;}
    if(command.command==='stop'){setMariaMarkActive(false);mariaLastFilledRef.current=null;setMariaMarkStatus('Maria voice mark entry stopped. Existing unsaved marks remain in the normal Mark List draft.');return;}
    if(command.command==='next'||command.command==='previous'){
      const anchor=mariaAnchorRef.current;if(!anchor){setMariaMarkStatus('Select a Mark List cell first.');return;}const target=nextMariaInput(anchor,mariaDirectionRef.current,command.command==='next'?1:-1);if(!target){setMariaMarkStatus('There is no further editable cell in that direction.');return;}setMariaMarkActive(true);setMariaCursor(mariaAnchorFromInput(target));setMariaMarkStatus(`Moved ${command.command==='next'?'forward':'back'} one cell.`);return;
    }
    if(command.command==='correct_previous'){
      const previous=mariaLastFilledRef.current;if(!previous){setMariaMarkStatus('No previous Maria-entered mark is available to correct.');return;}const checked=validateMariaMark(previous,command.value);if(!checked.ok){setMariaMarkStatus(checked.message);return;}setCell(previous.studentId,previous.markKey,checked.value,previous.maxMarks,previous.kind);setMariaMarkStatus(`Previous mark corrected to ${checked.value}.`);return;
    }
    if(command.command==='enter_values'){
      if(!mariaMarkActive){setMariaMarkStatus('Say which column/student to start, or tap a cell and say “start from here”, before dictating marks.');return;}
      let current=mariaAnchorRef.current;if(!current){setMariaMarkStatus('No active Maria cursor. Select a cell first.');return;}let entered=0;
      for(const raw of command.values||[]){const checked=validateMariaMark(current,raw);if(!checked.ok){setMariaCursor(current);setMariaMarkStatus(`${entered?`${entered} mark${entered===1?'':'s'} entered. `:''}${checked.message}`);return;}setCell(current.studentId,current.markKey,checked.value,current.maxMarks,current.kind);mariaLastFilledRef.current=current;entered+=1;const next=nextMariaInput(current,mariaDirectionRef.current,1);if(!next){setMariaCursor(current,false);setMariaMarkActive(false);setMariaMarkStatus(`${entered} mark${entered===1?'':'s'} entered. End of this editable ${mariaDirectionRef.current==='vertical'?'column':'row'} reached.`);return;}current=mariaAnchorFromInput(next);setMariaCursor(current);}
      if(entered)setMariaMarkStatus(`${entered} mark${entered===1?'':'s'} entered. Ready for the next value${entered===1?'':'s'}.`);return;
    }
  };
  const handleMariaMarkCommand=(command:MariaMarkEntryCommand)=>{
    if(command.command==='open'){
      const nextTerm=command.term;if(!nextTerm){setMariaMarkStatus('Maria needs First Term or Second Term before opening the Mark List.');return;}
      if(term!==nextTerm){setScopeId('');setTemplate(null);setRoster([]);setRows({});setList(null);setMessage('');setErr('');setTerm(nextTerm);}
      setPendingMariaOpen(command);setMariaMarkStatus('Maria is opening the existing Mark List. No marks are changed until you start voice entry.');return;
    }
    executeMariaMarkControl(command);
  };
  mariaCommandHandlerRef.current=handleMariaMarkCommand;

  useEffect(()=>{const unsubscribe=subscribeMariaDynamicCommands(command=>{if(command.type==='maria_mark_entry')mariaCommandHandlerRef.current(command as MariaMarkEntryCommand)});for(const command of takePendingMariaDynamicCommands('maria_mark_entry'))mariaCommandHandlerRef.current(command as MariaMarkEntryCommand);return unsubscribe;},[]);
  useEffect(()=>{if(!pendingMariaOpen||!term||pendingMariaOpen.term!==term||!scopes.length)return;const assignmentId=String(pendingMariaOpen.assignmentId||'');const subject=normalizeMariaText(pendingMariaOpen.subjectName);const className=normalizeMariaText(pendingMariaOpen.className);const division=normalizeMariaText(pendingMariaOpen.division);const matches=scopes.filter(item=>{if(assignmentId&&(item.id===assignmentId||item.id.includes(assignmentId)))return true;const subjectMatch=!subject||normalizeMariaText(item.subjectName)===subject||normalizeMariaText(item.subjectName).includes(subject)||subject.includes(normalizeMariaText(item.subjectName));return subjectMatch&&(!className||normalizeMariaText(item.className)===className)&&(!division||normalizeMariaText(item.division)===division);});if(matches.length===1){if(scopeId!==matches[0].id)setScopeId(matches[0].id);setPendingMariaOpen(null);setMariaMarkStatus(`Maria opened ${matches[0].subjectName} · ${termLabel[term]}. Waiting for the Mark List cells to be ready.`);}else if(matches.length>1)setMariaMarkStatus('More than one assigned Mark List matched. Please specify Class/Division.');else setMariaMarkStatus('That assigned Mark List was not found in the current Result scope.');},[pendingMariaOpen,term,scopes,scopeId]);
  useEffect(()=>{if(!template||!scope||!roster.length||!markSheetRef.current)return;const queued=pendingMariaControlsRef.current.splice(0);if(!queued.length){if(mariaMarkStatus.includes('Waiting for the Mark List'))setMariaMarkStatus('Mark List ready. Say a column, Student/Roll Number, or tap any editable cell and say “start from here”.');return;}for(const command of queued)executeMariaMarkControl(command);},[template,scope?.id,roster.length,locked]);

  // Required opening structure: only two term buttons. No search/dropdown form.
  if(!term) return <div className="space-y-5">
    {mariaMarkStatus&&<Banner tone="blue"><b>Maria:</b> {mariaMarkStatus}</Banner>}
  ...
  return <div className="space-y-5">
    ...
    {otherEditors>0&&<Banner tone="amber"><b>Concurrent edit warning.</b> {otherEditors} other Classtago session{otherEditors===1?' is':'s are'} on this same Subject Mark List. Save normally; revision/conflict checks remain authoritative.</Banner>}
    {template&&<>
      ...
      {(mariaMarkStatus||mariaMarkActive)&&<Banner tone="blue"><b>Maria Voice Mark Entry {mariaMarkActive?'ON':'ready'}:</b> {mariaMarkStatus||'Ready.'} {mariaAnchor&&<span> Cursor: {mariaAnchor.studentName} · {mariaAnchor.headLabel} · moving {mariaDirection==='vertical'?'down':'sideways'}.</span>} <span className="opacity-80">This is optional; manual entry and the existing Save/Send workflow are unchanged.</span></Banner>}
      <div ref={markSheetRef}><ClerkMasterMarkListSheet schoolName={context.schoolName} teacherName={context.teacherName} scope={scope} term={term} template={template} roster={roster} rows={rows} locked={locked} onChange={setCell} onMariaCellFocus={(cell)=>{mariaAnchorRef.current=cell;setMariaAnchor(cell);if(!mariaMarkActive)setMariaMarkStatus(`Selected ${cell.studentName} · ${cell.headLabel}. Say “start from here” to use this cell as Maria's anchor.`)}} activeMariaCellKey={mariaMarkActive?mariaAnchor?.cellKey:undefined}/></div>
