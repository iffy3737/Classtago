...
export type MariaMarkCellAnchor = {
  cellKey: string; studentId: string; studentName: string; rollNumber: string; markKey: string; headLabel: string; maxMarks?: number; kind?: string; rowIndex: number; columnIndex: number;
};
...
type Props = {
  ...
  onMariaCellFocus?: (cell: MariaMarkCellAnchor) => void;
  activeMariaCellKey?: string;
};
  ...
  return <>
    {props.roster.map((student,index)=>{
      ...
      return <tr key={student.id} className="border-b border-slate-200 hover:bg-slate-50/40">
        ...
        {(master.headers||[]).map((h:any,headerIndex:number)=>{
            ...
            {binding.calculatedKey
              ...
              : binding.markKey
                ? <input
                    disabled={!editable}
                    value={value}
                    data-maria-mark-cell="true"
                    data-maria-cell-key={`${student.id}:${binding.markKey}`}
                    data-maria-student-id={student.id}
                    data-maria-student-name={student.name}
                    data-maria-roll-number={String(student.rollNumber || index + 1)}
                    data-maria-mark-key={binding.markKey}
                    data-maria-head-label={text(h.text)}
                    data-maria-max-marks={h.maxMarks == null ? '' : String(h.maxMarks)}
                    data-maria-kind={props.template.heads.find(x=>x.key===binding.markKey)?.kind || 'mark'}
                    data-maria-row-index={String(index)}
                    data-maria-column-index={String(headerIndex)}
                    onFocus={() => props.onMariaCellFocus?.({cellKey:`${student.id}:${binding.markKey}`,studentId:student.id,studentName:student.name,rollNumber:String(student.rollNumber||index+1),markKey:binding.markKey!,headLabel:text(h.text),maxMarks:h.maxMarks,kind:props.template.heads.find(x=>x.key===binding.markKey)?.kind||'mark',rowIndex:index,columnIndex:headerIndex})}
                    onChange={e=>props.onChange(student.id,binding.markKey!,e.target.value,h.maxMarks,props.template.heads.find(x=>x.key===binding.markKey)?.kind)}
                    placeholder={inactive?'—':'-'}
                    className={`h-9 w-full rounded-md border bg-transparent px-1 text-center font-mono text-xs font-extrabold focus:outline-none disabled:text-slate-400 ${props.activeMariaCellKey===`${student.id}:${binding.markKey}`?'border-emerald-400 bg-emerald-50 ring-2 ring-emerald-200':'border-transparent border-b-slate-200 focus:border-amber-300 focus:bg-amber-50'}`}/>
          ...
          return <td key={h.id} className={`${h.border||'border-r border-slate-200'} p-1 text-center ${h.font||''} ${ownerAllowed?'bg-white':'bg-slate-100/80'}`} style={{minWidth:h.width,textAlign:h.align==='right'?'right':h.align==='left'?'left':'center'}}>
            {binding.calculatedKey ? <div className="px-2 py-2 font-mono text-xs font-black text-emerald-700">{text(value)||'—'}</div>
              : binding.markKey ? <input
                  ...
                  data-maria-roll-number={String(student.rollNumber || index + 1)}
                  data-maria-mark-key={binding.markKey}
                  data-maria-head-label={`${label} ${text(h.text)}`}
                  data-maria-max-marks={h.maxMarks == null ? '' : String(h.maxMarks)}
                  data-maria-kind="mark"
                  data-maria-row-index={String(index*3+subIndex)}
                  data-maria-column-index={String(headerIndex)}
                  onFocus={() => props.onMariaCellFocus?.({cellKey:`${student.id}:${binding.markKey}`,studentId:student.id,studentName:student.name,rollNumber:String(student.rollNumber||index+1),markKey:binding.markKey!,headLabel:`${label} ${text(h.text)}`,maxMarks:h.maxMarks,kind:'mark',rowIndex:index*3+subIndex,columnIndex:headerIndex})}
                  onChange={e=>props.onChange(student.id,binding.markKey!,e.target.value,h.maxMarks,'mark')}
                  placeholder={ownerAllowed?'-':'Other teacher'}
                  className={`h-9 w-full rounded-md border bg-transparent px-1 text-center font-mono text-xs font-extrabold focus:outline-none disabled:text-slate-400 ${props.activeMariaCellKey===`${student.id}:${binding.markKey}`?'border-emerald-400 bg-emerald-50 ring-2 ring-emerald-200':'border-transparent border-b-slate-200 focus:border-amber-300 focus:bg-amber-50'}`}/>
