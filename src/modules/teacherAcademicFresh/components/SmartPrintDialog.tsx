...
export function SmartPrintDialog({ disabled = false, targetId, title = 'Teacher Academic Document', defaultPaper = 'A4', defaultOrientation = 'portrait', moduleName, buttonLabel = 'Smart Print / PDF' }: { disabled?: boolean; targetId?: string; title?: string; defaultPaper?: PrintSettings['paper']; defaultOrientation?: PrintSettings['orientation']; moduleName?: string; buttonLabel?: string }) {
  ...
  const [settings, setSettings] = useState<PrintSettings>(() => ({ ...defaults, paper: defaultPaper, orientation: defaultOrientation, marginMm: moduleName === 'year-plan' ? 5 : moduleName === 'question-paper' ? 10 : defaults.marginMm, showSchoolHeader: moduleName === 'year-plan' || moduleName === 'question-paper' ? false : defaults.showSchoolHeader, showPageNumbers: moduleName === 'year-plan' || moduleName === 'question-paper' ? false : defaults.showPageNumbers }));
  ...
  return (
    <>
      <button className="btn secondary screen-only" disabled={disabled} onClick={() => setOpen(true)}>{moduleName === 'question-paper' ? 'Page Setup & Print' : buttonLabel}</button>
      {open && (
        <div className="modal screen-only" role="dialog" aria-modal="true">
          <div className="modal-card">
            <div className="modal-head"><h3>{moduleName === 'question-paper' ? 'Question Paper — Page Setup' : 'Classtago Smart Print'}</h3><button className="icon-btn" onClick={() => setOpen(false)}>×</button></div>
            <div className="form-grid two">
              <label className="field"><span>Paper Size</span><select value={settings.paper} onChange={(e) => setSettings({ ...settings, paper: e.target.value as PrintSettings['paper'] })}>{['A3','A4','A5','Letter','Legal','Folio','Custom'].map((p) => <option key={p}>{p}</option>)}</select></label>
              ...
              <label className="field"><span>Print Scale (%)</span><input type="number" min={50} max={140} value={settings.scalePercent} onChange={(e) => setSettings({ ...settings, scalePercent: Number(e.target.value) })} /></label>
            </div>
            ...
            <div className="actions"><button className="btn ghost" onClick={() => setOpen(false)}>Cancel</button><button className="btn primary" onClick={print}>{moduleName === 'question-paper' ? 'Apply Page Setup & Print' : 'Open Print / Save PDF'}</button></div>
          </div>
        </div>
      )}
    </>
