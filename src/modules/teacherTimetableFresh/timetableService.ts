...
export async function loadTeacherTimetableData(_context: TeacherCloudContext): Promise<TeacherTimetableData> {
  ...
  if (!response.ok) throw new Error(payload.error || 'Teacher Timetable could not be loaded.');
  return {
    source: 'cloud',
    regularRows: Array.isArray(payload.regularRows) ? payload.regularRows : [],
    classRows: Array.isArray(payload.classRows) ? payload.classRows : [],
    substituteRows: Array.isArray(payload.substituteRows) ? payload.substituteRows : [],
    classTeacherScopes: Array.isArray(payload.classTeacherScopes) ? payload.classTeacherScopes : [],
...
export async function publishHeadmasterTimetable(input: { academicYear: string; timetable: any[] }) {
  ...
  const [years, classesRead, divisionsRead] = await Promise.all([
    ...
    supabase.from('school_classes').select('id,class_name,name').eq('school_id', schoolId),
  ...
  const normLabel = (value: unknown) => String(value || '').trim().toLowerCase().replace(/^class\s+/, '').replace(/^division\s+/, '').replace(/\s+/g, ' ');
  const classRows = classesRead.data || [];
  const divisionRows = divisionsRead.data || [];
  const classByName = new Map(classRows.flatMap((r:any) => { const keys=[r.class_name,r.name].filter(Boolean).map(normLabel); return keys.map((k:string)=>[k,String(r.id)] as const); }));
  const divisionByName = new Map(divisionRows.flatMap((r:any) => { const keys=[r.division_name,r.name].filter(Boolean).map(normLabel); return keys.map((k:string)=>[k,String(r.id)] as const); }));
  ...
  const payload = (input.timetable || []).filter((row: any) => row.teacherName && row.teacherName !== 'Assigned Duty').map((row: any) => {
    ...
    return {
      ...
      class_id: row.classId || row.class_id || classByName.get(normLabel(row.className)) || null,
      division_id: row.divisionId || row.division_id || divisionByName.get(normLabel(row.division)) || null,
