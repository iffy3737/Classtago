...
export function CombinedQuestionPaperBuilder({group,currentAssignment,assignments,materials,exam,combinedTotalMarks,durationMinutes,onTotalMarksChange,onGenerated,onBusy,onError}:Props){
  ...
  const generate=async()=>{
    ...
    setGenerationStage('Preparing the combined paper…');
    onBusy(true);
    let targetCount = 0;
    // Yield twice so React paints the visible progress state before the first
    // AI request starts. This fixes the case where generation succeeds but the
    // button appears unchanged because the first async chain blocks the paint.
    ...
    try{
      ...
      const targets=group.currentTeacherHasAll?group.members:editableMembers;
      targetCount = targets.length;
      if(!targets.length)throw new Error('This Teacher has no editable Subject component in the selected Combined Subject Group.');
      const generatedParts:Array<{member:any;paper:QuestionPaperDraft}>=[];
      for(let targetIndex=0; targetIndex<targets.length; targetIndex++){
        const member=targets[targetIndex];
        setGenerationIndex(targetIndex + 1);
        setGenerationStage(`Generating ${member.subjectName} — Subject ${targetIndex + 1} of ${targets.length}…`);
        const a=assignments.find(x=>x.id===member.assignmentId);const st=states[member.subjectId];
    ...
    }catch(e){onError(e instanceof Error?e.message:'Combined Question Paper generation failed.');}
    finally{
      setGenerationStage('Generation complete. Preparing the paper preview…');
      setGenerationIndex(targetCount);
  ...
  return <div className="combined-paper-builder">
    ...
    {generating && <div className="combined-generation-status" role="status" aria-live="polite"><span className="combined-generation-spinner" aria-hidden="true"/><div className="combined-generation-copy"><strong>{generationStage || 'Generating Combined Question Paper…'}</strong><small>{generationIndex > 0 ? `Subject ${generationIndex} of ${group.currentTeacherHasAll ? group.members.length : editableMembers.length}` : 'Please wait — the paper is being prepared from the selected textbooks.'}</small></div></div>}
    <div className="actions"><button className="btn primary" type="button" disabled={generating} onClick={()=>void generate()}>{generating?'Generating…':group.currentTeacherHasAll?'Generate Complete Combined Paper':'Generate My Subject Section for Clerk'}</button></div>
  </div>;
}
