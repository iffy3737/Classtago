...
const CLASSTAGO_DEFAULT_QUESTION_PAPER_INSTRUCTIONS = new Set([
  'تمام سوالات حل کرنا لازمی ہیں۔', 'تمام سوالات حل کرنا لازمی ہیں',
  'ہر سوال کے سامنے اس کے نمبر درج ہیں۔', 'ہر سوال کے سامنے اس کے نمبر درج ہیں',
  'جوابات اپنی کاپی میں صاف اور واضح لکھیں۔', 'جوابات اپنی کاپی میں صاف اور واضح لکھیں',
]);
const sanitizeQuestionPaperInstructions = (values: unknown) => (Array.isArray(values) ? values : [])
  .map(value => String(value || '').trim()).filter(Boolean)
  .filter(value => !CLASSTAGO_DEFAULT_QUESTION_PAPER_INSTRUCTIONS.has(value)).slice(0, 12);
...
export function QuestionPaperPage({ assignments, materials, initialTerm, initialExam, onBackToTeacher, onBackToTerm }: Props) {
  ...
  const generate = async () => {
    ...
    try {
      ...
      setPaper({ ...generated, instructions: sanitizeQuestionPaperInstructions(generated.instructions), reviewStatus: 'ai_draft' });
  ...
  const save = async () => {
    ...
    try {
      ...
      const cleanedPaper = { ...paper, instructions: sanitizeQuestionPaperInstructions(paper.instructions) };
      const id = await saveQuestionPaper(cleanedPaper);
      setPaper({ ...cleanedPaper, id, reviewStatus: 'final' });
