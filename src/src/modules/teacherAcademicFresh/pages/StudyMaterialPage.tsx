...
type Props = { assignments: TeacherAssignment[]; materials: StudyMaterial[]; reload: () => Promise<void>; viewerRole?: string };
...
export function StudyMaterialPage({ assignments, materials, reload, viewerRole = 'teacher' }: Props) {
  const isHeadmaster = viewerRole === 'headmaster';
  ...
  if (selectedClass == null) {
    return <section className="workspace">
      <header className="page-head"><div><h2>Study Material</h2><p>{isHeadmaster ? 'Choose a class. Classtago shows only subjects assigned to your Headmaster teaching profile.' : 'Choose a class. Classtago then shows only the subjects assigned to you.'}</p></div><span className="pill">{isHeadmaster ? 'Headmaster Teaching Library' : 'Teacher Library'}</span></header>
      <div className="card">
        <h3>Select Class</h3>
  ...
  if (!assignment) {
    return <section className="workspace">
      <header className="page-head"><div><button type="button" className="link-btn" onClick={() => setSelectedClass(null)}>← Classes 1–12</button><h2>Class {selectedClass}</h2><p>{isHeadmaster ? 'Select one of the subjects assigned to your Headmaster teaching profile.' : 'Select one of your Headmaster-assigned subjects.'}</p></div><span className="pill">Study Material</span></header>
  ...
  return <section className="workspace">
    ...
    <div className="card">
      ...
      {category === 'textbook' && sharedSubjectOptions.length > 0 && <div className="mt-3 rounded-xl border border-indigo-200 bg-indigo-50 p-3">
        ...
        <p className="mt-1 text-[11px] leading-5 text-indigo-800">Use this only when one physical PDF contains more than one Subject. Classtago stores the PDF once and tags it to your other assigned Subject(s).</p>
      ...
      {category === 'textbook' && textbookSourceMode === 'file' && <div className="mt-3 rounded-xl border border-emerald-200 bg-emerald-50 p-3 text-xs text-emerald-900"><strong>Large PDF supported.</strong> No manual splitting is needed. Classtago uploads the book in safe parts, automatically retries interrupted parts, and then prepares it for AI Homework. AI-ready Textbook PDFs can be up to 2 GB each. <strong>AI:</strong> Gemini Free Tier only; no paid-model fallback.</div>}
