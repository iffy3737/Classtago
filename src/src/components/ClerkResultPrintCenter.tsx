...
export default function ClerkResultPrintCenter({ user, activeFeatureId }: { user: User; activeFeatureId?: string | null }) {
  ...
  const markPrinted = async (item: QueueItem) => {
    ...
    try {
      ...
      if (session.error || !token) throw session.error || new Error('Secure Clerk session was not found.');
      const response = await fetch(`/api/clerk/result-print-queue/${encodeURIComponent(item.id)}/printed`, {
        method: 'POST', headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' }, body: JSON.stringify({})
      });
      const payload = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(payload.error || 'Print status could not be updated securely.');
      await load();
