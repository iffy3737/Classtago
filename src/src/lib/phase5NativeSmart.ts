...
export async function scanNativeCode() {
  if (!Capacitor.isNativePlatform()) throw new Error('Native ML Kit code scanner is available in the Classtago Android app.');
  return EdunixoSmart.scanCode();
}

export async function scanNativeDocument() {
  if (!Capacitor.isNativePlatform()) throw new Error('Native ML Kit document scanner is available in the Classtago Android app.');
