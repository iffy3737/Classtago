...
export async function registerCurrentUserPasskey() {
  if (!passkeyFeatureEnabled) throw new Error('Passkey enrollment will be enabled after the final Classtago custom domain is locked.');
...
export async function signInWithPasskey() {
  if (!passkeyFeatureEnabled) throw new Error('Passkey sign-in will be enabled after the final Classtago custom domain is locked.');
  if (!passkeySupportedOnDevice()) throw new Error('This browser/device does not support secure passkeys.');
  const auth: any = supabase.auth as any;
