...
import TeacherLeavePersonalHR from '../modules/teacherLeaveFresh/TeacherLeavePersonalHR';
import type { TeacherLeaveView } from '../modules/teacherLeaveFresh/types';
...
const leaveMap: Record<string, TeacherLeaveView> = {
  'tr-leave-apply': 'apply',
  'tr-leave-status': 'status',
  'tr-leave-history': 'history',
  'tr-personal-hr-summary': 'summary',
};
...
export default function TeacherWorkspace({ user, initialModule = 'dashboard', initialFeature = null, onNavigate }: TeacherWorkspaceProps) {
  ...
  useEffect(() => {
    const refresh = () => setContextRefreshNonce(value => value + 1);
    const onVisibility = () => { if (document.visibilityState === 'visible') refresh(); };
    window.addEventListener('class_teacher_assignments_updated', refresh);
    window.addEventListener('focus', refresh);
    document.addEventListener('visibilitychange', onVisibility);
    return () => {
      window.removeEventListener('class_teacher_assignments_updated', refresh);
  ...
  }, [user.id, user.authUserId, user.employeeCode, user.shalarthId, user.name, user.role, contextRefreshNonce]);
  ...
  const leavePage = leaveMap[initialModule];
  if (leavePage) {
    return <div className="edx-teacher-ai"><TeacherLeavePersonalHR user={user} context={context} loading={loading} error={contextError} view={leavePage} /></div>;
  }
  ...
  if (academicPage) {
    return <div className="edx-teacher-ai">
      ...
      {academicPage === 'study-material'
        ? <StudyMaterialPage assignments={aiAssignments} materials={materials} reload={reloadMaterials} viewerRole={user.role} />
