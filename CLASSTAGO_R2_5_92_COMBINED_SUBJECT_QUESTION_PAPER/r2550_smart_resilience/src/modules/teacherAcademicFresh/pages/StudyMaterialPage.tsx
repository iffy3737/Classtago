...
export function StudyMaterialPage({ assignments, materials, reload, viewerRole = 'teacher' }: Props) {
  ...
  if (selectedClass == null) {
    return <section className="workspace">
      <header className="page-head"><div><h2>Study Material</h2><p>{isHeadmaster ? 'Choose a class. EDUNIXO shows only subjects assigned to your Headmaster teaching profile.' : 'Choose a class. EDUNIXO then shows only the subjects assigned to you.'}</p></div><span className="pill">{isHeadmaster ? 'Headmaster Teaching Library' : 'Teacher Library'}</span></header>
      <div className="card">
        <h3>Select Class</h3>
