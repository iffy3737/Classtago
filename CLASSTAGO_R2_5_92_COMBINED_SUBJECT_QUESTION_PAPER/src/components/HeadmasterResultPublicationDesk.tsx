  ...
  return <div className="space-y-5 text-left">
    ...
    {!rows.length?<div className="rounded-3xl border border-dashed border-slate-300 bg-white p-10 text-center"><CheckCircle2 className="mx-auto h-9 w-9 text-emerald-600"/><h3 className="mt-3 font-black text-slate-950">No Progress Card batch yet</h3><p className="mt-2 text-xs text-slate-500">A batch will appear after the Class Teacher completes the Result Book and sends it to Progress Card & Clerk.</p></div>:
      <div className="space-y-4">{rows.map(row=>{
        ...
        const readyForPublish=row.readyForPublish !== false && Number(row.studentCount || 0) > 0 && Boolean(row.resultBookId);
        return <article key={row.id} className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            ...
            {!final&&<div className="w-full xl:max-w-md"><textarea rows={2} value={note[row.id]||''} onChange={e=>setNote(prev=>({...prev,[row.id]:e.target.value}))} placeholder="Optional Headmaster review note" className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-xs"/>{!readyForPublish&&<div className="mt-2 rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-[10px] font-bold text-rose-700">Publication blocked: linked Result Book and at least one student result are required.</div>}<div className="mt-3 flex flex-wrap gap-2">{!reviewed&&<button disabled={Boolean(busy)} onClick={()=>void act(row,'review')} className="inline-flex items-center gap-2 rounded-xl border border-cyan-200 bg-cyan-50 px-4 py-2.5 text-xs font-black text-cyan-800 disabled:opacity-50"><CheckCircle2 className="h-4 w-4"/>Mark Reviewed</button>}<button disabled={Boolean(busy) || !reviewed || !readyForPublish} onClick={()=>void act(row,'publish')} title={!reviewed ? 'Review this batch first.' : (!readyForPublish ? 'Result Book/student results are incomplete.' : 'Publish and lock this reviewed batch.')} className="inline-flex items-center gap-2 rounded-xl bg-slate-950 px-4 py-2.5 text-xs font-black text-white disabled:cursor-not-allowed disabled:opacity-40"><LockKeyhole className="h-4 w-4"/>{reviewed ? 'Publish & Lock' : 'Review First'}</button>{busy.startsWith(`${row.id}:`)&&<Loader2 className="h-5 w-5 animate-spin text-violet-600"/>}</div></div>}
          </div>
        </article>;
      })}</div>}
  </div>;
