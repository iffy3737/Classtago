...
function routeDeepLink(raw: string): void {
  try {
    ...
    if (url.protocol === 'classtago:' || url.protocol === 'edunixo:') {
...
export async function installNativeAppRuntime(): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.warn('[Classtago native] Status bar setup skipped.', error);
    }

    try {
      await Keyboard.setResizeMode({ mode: KeyboardResize.Body });
    } catch (error) {
      console.warn('[Classtago native] Keyboard resize setup skipped.', error);
    }
    ...
    // resources. Keep internal routes inside Classtago and hand external URLs to
    ...
    try {
      ...
      console.warn('[Classtago native] Network status listener skipped.', error);
    }

    // Let React paint the secure launcher before dismissing the native splash.
    window.setTimeout(() => { void SplashScreen.hide(); }, 250);
  } catch (error) {
    console.error('[Classtago native] Native bridge could not initialize.', error);
  }
}
