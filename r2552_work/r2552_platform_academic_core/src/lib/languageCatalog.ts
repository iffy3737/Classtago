...
export function mergeLanguageCatalogues(
  primary: LanguageOption[],
  fallback: LanguageOption[] = FALLBACK_LANGUAGE_CATALOGUE
): LanguageOption[] {
  ...
  for (const option of primary || []) {
    ...
    merged.set(code, { ...option, code });
  }
  for (const option of fallback) {
    const code = normalizeLanguageCode(option.code);
    if (!merged.has(code)) merged.set(code, { ...option, code });
  }
