/**
 * EDUNIXO Phase 1 — encrypted offline cache + durable action queue.
...
export async function getPhase1Cache<T>(scope: Phase1Scope, namespace: string, recordKey: string): Promise<T | null> {
  try {
    ...
    console.warn('[EDUNIXO offline] cache read failed', error);
    return null;
  }
}
  ...
  try {
    ...
    for (const row of rows) {
      try {
        ...
        console.warn('[EDUNIXO offline] queued item could not be decrypted', row.id, error);
      }
    }
...
export async function clearPhase1Scope(scope: Phase1Scope): Promise<void> {
  try {
    ...
    console.warn('[EDUNIXO offline] scope cleanup failed', error);
  }
}
