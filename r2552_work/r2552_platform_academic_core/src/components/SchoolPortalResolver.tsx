  ...
  if (loading) {
    return (
      <div className="flex min-h-[78vh] flex-col items-center justify-center bg-[#050816] p-8 text-center text-white">
        ...
        <p className="mt-3 max-w-lg text-sm leading-7 text-slate-400">EDUNIXO is verifying the institution, active subscription and published website.</p>
      </div>
    );
  }
  ...
  if (error || !school) {
    return (
      <div className="flex min-h-[78vh] flex-col items-center justify-center bg-[#050816] p-8 text-center text-white">
        ...
        <p className="mt-4 max-w-xl text-sm leading-7 text-slate-400">{error || 'This school does not have a published EDUNIXO website.'}</p>
        <button onClick={onBackToPlatform} className="mt-8 flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-sm font-extrabold text-slate-950">
          <ArrowLeft className="h-4 w-4" /> Return to EDUNIXO
        </button>
      </div>
    );
  }
