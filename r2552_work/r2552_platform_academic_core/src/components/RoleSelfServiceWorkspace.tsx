...
export default function RoleSelfServiceWorkspace({ user, module, feature, lang }: RoleSelfServiceWorkspaceProps) {
  ...
  const header = (
    <div className="relative overflow-hidden border-b border-white/10 bg-slate-950 px-5 py-6 text-white sm:px-7">
      <div className="absolute -right-20 -top-28 h-64 w-64 rounded-full bg-cyan-400/15 blur-3xl" />
      <div className="absolute -left-16 bottom-0 h-52 w-52 rounded-full bg-violet-500/15 blur-3xl" />
      <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-start gap-4"><div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl border border-white/10 bg-white/5 text-cyan-300"><HeadingIcon className="h-6 w-6" /></div><div><p className="text-[9px] font-black uppercase tracking-[0.2em] text-cyan-300">Secure self-service workspace</p><h1 className="mt-1 text-xl font-black sm:text-2xl">{module.label}</h1><p className="mt-2 text-xs text-slate-400">{feature?.label || module.description || 'Only role-scoped information is displayed.'}</p></div></div>
        {user.role === 'parent' && linkedChildren.length > 1 && <label className="min-w-56 text-xs font-bold text-slate-300"><span className="mb-1.5 flex items-center gap-2"><Users className="h-4 w-4" />Viewing child</span><select value={selectedStudent?.id || ''} onChange={event => setSelectedChildId(event.target.value)} className="w-full rounded-xl border border-white/10 bg-slate-900 px-3 py-2.5 text-sm font-bold text-white">{linkedChildren.map(child => <option key={child.id} value={child.id}>{child.name}</option>)}</select></label>}
      </div>
    </div>
  );
