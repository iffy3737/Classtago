    ...
    <section className="workspace">
      ...
      <div className="card screen-only">
        {(combinedLoading || combinedGroups.length > 0) && <div className="combined-paper-mode-box">
          ...
          {paperMode === 'combined' && combinedGroups.length > 0 && <label className="field combined-group-picker"><span>Combined Subject</span><select value={combinedGroupId} onChange={(e) => { setCombinedGroupId(e.target.value); setPaper(null); setError(''); }}><option value="">Select Combined Subject</option>{combinedGroups.map((group) => <option key={group.id} value={group.id}>{group.groupName}</option>)}</select><small>{selectedCombinedGroup?.currentTeacherHasAll ? 'All component Subjects are assigned to you, so EDUNIXO will generate one complete combined paper.' : selectedCombinedGroup ? 'Different Teachers are assigned. You will prepare only your section; Clerk will review and combine all submitted sections.' : 'Choose a Subject Group.'}</small></label>}
        </div>}

        {paperMode === 'combined' ? (selectedCombinedGroup && exam && assignment ? <>
          <div className="form-grid two combined-paper-settings">
            <label className="field"><span>Combined Total Marks</span><input type="number" min={1} value={totalMarks} disabled={!selectedCombinedGroup.currentTeacherHasAll && Boolean(selectedCombinedGroup.hasSubmittedComponents)} onChange={(e) => { setTotalMarks(Number(e.target.value)); setPaper(null); }} /><small>{!selectedCombinedGroup.currentTeacherHasAll && selectedCombinedGroup.hasSubmittedComponents ? `Locked to ${selectedCombinedGroup.configuredTotalMarks || totalMarks} marks from the first submitted Subject section.` : 'All component Subject marks must add up to this total.'}</small></label>
        ...
        {!patternLoading && !schoolPattern.available && <div className="question-auto-note"><strong>School Pattern:</strong> No saved pattern is configured for this Exam/Class/Subject. You can keep the School Pattern preference ticked, but EDUNIXO will safely use the editable Manual Paper Pattern until a saved pattern is available.</div>}
